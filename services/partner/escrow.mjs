...
export class EscrowService {
  constructor({ jobs, payments, aiVerifier, db, notifier }) {
    this.jobs = jobs; this.payments = payments; this.aiVerifier = aiVerifier; this.db = db; this.notifier = notifier;
  }
  async createJob({ partnerId, customerId, services, currency, paymentProvider }) {
    ...
    const payment = await this.payments.createPayment({ provider: paymentProvider, paymentId: `job-payment:${job.id}`, amountMinor: job.amountMinor, currency: job.currency, customer: { id: customerId }, metadata: { jobId: job.id, partnerId } });
  ...
  async recordEvidence(jobId, { submittedBy, files = [], metadata = {} }) {
    ...
    const evidence = { id: evidenceId, jobId, submittedBy, files, metadata, status: 'submitted', createdAt: new Date().toISOString() };
  ...
  async verifyCompletion(jobId, evidence) {
    ...
    return this.jobs.verifyJob(jobId, { passed: Boolean(verification.passed), confidence: Number(verification.confidence ?? 0), issues: Array.isArray(verification.issues) ? verification.issues : [], evidenceId: verification.evidenceId ?? null, model: verification.model ?? null });
  }
  async releaseAfterHumanApproval(jobId, { reviewerId, partnerRateBps, idempotencyKey }) {
    ...
    return this.jobs.releaseJob(jobId, { actor: { type: 'human', id: reviewerId }, partnerRateBps, idempotencyKey });
  }
}
