...
class MastercardA2AService {
  ...
  generateOAuthSignature(method, url, payload = '') {
    ...
    const signature = crypto.createSign('RSA-SHA256').update(baseString).sign(privateKey, 'base64');
  ...
  _getPrivateKey() {
    try {
      ...
      return fs.readFileSync(this.config.keystorePath.replace('.p12', '.pem'), 'utf8');
    } catch (error) {
