...
const Tools = (() => {
    ...
    'router.users': {
      ...
      format: r => {
        ...
        return users
          .map(
            u =>
              `${u.name || u.username || u['.id']}  profile=${u.profile || '—'}  disabled=${u.disabled || 'false'}`
          )
          .join('\n');
      },
    },
  ...
  function get(toolId) {
    return _reg[toolId] || null;
  }

  function list() {
    return Object.entries(_reg).map(([id, t]) => ({
      id,
      label: t.label,
      group: t.group,
  ...
  function getAgentSpec() {
    return list()
      .map(t => `  - ${t.id}${t.dangerous ? ' [DANGEROUS]' : ''}: ${t.label}`)
      .join('\n');
  }
