...
const Client = (() => {
  ...
  const v1 = {
    ...
    ping: (h, n) =>
      request(`${CFG.API}/system/ping`, { method: 'POST', body: { host: h, count: n || 4 } }),
    ...
    addUser: (u, p, pl) =>
      request(`${CFG.API}/users/add`, {
        method: 'POST',
        body: { username: u, password: p, profile: pl },
      }),
    ...
    redeemVoucher: (c, u) =>
      request(`${CFG.API}/vouchers/redeem`, { method: 'POST', body: { code: c, user: u } }),
    voucherQR: c => request(`${CFG.API}/vouchers/${encodeURIComponent(c)}/qr`),
    bulkVouchers: (n, p) =>
      request(`${CFG.API3}/bulk/vouchers`, { method: 'POST', body: { count: n, plan: p } }),
    ...
    execute: (t, p) =>
      request(`${CFG.API}/execute`, { method: 'POST', body: { tool: t, params: p || {} } }),
  ...
  const v2 = {
    ...
    ask: (p, s) =>
      request(`${CFG.API2}/ask`, { method: 'POST', body: { prompt: p, sessionId: s } }),
  ...
  const v3 = {
    ...
    bulkVouchers: (n, p) =>
      request(`${CFG.API3}/bulk/vouchers`, { method: 'POST', body: { count: n, plan: p } }),
    bulkDisconnect: ids =>
      request(`${CFG.API3}/bulk/users/disconnect`, { method: 'POST', body: { ids } }),
  };

  return { configure, request, v1, v2, v3, getBase: () => _base, hasConfig: () => !!_base };
})();
