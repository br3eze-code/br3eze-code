...
class ModelArmor {
  ...
  async initialize() {
    ...
    if (GoogleAuth && this.config.project && this.config.endpoint) {
      try {
        ...
        console.warn(
          '[ModelArmor] Could not init Google auth, falling back to local heuristics:',
          err.message
        );
      }
    } else {
  ...
  async screenInput(message) {
    ...
    return this._useRemote ? this._remoteScreen(message, 'input') : this._localScreen(message);
  }
  ...
  async screenOutput(message) {
    if (!this.config.enabled) return { blocked: false, content: message };
    return this._useRemote ? this._remoteScreen(message, 'output') : this._localScreen(message);
  }
  ...
  async _remoteScreen(message, direction) {
    ...
    const urlPath =
      `/v1/projects/${this.config.project}/locations/${this.config.location}` +
  ...
  _httpsPost(baseUrl, path, body, token) {
    return new Promise((resolve, reject) => {
      ...
      req.on('timeout', () => {
        req.destroy();
        reject(new Error('Model Armor request timed out'));
      });
      req.on('error', reject);
      req.write(data);
      req.end();
  ...
  _localScreen(message) {
    ...
    if (violations.length > 0) {
      return {
        blocked: true,
        reason: `Local policy violation: ${violations.join(', ')}`,
        content: null,
      };
    }
  ...
  _detectViolations(text) {
    ...
    if (/<script[\s>]|javascript:/i.test(text)) violations.push('xss_payload');
    return violations;
  }
}
