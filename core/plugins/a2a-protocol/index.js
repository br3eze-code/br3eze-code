      ...
      try {
        ...
        if (!adapter) {
          return res.status(404).json({
            ...
            error: {
              code: 'NOT_FOUND',
              message: `Agent ${req.params.agentId} not found or A2A not enabled`,
            },
          });
        }
  ...
  async onCapabilityRegister(ctx, agent, name, def) {
    ...
    if (adapter) {
      adapter.registerCapability(
        name,
        def,
        agent.capabilities.get(name).handler,
        agent.capabilities.get(name).streamingHandler
      );
    }
  }
}
