...
class A2AProtocolAdapter extends EventEmitter {
  ...
  async *sendStreamingTask(targetAgentSPIFFE, task) {
    ...
    while (true) {
      ...
      const rawResp = await this.transport.send(
        await this.identity.signMessage(statusMsg),
        targetAgentSPIFFE
      );
      ...
      if (statusResp.type === 'ERROR')
        throw new A2AError(statusResp.error.code, statusResp.error.message);
  ...
  async handleIncomingMessage(rawMessage) {
    try {
      ...
      switch (verifiedMessage.type) {
        case 'TASK_REQUEST':
          return await this.handleTaskRequest(verifiedMessage);
        case 'TASK_STREAM_REQUEST':
          return await this.handleStreamingTaskRequest(verifiedMessage);
        case 'TASK_STREAM_STATUS':
          return await this.handleStreamStatus(verifiedMessage);
        case 'CAPABILITY_QUERY':
          return await this.handleCapabilityQuery(verifiedMessage);
        case 'CAPABILITY_ANNOUNCE':
          return await this.handleCapabilityAnnounce(verifiedMessage);
        case 'HEALTH_CHECK':
          return await this.handleHealthCheck(verifiedMessage);
        default:
          return await this.buildErrorResponse(
            `Unknown type: ${verifiedMessage.type}`,
            'UNIMPLEMENTED'
          );
      }
    } catch (error) {
  ...
  async handleStreamingTaskRequest(message) {
    ...
    if (!capDef.streamingHandler) {
      return await this.buildErrorResponse(
        `Capability '${capability}' does not support streaming`,
        'UNIMPLEMENTED'
      );
    }
  ...
  async runStreamingCapability(capDef, parameters, originalMessage, streamId, traceId) {
    ...
    try {
      for await (const update of capDef.streamingHandler(
        parameters,
        session,
  ...
  startRateLimitCleanup() {
    this.rateLimitInterval = setInterval(() => {
      ...
      for (const [spiffeID, bucket] of this.rateLimitBuckets) {
        ...
        valid.length === 0
          ? this.rateLimitBuckets.delete(spiffeID)
          : this.rateLimitBuckets.set(spiffeID, valid);
      }
    }, 30000);
