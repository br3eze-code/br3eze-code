...
module.exports = async function queryNetwork(parameters, session, senderSPIFFE, a2aAdapter) {
  ...
  if (!capability) {
    throw new Error(
      `Unknown query_type "${query_type}". Valid: ${Object.keys(QUERY_TYPE_MAP).join(', ')}`
    );
  }
...
async function _directMikroTikQuery(queryType) {
  ...
  try {
    ...
    throw new Error(
      'Network-agent unavailable and MikroTikManager cannot be loaded from this context'
    );
  }
  ...
  try {
    switch (queryType) {
      case 'active_users':
        return { source: 'direct', query_type: queryType, data: await mtk.getActiveUsers() };
      case 'stats':
        return { source: 'direct', query_type: queryType, data: await mtk.getSystemResources() };
      case 'interfaces':
        return { source: 'direct', query_type: queryType, data: await mtk.getInterfaces() };
      case 'dhcp':
        return { source: 'direct', query_type: queryType, data: await mtk.getDHCPLeases() };
      default:
        throw new Error(`Unhandled query_type: ${queryType}`);
    }
  } finally {
