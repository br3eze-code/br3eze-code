...
create unique index if not exists payment_events_provider_transaction_type_idx on public.payment_events(provider, transaction_id, event_type);
create unique index if not exists payment_events_idempotency_idx on public.payment_events(idempotency_key) where idempotency_key is not null;
create index if not exists payment_events_reference_idx on public.payment_events(reference);
create index if not exists payment_events_occurred_at_idx on public.payment_events(occurred_at desc);
...
create unique index if not exists payment_transactions_provider_reference_idx on public.payment_transactions(provider, reference);
create unique index if not exists payment_transactions_idempotency_idx on public.payment_transactions(idempotency_key) where idempotency_key is not null;
...
create index if not exists payment_ledger_transaction_idx on public.payment_ledger_entries(transaction_id, created_at);
...
create index if not exists payment_settlements_provider_reference_idx on public.payment_settlements(provider, provider_reference);
...
create index if not exists payment_reconciliation_transaction_idx on public.payment_reconciliation(transaction_id);
