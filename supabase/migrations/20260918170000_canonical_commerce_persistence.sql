-- Canonical commerce persistence: products/services, carts, orders, invoices and inventory.
-- Financial settlement remains in payment_* tables. Firebase keeps its adapter as fallback.
...
create unique index if not exists carts_user_channel_idx
  on public.carts(user_id, channel, channel_id, tenant_id, site_id, domain)
...
create table if not exists public.orders (
  ...
  shipping numeric(20,8) not null default 0,
  ...
  stock_reservation_status text not null default 'none',
  metadata jsonb not null default '{}'::jsonb,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
...
-- Keep inventory consistent with the existing product quantity column.
insert into public.product_inventory(product_id, quantity)
select id, greatest(quantity, 0) from public.products
...
grant all on public.product_inventory, public.service_inventory to service_role;
...
-- Atomic stock reservation + order creation. Client never writes inventory or order rows directly.
...
begin
  ...
  if p_total < 0 or p_subtotal < 0 or p_shipping < 0 then
  ...
  loop
    ...
    if available is null or available < qty then
      raise exception 'insufficient stock for %', pid;
    end if;
  end loop;
  ...
  insert into public.orders(
    id,user_id,channel,channel_id,tenant_id,site_id,domain,items,subtotal,shipping,total,currency,
    status,payment_method,payment_transaction_id,invoice_id,invoice_number,shipping_address,billing_address,
    fulfillment_status,stock_reservation_status
  ) values (
    p_order_id,p_user_id,p_channel,p_channel_id,p_tenant_id,p_site_id,p_domain,p_items,p_subtotal,p_shipping,p_total,
    p_currency,case when p_payment_method = 'cod' then 'pending_payment' else 'paid' end,
    p_payment_method,p_payment_transaction_id,p_invoice_id,p_invoice_number,p_shipping_address,p_billing_address,
  ...
  values (p_invoice_id,p_order_id,p_user_id,p_invoice_number,p_items,p_subtotal,p_shipping,p_total,p_currency,p_billing_address,
    case when p_payment_method = 'cod' then 'unpaid' else 'paid' end);

  return jsonb_build_object('orderId', p_order_id, 'invoiceId', p_invoice_id, 'status',
    case when p_payment_method = 'cod' then 'pending_payment' else 'paid' end);
end;
$$;
