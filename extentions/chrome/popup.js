...
document.getElementById('copy-actions').onclick = () => {
  ...
  setTimeout(
    () => (document.getElementById('copy-actions').textContent = 'Copy Actions JSON'),
    1500
  );
};
...
async function callSkill(endpoint) {
  ...
  try {
    const res = await fetch(`${gateway}${endpoint}?token=${token}`);
    const data = await res.json();
    document.getElementById('skill-status').textContent =
      `✅ ${data.message?.slice(0, 50) || 'Done'}`;
  } catch (e) {
    document.getElementById('skill-status').textContent = `❌ ${e.message}`;
  }
