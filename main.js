...
function getProfileDir() {
  const profile =
    process.env.AGENTOS_PROFILE || (process.argv.includes('--dev') ? 'dev' : 'default');
...
program
  ...
  .action(async mode => {
    ...
    console.log(
      chalk.gray('  Config:   '),
      fs.existsSync(CONFIG_PATH) ? chalk.green('Found') : chalk.red('Missing')
    );
    ...
    envKeys.forEach(k => {
      ...
      console.log(
        `    ${chalk.gray(k.padEnd(20))}: ${val ? chalk.green('✓ Set') : chalk.yellow('○ Not Set')}`
      );
    });
...
const run = async () => {
  ...
  const currentCommand =
    program.args[0] || (process.argv.some(arg => daemonCommands.includes(arg)) ? 'gateway' : null);

  const isDashboard = currentCommand === 'dashboard';
  const isRefreshing = process.argv.includes('--refresh');

  const shouldExit = !daemonCommands.includes(currentCommand) && (!isDashboard || !isRefreshing);
