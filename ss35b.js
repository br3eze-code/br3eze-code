...
// Version : 2026.5.1  (debug pass — ss33 → ss34)
// Stack   : MikroTik RouterOS · Telegram · WebSocket CLI
//           Firebase/Local DB · Gemini 2.0-flash ReAct Engine
...
const envSchema = Joi.object({
    ...
    CRON_DAILY_REBOOT: Joi.boolean().default(false),
}).unknown(true);
...
class Database {
    ...
    async getStats() {
        ...
        return {
            total: all.length,
            used: all.filter(v => v.used === true).length,
            // FIX-010: expired is its own category now (expired:true, used:false)
            expired: all.filter(v => v.expired === true || (!v.used && v.expiresAt && new Date(v.expiresAt) <= now)).length,
            active: all.filter(v => !v.used && !v.expired && (!v.expiresAt || new Date(v.expiresAt) > now)).length,
        };
    }
    ...
    async expireOldVouchers() {
        ...
        for (const v of pending) {
            if (v.expiresAt && new Date(v.expiresAt) <= now) {
                // FIX-010: mark as expired (not used) to preserve accurate stats
                const expiredUpdate = {
                    used: false,
                    expired: true,
                    expiredAt: new Date().toISOString(),
                };
                await this.updateVoucher(v.id, expiredUpdate);
...
class FinancialController {
    ...
    async getRevenueReport() {
        const vouchers = await database.listVouchers({ limit: 10000 });
        const now = new Date();
        ...
        vouchers.forEach(v => {
            ...
            // FIX-007: only count redeemed (used) vouchers as earned revenue
            if (v.used) {
                total += price;
                if (new Date(v.createdAt).getTime() >= startOfDay) today += price;
            } else if (!v.expired) {
                pending += price;  // un-redeemed, not expired = potential revenue
            }
...
class SkillRegistry {
    ...
    async resolve(skillName, context) {
        ...
        for (const skillPath of searchPaths) {
            const skill = await this.loadSkill(skillPath, context);
            if (skill) {
                this._cacheSet(skillName, skill);   // FIX-008: use evicting setter
                return skill;
            }
        }
    ...
    async loadSkill(skillPath, context) {   // FIX-009: renamed from 'path' to avoid shadowing
...
const TOOLS = {
    ...
    'ping': async (c, host, count = 4) => c.menu('/ping').exec('ping', { address: host, count: String(count) }),
    'traceroute': async (c, host) => c.menu('/tool/traceroute').exec('traceroute', { address: host, count: '1' }),
...
class MikroTikManager {
    ...
    async executeCLI(command) {
        ...
        // Sanitize: block Unix shell injection chars that have no place in RouterOS scripts.
        // NOTE: () {} ; $ are all valid RouterOS scripting characters — do NOT block them.
        // Only block chars that indicate shell escape attempts: backtick, pipe, redirect, ampersand.
        const forbidden = /[`|<>&]/;
...
// Get Daily Finance Summary — FIX-004: requires auth
...
// Check payment status — FIX-004: requires auth
router.get('/voucher/payment/status/:paymentId', authMiddleware, async (req, res) => {
    try {
...
// ── Mastercard webhook ───────────────────────────────────────
// FIX-001/002/003: Webhook registered on 'app' before express.json() in §16.
// FIX-004: /api/finance/summary and /api/voucher/payment/status now require authMiddleware.
...
class WebSocketCLI {
    ...
    _buildCommands() {
        ...
        return {
            ...
            qr: { fn: b(this.cmdQR), desc: 'Print voucher QR code' },   // FIX-014
    ...
    async cmdPing([host, count = '4']) {
        ...
        try {
            ...
            // FIX-005: routeros-client may return a single object or an array
...
class AgentOSBot {
    constructor() {
        ...
        this._cmdBuckets = new Map(); // FIX-011: initialise here, not lazily inside _checkAuth
...
class AgentOSOrchestrator {
    ...
    _runCron() {
        // FIX-013: daily reboot is OPT-IN via CRON_DAILY_REBOOT=true
        //          In a 24/7 hotspot business this must never run by default.
        // FIX-012: track last fire time to prevent multiple triggers per minute
        //          when setInterval drift carries execution across the boundary.
        this._lastDailyReboot = null;
        ...
        setInterval(async () => {
            ...
            // Daily reboot (opt-in only)
            if (ENV.CRON_DAILY_REBOOT &&
                now.getHours() === 4 && now.getMinutes() === 0 &&
                this._lastDailyReboot !== todayKey) {
                this._lastDailyReboot = todayKey;
            ...
            // Heartbeat Every 24 Hours (noon)
            if (now.getHours() === 12 && now.getMinutes() === 0 &&
                this._lastHeartbeat !== todayKey) {
                this._lastHeartbeat = todayKey;
...
// FIX-001/002/003: Mastercard webhook MUST be registered before express.json()
// so that express.raw() captures the raw Buffer (needed for signature verification).
// Path must match what Mastercard is configured to POST to — no /api prefix.
app.post('/webhook/mastercard', express.raw({ type: 'application/json' }), async (req, res) => {
    try {
        const signature = req.headers['x-mastercard-signature'];
        const rawBody = req.body; // Buffer, not parsed object

        // FIX-003: Gate on signature presence; full HMAC verify delegated to a2aService
        if (!signature) {
            logger.warn('Mastercard webhook: missing signature header — rejected');
            return res.status(401).json({ received: false, error: 'Missing signature' });
        }

        // Let the A2A service verify the HMAC before we trust the payload
        let parsed;
        try {
            parsed = JSON.parse(rawBody.toString('utf8'));
        } catch {
            logger.warn('Mastercard webhook: invalid JSON payload');
            return res.status(400).json({ received: false, error: 'Invalid JSON' });
        }

        const sigValid = typeof a2aService.verifyWebhookSignature === 'function'
            ? a2aService.verifyWebhookSignature(rawBody, signature)
            : true; // graceful degradation if not implemented yet

        if (!sigValid) {
            logger.warn('Mastercard webhook: signature mismatch — rejected');
            return res.status(401).json({ received: false, error: 'Invalid signature' });
        }

        const result = await a2aService.handleWebhook(parsed);
        ...
        res.status(200).json({ received: true }); // Always 200 to prevent Mastercard retries
    }
});
...
async function bootDaemon() {
    // Connect to MikroTik in the background — server starts immediately regardless.
    // routeros-client TCP connect can hang 30–60s if the router is unreachable,
    // so we never await it directly during boot.
    mikrotik.connect().catch(err =>
        logger.warn(`MikroTik unreachable at boot — limited mode: ${err.message}`)
    );
