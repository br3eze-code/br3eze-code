...
if (envError) {
  console.error(`[AgentOS] ENV error: ${envError.message}`);
  process.exit(1);
}
...
const CONFIG = {
  ...
  TELEGRAM: {
    ...
    ALLOWED_CHATS: ENV.ALLOWED_CHAT_IDS ? ENV.ALLOWED_CHAT_IDS.split(',').filter(Boolean) : [],
...
const voucherCode = () =>
  CONFIG.VOUCHER_PREFIX + crypto.randomBytes(3).toString('hex').toUpperCase();
...
const TerminalAnimator = {
  ...
  async showSpinner(message, durationMs = 1000) {
    ...
    while (Date.now() < end) {
      process.stdout.write(
        `\r  ${A.CYBER_PURPLE}${frames[i % frames.length]}${A.RESET} ${message}`
      );
      await sleep(80);
      i++;
    }
  ...
  progressBar(label, progress, total = 100, width = 30) {
    ...
    process.stdout.write(
      `\r  ${A.DIM}${label.padEnd(15)}${A.RESET} [${A.PRIMARY}${bar}${A.RESET}] ${A.BOLD}${pct}%${A.RESET}`
    );
    if (p >= 1) console.log();
  },
  ...
  printHeader(title) {
    ...
    console.log(
      `  ${A.DIM}║${A.RESET} ${this.gradient(center, [0, 229, 255], [181, 102, 255])} ${A.DIM}║${A.RESET}`
    );
...
class CostTracker {
  ...
  snapshot() {
    return {
      ...
      estimatedUSD: (
        this.totalInputTokens * 0.00000025 +
        this.totalOutputTokens * 0.00000075
      ).toFixed(6),
    };
  }
}
      ...
      .map(msg => {
        ...
        if (msg.role === MessageRole.ASSISTANT) {
          const parts = msg.blocks
            .map(b => {
              ...
              if (b.type === 'tool_use')
                return { functionCall: { name: b.name, args: JSON.parse(b.input || '{}') } };
              return null;
            })
            .filter(Boolean);
        ...
        if (msg.role === MessageRole.TOOL) {
          ...
          return {
            role: 'user',
            parts: [
              { functionResponse: { name: block.toolName, response: { content: block.output } } },
            ],
          };
        }
        return null;
...
class PermissionPolicy {
  ...
  requireFor(toolName, mode) {
    this._toolRequirements.set(toolName, mode);
    return this;
  }
  ...
  check(toolName) {
    ...
    return {
      allowed: false,
      reason: `Tool "${toolName}" requires ${required}; active mode is ${this.activeMode}`,
    };
  }
...
class Database {
  ...
  _init() {
    ...
    try {
      ...
      const key = ENV.FIREBASE_PRIVATE_KEY.replace(/^['"]|['"]$/g, '') // Remove surrounding quotes
        .replace(/\\n/g, '\n'); // Convert literal \n to real newlines
  ...
  _calcExpiry(plan, duration) {
    if (duration) {
      ...
      if (m)
        return new Date(Date.now() + m[1] * (m[2] === 'h' ? 3_600_000 : 86_400_000)).toISOString();
    }
  ...
  async updateVoucher(code, updates) {
    if (this.db) {
      ...
      const v = this._local.get(code);
      if (v) {
        this._local.set(code, { ...v, ...updates });
        this._saveLocal();
      }
    }
  }
  ...
  async expireOldVouchers() {
    ...
    for (const v of pending) {
      if (v.expiresAt && new Date(v.expiresAt) <= now) {
        ...
        /* eslint-disable no-use-before-define -- mikrotik is a module-level const initialized before any of these methods actually run */
...
class FinancialController {
  ...
  async getTrends() {
    ...
    const days = Array.from({ length: 7 }, (_, i) => {
      ...
      const revenue = created.reduce(
        (s, v) => s + (this.pricing[v.plan] || this.pricing.default),
        0
      );
    ...
    const planMix = {};
    vouchers.forEach(v => {
      planMix[v.plan] = (planMix[v.plan] || 0) + 1;
    });
  ...
  parseFrontmatter(content) {
    ...
    match[1].split('\n').forEach(line => {
      ...
      if (k && v.length) {
        ...
        if (val.startsWith('[')) {
          try {
            attributes[k.trim()] = JSON.parse(val);
          } catch {
...
const TOOLS = {
  ...
  'system.reboot': async c => {
    await c.menu('/system').exec('reboot');
  ...
  'user.kick': async (c, username) => {
    ...
    if (active.length) {
      await c.menu('/ip/hotspot/active').remove(active[0]['.id']);
  ...
  'dns.flush': async c => {
    await c.menu('/ip/dns/cache').exec('flush');
    return { action: 'flushed', service: 'dns' };
  },
  ...
  'firewall.block': async (c, target, list = 'blocked') => {
    await c
      ...
      .add({ list, address: target, comment: 'Blocked via AgentOS' });
  ...
  'interface.monitor-traffic': async (c, iface) =>
    c.menu('/interface').exec('monitor-traffic', { interface: iface, once: true }),
...
class MikroTikManager {
  ...
  getLogs(n) {
    return this.executeTool('system.logs', n);
  }
  getActiveUsers() {
    return this.executeTool('users.active');
  }
  getAllHotspotUsers() {
    return this.executeTool('users.all');
  }
  addHotspotUser(u, p, pr) {
    return this.executeTool('user.add', u, p, pr);
  }
  removeHotspotUser(u) {
    return this.executeTool('user.remove', u);
  }
  kickUser(u) {
    return this.executeTool('user.kick', u);
  }
  getUserStatus(u) {
    return this.executeTool('user.status', u);
  }
  ping(h, c) {
    return this.executeTool('ping', h, c);
  }
  traceroute(h) {
    return this.executeTool('traceroute', h);
  }
  getDhcpLeases() {
    return this.executeTool('dhcp.leases');
  }
  getInterfaces() {
    return this.executeTool('interfaces');
  }
  getArpTable() {
    return this.executeTool('arp.table');
  }
  getFirewallRules(t) {
    return this.executeTool('firewall.list', t);
  }
  addToBlockList(a, l) {
    return this.executeTool('firewall.block', a, l);
  }
  unblockAddress(a, l) {
    return this.executeTool('firewall.unblock', a, l);
  }
  reboot() {
    return this.executeTool('system.reboot');
  }
}
  ...
  try {
    ...
    if (result?.status === 'COMPLETED' && result?.userId) {
      ...
      if (voucher) {
        ...
        global.agentBot?.sendToAll(
          `💰 *Payment Received:* Voucher \`${voucher.id}\` deposited to wallet for user ${result.userId}`
        );
      }
    }
      ...
      },
      {
        ...
        parameters: {
          ...
          properties: {
            action: {
              type: 'string',
              enum: ['revenue_report', 'verify_payment', 'audit_log', 'trends'],
            },
    ...
    if (tier1) {
      try {
        return {
          tier: 1,
          type: 'tool',
          result: await this.mikrotik.executeTool(tier1.name, ...tier1.args),
        };
      } catch (e) {
...
class WebSocketCLI {
  ...
  handleInput(input) {
    ...
    if (input === '\r' || input === '\n') {
      ...
      if (this.cursorPos < this.buffer.length) {
        this.cursorPos++;
        this._out({ type: 'cursor', pos: this.cursorPos });
      }
    } else if (input === '\u001b[D') {
      // Arrow left
      if (this.cursorPos > 0) {
      ...
      this.buffer =
        this.buffer.slice(0, this.cursorPos) + input + this.buffer.slice(this.cursorPos);
      this.cursorPos++;
      this._updateLine();
    }
  }
  ...
  async cmdStatus() {
    try {
      ...
      this._out({
        type: 'table',
        title: `Router Status (${CONFIG.MIKROTIK.IP})`,
        data: {
          'CPU Load': `${s['cpu-load']}%`,
  ...
  async cmdRawCli(args) {
    const cmd = args.join(' ');
    if (!cmd) {
      this._out({ type: 'error', message: 'Usage: cli <command>' });
      return;
    }
    try {
  ...
  async cmdRawApi(args) {
    const cmd = args.join(' ');
    if (!cmd) {
      this._out({ type: 'error', message: 'Usage: api </path/command>' });
      return;
    }
    try {
  ...
  async cmdVoucher([plan, duration]) {
    if (!plan) {
      this._out({ type: 'error', message: 'Usage: voucher <plan> [duration]' });
      return;
    }
    try {
      const code = voucherCode();
      ...
      this._out({
        type: 'success',
        message: `🎫 Code: ${code}  Plan: ${plan}${mikrotik.isConnected ? '\n✅ Auto-provisioned' : ''}`,
      });
    } catch (err) {
      this._out({ type: 'error', message: err.message });
    }
  ...
  async cmdVouchers([limit = '20']) {
    try {
      ...
      this._out({
        type: 'list',
        title: `Recent Vouchers (${list.length})`,
        items: list.map(v => {
          const tag = v.used
            ? '✅ USED'
  ...
  async cmdRedeem([code, username]) {
    if (!code || !username) {
      this._out({ type: 'error', message: 'Usage: redeem <code> <username>' });
      return;
    }
    try {
  ...
  async cmdRevoke([code]) {
    if (!code) {
      this._out({ type: 'error', message: 'Usage: revoke <code>' });
      return;
    }
    try {
      await database.deleteVoucher(code);
  ...
  async cmdPing([host, count = '4']) {
    if (!host) {
      this._out({ type: 'error', message: 'Usage: ping <host> [count]' });
      return;
    }
    try {
      ...
      this._out({
        type: 'result',
        text: `Sent: ${count}  Received: ${recv}  Lost: ${count - recv}`,
      });
    } catch (err) {
      this._out({ type: 'error', message: err.message });
    }
  ...
  async cmdDhcp() {
    try {
      ...
      this._out({
        type: 'table',
        title: `DHCP Leases (${leases.length})`,
        data: leases.slice(0, 20).reduce((acc, l) => {
          acc[l.address] = `${l.hostname || 'N/A'} (${l.status || 'bound'})`;
          return acc;
        }, {}),
      });
    } catch (err) {
  ...
  async cmdArp() {
    try {
      ...
      this._out({
        type: 'table',
        title: `ARP Table (${arp.length})`,
        data: arp
          .filter(e => e.address)
          .slice(0, 20)
          .reduce((acc, e) => {
            acc[e.address] = e['mac-address'] || 'N/A';
            return acc;
          }, {}),
      });
    } catch (err) {
  ...
  async cmdFirewall() {
    try {
      ...
      this._out({
        type: 'list',
        title: `Firewall Filter (${rules.length})`,
        items: rules
          .slice(0, 10)
  ...
  async cmdBlock([target]) {
    if (!target) {
      this._out({ type: 'error', message: 'Usage: block <ip-or-mac>' });
      return;
    }
    try {
  ...
  async cmdUnblock([target]) {
    if (!target) {
      this._out({ type: 'error', message: 'Usage: unblock <ip-or-mac>' });
      return;
    }
    try {
      ...
      this._out({
        type: 'success',
        message: `Unblocked: ${target} (${res.count} entries removed)`,
      });
    } catch (err) {
      this._out({ type: 'error', message: err.message });
    }
  ...
  async cmdAgent(args) {
    ...
    if (!query) {
      this._out({ type: 'error', message: 'Usage: agent <query>' });
      return;
    }
  ...
  async cmdUsers() {
    try {
      ...
      this._out({
        type: 'table',
        title: `Hotspot Users (${list.length})`,
        data: list.slice(0, 50).reduce((acc, u) => {
          acc[u.name] = u.profile;
          return acc;
        }, {}),
      });
    } catch (err) {
  ...
  async cmdActive() {
    try {
      ...
      this._out({
        type: 'table',
        title: `Active Sessions (${list.length})`,
        data: list.slice(0, 50).reduce((acc, s) => {
          acc[s.user] = `${s.address} (${s.uptime})`;
          return acc;
        }, {}),
      });
    } catch (err) {
  ...
  async cmdKick([user]) {
    if (!user) {
      this._out({ type: 'error', message: 'Usage: kick <user>' });
      return;
    }
    try {
      ...
      this._out({
        type: 'success',
        message: res.kicked ? `🚫 Kick successful: ${user}` : `⚠️ ${user} not active.`,
      });
    } catch (err) {
      this._out({ type: 'error', message: err.message });
    }
  ...
  async cmdAddUser([user, pass, profile = 'default']) {
    if (!user || !pass) {
      this._out({ type: 'error', message: 'Usage: adduser <user> <pass> [profile]' });
      return;
    }
    try {
  ...
  async cmdDelUser([user]) {
    if (!user) {
      this._out({ type: 'error', message: 'Usage: deluser <user>' });
      return;
    }
    try {
...
class AgentOSGateway {
  ...
  _setupHandlers() {
    this.wss.on('connection', ws => {
      ...
      ws.on('error', err => {
        logger.error(`WS error (${id}): ${err.message}`);
        this._onDisconnect(id);
      });
    });
  }
  ...
  _onMessage(id, raw) {
    ...
    try {
      ...
      return this._sendToClient(id, { type: 'error', error: 'Invalid JSON' });
    }
    ...
    switch (msg.type) {
      ...
      case 'status':
        ...
        break;
      case 'cli.exec':
        this._handleCliExec(id, msg);
        break;
      case 'cli.start':
        this._handleCliStart(id);
        break;
      case 'cli.input':
        this._handleCliInput(id, msg);
        break;
      case 'cli.stop':
        this._handleCliStop(id);
        break;
      case 'cli.resize':
        this._handleCliResize(id, msg);
        break;
      default:
  ...
  _handleCliInput(clientId, msg) {
    ...
    if (!session) {
      ...
      if (c)
        this._send(c.ws, {
          type: 'cli.error',
          error: 'No active CLI session — send cli.start first.',
        });
      return;
    }
  ...
  async _invokeTool(ws, msg) {
    try {
      const result = await mikrotik.executeTool(
        msg.tool.replace(/^mikrotik\./, ''),
        ...(msg.params || [])
      );
...
class ChatRateLimiter {
  ...
  allow(chatId) {
    ...
    if (b.count >= CONFIG.SECURITY.VOUCHER_RATE_LIMIT) {
  ...
  _purge() {
    ...
    for (const [id, b] of this._buckets) if (b.windowStart < cutoff) this._buckets.delete(id);
  }
...
class AgentOSBot {
  constructor() {
    ...
    this.bot.on('polling_error', err => {
      const isConflict =
        err.code === 'ETELEGRAM' && err.response?.body?.description?.includes('Conflict');
      logger.error(
        isConflict
  ...
  _checkAuth(msg) {
    ...
    if (bucket.count > 30) {
      this.bot
        .sendMessage(chatId, '⏳ *Rate limit:* Too many commands. Please wait a moment.', {
          parse_mode: 'Markdown',
        })
        .catch(() => {});
      return false;
    ...
    if (!CONFIG.TELEGRAM.ALLOWED_CHATS.length) {
      this.bot.sendMessage(
        chatId,
        '⚠️ *Setup Mode Active*\nNo administrators are configured. Use `/claim` to become the primary admin.',
        { parse_mode: 'Markdown' }
      );
      return false;
    }
  ...
  async _cmdSetupRouter(msg) {
    ...
    this.promptUser(
      msg.chat.id,
      '🌐 *Step 1: Router IP*\nPlease enter the IP address of your MikroTik (e.g., `192.168.88.1`):',
      'setup:ip'
    );
  }

  async _finishSetup(chatId, ip, user, pass) {
    const status = await this.bot.sendMessage(
      chatId,
      '⚙️ `[ Attempting connection & provisioning… ]`',
      { parse_mode: 'Markdown' }
    );
    try {
  ...
  _reply(chatId, text, opts = {}) {
    ...
    if (opts.editMessageId) {
      return this.bot
        ?.editMessageText(text, { chat_id: chatId, message_id: opts.editMessageId, ...botOpts })
        .catch(e => {
    ...
    return this.bot
      ?.sendMessage(chatId, text, botOpts)
    ...
    try {
      ...
      switch (action) {
        case 'setup:ip':
          this.promptUser(
            chatId,
            `👤 *Step 2: Username*\nIP: \`${input}\` set. Enter MikroTik user:`,
            `setup:user:${input}`
          );
          break;
        default: {
          if (action.startsWith('setup:user:')) {
            ...
            this.promptUser(
              chatId,
              `🔑 *Step 3: Password*\nUser: \`${input}\` set. Enter MikroTik password:`,
              `setup:pass:${ip}:${input}`
            );
            break;
          }
    ...
    this._reply(msg.chat.id, `${BRAND.emoji} *${BRAND.name}*\nWelcome, ${msg.from.first_name}!`, {
      ...
      reply_markup: {
        inline_keyboard: [
          ...
          [
            { text: '🎫 Voucher', callback_data: 'action:voucher' },
            { text: '👛 Wallet', callback_data: 'wallet:list' },
          ],
  ...
  async _cmdTools(msg, opts = {}) {
    ...
    const btns = mikrotik
      .availableTools()
      .map(t => ({ text: `🔧 ${t}`, callback_data: `tool:${t}` }));
    const rows = [];
    for (let i = 0; i < btns.length; i += 2) rows.push(btns.slice(i, i + 2));
    rows.push([{ text: '⬅️ Back', callback_data: 'action:start' }]);
    this._reply(msg.chat.id, '*Available Tools*', {
      editMessageId: opts.editMessageId,
      ...
      reply_markup: {
        inline_keyboard: [
          [
            ...
            { text: '🛤 Trace', callback_data: 'net:traceroute' },
          ],
          [
            { text: '🔥 Firewall', callback_data: 'net:firewall' },
            { text: '🚫 Block', callback_data: 'net:block' },
          ],
          [
            { text: '📋 DHCP', callback_data: 'net:dhcp' },
            { text: '🔍 LAN Scan', callback_data: 'net:scan' },
          ],
          [
            { text: '📊 Interfaces', callback_data: 'net:bandwidth' },
            { text: '🧹 Flush DNS', callback_data: 'net:flush_dns' },
          ],
          [
            { text: '💾 Backup', callback_data: 'net:backup' },
  ...
  async _cmdUsers(msg, opts = {}) {
    ...
    this._reply(msg.chat.id, '👥 *Users* — Select action:', {
      ...
      reply_markup: {
        inline_keyboard: [
          [
            { text: '👁 Active', callback_data: 'users:active' },
            { text: '📋 All', callback_data: 'users:all' },
          ],
          [
            { text: '➕ Add', callback_data: 'users:add' },
            { text: '🚫 Kick', callback_data: 'users:kick' },
          ],
          [
    ...
    this._reply(msg.chat.id, '🎫 *Create Voucher* — Select duration:', {
      ...
      reply_markup: {
        inline_keyboard: [
          ...
          [
            { text: '📆 7 Days', callback_data: 'voucher:7d' },
            { text: '🌙 30 Days', callback_data: 'voucher:30d' },
          ],
        ],
      },
    });
  ...
  async _cmdLogs(msg) {
    ...
    try {
      ...
      this._reply(msg.chat.id, `❌ ${e.message}`);
    }
  }
  ...
  async _cmdAsk(msg, match) {
    ...
    const status = await this.bot.sendMessage(chatId, '⏳ `[ AgentOS thinking… ]`', {
      parse_mode: 'Markdown',
      reply_markup: {
        inline_keyboard: [[{ text: '🛑 Cancel', callback_data: 'action:cancel_ai' }]],
      },
    });
    ...
    try {
      ...
      clearInterval(anim);
      this.bot
        .editMessageText(`❌ *AI Error:* ${e.message}`, {
          chat_id: chatId,
          message_id: status.message_id,
          parse_mode: 'Markdown',
        })
  ...
  async _cmdKick(msg, match) {
    ...
    try {
      ...
      this._reply(
        msg.chat.id,
        res.kicked ? `🚫 Kicked *${match[1]}*` : `⚠️ *${match[1]}* not active`
      );
    } catch (e) {
      this._reply(msg.chat.id, `❌ ${e.message}`);
    }
  ...
  async _cmdCli(msg, match) {
    ...
    try {
      ...
      this._reply(msg.chat.id, `❌ CLI Error: ${e.message}`);
    }
  }
  ...
  async _cmdApi(msg, match) {
    ...
    try {
      ...
      this._reply(msg.chat.id, `❌ API Error: ${e.message}`);
    }
  }
  ...
  async _onCallback(query) {
    ...
    try {
      ...
      if (cat !== 'action' || act !== 'cancel_ai') {
        await this.bot
          .editMessageText(`⏳ *Processing ${cat}:${act}...*`, {
            chat_id: chatId,
            message_id: msgId,
            parse_mode: 'Markdown',
          })
      ...
      if (cat === 'action') {
        if (act === 'cancel_ai') {
          ...
          if (map[act])
            this[map[act]]({ chat: { id: chatId }, from: query.from }, { editMessageId: msgId });
        }
      } else if (cat === 'tool') {
        ...
        switch (act) {
          case 'ping':
            this.promptUser(chatId, '📡 Enter IP/host to ping:', 'ping');
            break;
          case 'traceroute':
            this.promptUser(chatId, '🛤 Enter IP/host to trace:', 'traceroute');
            break;
          case 'block':
            this.promptUser(chatId, '🚫 Enter IP/MAC to block:', 'block');
            break;
          case 'flush_dns': {
          ...
          case 'reboot':
            this._reply(chatId, '⚠️ Confirm router reboot?', {
              reply_markup: {
                inline_keyboard: [[{ text: '✅ Yes, reboot', callback_data: 'confirm:reboot' }]],
              },
            });
            break;
          default: {
        ...
        if (plan) {
          if (!this.rateLimiter.allow(chatId)) {
            this._reply(chatId, '⏳ Too many requests — slow down.');
            return;
          }
        ...
        if (act === 'list') {
          ...
          if (!codes.length) {
            this._reply(chatId, 'Your wallet is empty or all vouchers have been claimed.', {
              editMessageId: msgId,
              reply_markup: {
                inline_keyboard: [[{ text: '⬅️ Back', callback_data: 'action:start' }]],
              },
            });
          } else {
            const btns = codes.map(c => [
              { text: `🎟 Activate ${c}`, callback_data: `wallet:claim:${c}` },
            ]);
          ...
          this._reply(
            ...
            {
              reply_markup: {
                inline_keyboard: [[{ text: '📊 Dashboard', callback_data: 'action:dashboard' }]],
              },
            }
          );
        }
...
class AgentOSOrchestrator {
  ...
  async _provisionRouter() {
    ...
    await this.mikrotik
      .executeCLI(
        '/ip/firewall/address-list add list=AgentOS-Protected address=127.0.0.1 comment="Reserved"'
      )
      .catch(() => {});

    // 2. Ensure logging is set up for hotspot
    await this.mikrotik
      .executeCLI('/system/logging add topics=hotspot,info,debug action=memory')
      .catch(() => {});
  ...
  _monitorSystem() {
    setInterval(async () => {
      ...
      try {
        ...
        if (cpu > 90) this.bot?.alertOnce('cpu-high', `⚠️ *High CPU:* ${cpu}%`);
        if (1 - fm / tm > 0.85)
          this.bot?.alertOnce(
            'mem-high',
            `⚠️ *High Memory:* ${Math.round((1 - fm / tm) * 100)}% used`
          );
      } catch (err) {
  ...
  _monitorNewDevices() {
    ...
    setInterval(async () => {
      ...
      try {
        ...
        /* silence transient read failures */
      }
    }, 60_000);
  }
...
app.post('/api/nodes', authMiddleware, async (req, res) => {
  ...
  if (!name || !ip || !user || !pass)
    return res.status(400).json({ error: 'name, ip, user, pass required' });
  try {
...
${A.RESET}`
      ...
      .on('line', async line => {
        ...
        if (this._commands[key]) {
          try {
            ...
            console.error(`  ${A.ERROR}Error: ${err.message}${A.RESET}`);
          }
        } else {
  ...
  async cmdRawApi(args) {
    const cmd = args.join(' ');
    if (!cmd) {
      console.log('Usage: api </path/command>');
      return;
    }
    const res = await mikrotik.executeRawAPI(cmd);
    console.log(`\n⚙️ Result:\n${JSON.stringify(res, null, 2)}\n`);
  ...
  async cmdNodes() {
    ...
    await sleep(300);
    console.log(
      `  ${A.PRIMARY}◆${A.RESET} ${TerminalAnimator.gradient('AgentOS-Main-Gateway', [0, 255, 127], [50, 150, 255])}`
    );
    console.log(
      `  ${A.DIM}│  Status: ${mikrotik.isConnected ? `${A.SUCCESS}ONLINE` : `${A.ERROR}OFFLINE`}${A.RESET}`
    );
  ...
  async cmdUsers() {
    ...
    users
      ...
      .forEach(u =>
        console.log(
          `  ${u.disabled === 'yes' ? '🔴' : '🟢'} ${u.name.padEnd(15)} ${u.profile || 'default'}`
        )
      );
    console.log('');
  }
  ...
  async cmdActive() {
    ...
    console.log(`\n👥 Active (${users.length}):\n`);
    users.forEach(u =>
      console.log(`  🟢 ${u.user.padEnd(15)} ${u.address.padEnd(15)} ${u.uptime}`)
    );
    console.log('');
  }
  ...
  async cmdAddUser([username, password, profile = 'default']) {
    if (!username || !password) {
      console.log('Usage: adduser <name> <pass> [profile]');
      return;
    }
  ...
  async cmdDelUser([username]) {
    if (!username) {
      console.log('Usage: deluser <name>');
      return;
    }
  ...
  async cmdVoucher([plan, duration]) {
    if (!plan) {
      console.log('Usage: voucher <plan> [duration]');
      return;
    }
    const code = voucherCode();
  ...
  async cmdRedeem([code, username]) {
    if (!code || !username) {
      console.log('Usage: redeem <code> <username>');
      return;
    }
    const v = await database.getVoucher(code);
    if (!v) {
  ...
  async cmdPing([host, count = '4']) {
    if (!host) {
      console.log('Usage: ping <host> [count]');
      return;
    }
  ...
  async cmdLogs([lines = '20']) {
    ...
    console.log(`\n📋 Logs (${logs.length}):\n`);
    logs.forEach(l =>
      console.log(`  ${l.time || ''} [${(l.topics || '').padEnd(15)}] ${l.message || ''}`)
    );
    console.log('');
  }
  ...
  async cmdDhcp() {
    ...
    leases
      ...
      .forEach(l =>
        console.log(
          `  ${l.address.padEnd(15)} ${(l.hostname || '').padEnd(20)} ${l.status || 'bound'}`
        )
      );
    console.log('');
  }
  ...
  async cmdArp() {
    ...
    arp
      ...
      .forEach(e =>
        console.log(
          `  ${A.PRIMARY}◆${A.RESET} ${e.address.padEnd(15)} ${A.DIM}${e['mac-address'] || 'N/A'}${A.RESET}`
        )
      );
    console.log('');
  }
  ...
  async cmdFirewall() {
    ...
    rules
      .slice(0, 10)
      .forEach(r => console.log(`  ${r.chain}: ${r.action}${r.comment ? ` # ${r.comment}` : ''}`));
    console.log('');
  }

  async cmdBlock([target]) {
    if (!target) {
      console.log('Usage: block <ip-or-mac>');
      return;
    }
    await mikrotik.addToBlockList(target);
  ...
  async cmdUnblock([target]) {
    if (!target) {
      console.log('Usage: unblock <ip-or-mac>');
      return;
    }
    const res = await mikrotik.unblockAddress(target);
  ...
  async cmdQR([code]) {
    ...
    try {
      console.log(
        await QRCode.toString(JSON.stringify({ code, plan: v.plan, url }), {
          type: 'terminal',
          small: true,
        })
      );
...
async function bootDaemon() {
  ...
  try {
    ...
    logger.warn(`Starting in limited mode — MikroTik unreachable: ${err.message}`);
  }
  ...
  if (CONFIG.GATEWAY.PORT !== CONFIG.SERVER.PORT) {
    ...
    gwServer.listen(CONFIG.GATEWAY.PORT, CONFIG.GATEWAY.HOST, () => {
      logger.info(
        `WS Gateway  → ws://${CONFIG.GATEWAY.HOST}:${CONFIG.GATEWAY.PORT}${CONFIG.GATEWAY.WS_PATH}`
      );
  ...
  expressServer.listen(CONFIG.SERVER.PORT, CONFIG.SERVER.HOST, () => {
    logger.info(
      `${BRAND.name} v${BRAND.version} → http://${CONFIG.SERVER.HOST}:${CONFIG.SERVER.PORT}`
    );
