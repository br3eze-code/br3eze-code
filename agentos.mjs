...
const buildMissingEntryErrorMessage = async () => {
    const lines = ["AgentOS: missing dist/entry.(m)js (build output)."];
    ...
    lines.push(
        "For pinned GitHub installs, use `npm install -g github:breezetech/agentos#<ref>` instead of a raw `/archive/<ref>.tar.gz` URL.",
    );
    lines.push("For releases, use `npm install -g agentos@latest`.");
    return lines.join("\n");
};
