...
const freeze = {
  name: 'freeze',
  description:
    'EMERGENCY: Disable AgentOS self-modification. Sets self_edit enabled: false in soul.md',
  parameters: {
    type: 'object',
    properties: {
  ...
  run: async ({ action }) => {
    ...
    if (action === 'freeze') {
      ...
      return {
        success: true,
        message: `🛑 *FROZEN*: Self-modification disabled.\n\nself_edit: false\nskill_create: false\n\nUse \`freeze unfreeze\` to re-enable.`,
      };
    }
    ...
    if (action === 'unfreeze') {
      ...
      return {
        success: true,
        message: `✅ *UNFROZEN*: Self-modification re-enabled.\n\nself_edit: true\nskill_create: true`,
      };
    }
  },
};
