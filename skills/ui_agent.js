...
const ui_agent = {
  name: 'ui_agent',
  description:
    'Controls a headless browser to search, click buttons, fill forms on any website. Returns screenshots + extracted data.',
  parameters: {
    type: 'object',
    properties: {
      ...
      actions: {
        ...
        items: {
          ...
          properties: {
            type: {
              type: 'string',
              enum: ['goto', 'click', 'type', 'wait', 'select', 'screenshot', 'extract'],
            },
            selector: {
              type: 'string',
              description: "CSS selector, text, or xpath. For 'type': the text to input.",
            },
            value: { type: 'string', description: "Value to type/select. For 'wait': ms to wait." },
            name: {
              type: 'string',
              description: "For 'screenshot' or 'extract': filename or key name",
            },
          },
          required: ['type'],
        },
      ...
      target: {
        type: 'string',
        default: 'local',
        description: "Where to run: 'local'=AgentOS server. Future: device IP",
      },
    },
  ...
  run: async ({ url, actions, target = 'local' }, { logger }) => {
    if (target !== 'local') {
      throw new Error(
        'Remote device control not supported yet. Only target="local" works. AgentOS will run browser on its own server.'
      );
    }
    ...
    try {
      ...
      await fs.appendFile(
        './knowledge/failed-commands.md',
        `\n## ui_agent failed ${new Date().toISOString()}\nURL: ${url}\nError: ${err.message}\nSteps completed: ${results.steps.length}\n`
      );
      throw new Error(
        `UI Agent failed at step ${results.steps.length + 1}: ${err.message}\n\nCompleted:\n${results.steps.join('\n')}`
      );
    }
  },
};
