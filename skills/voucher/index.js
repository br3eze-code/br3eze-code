...
class VoucherSkill {
  async execute(params, context) {
    ...
    switch (action) {
      ...
      case 'voucher.redeem':
        return this.redeem(params, context);
      case 'voucher.list':
        return this.list(params, context);
      case 'voucher.stats':
        return this.stats(context);
      case 'voucher.revoke':
        return this.revoke(params, context);
      case 'voucher.renew':
        return this.renew(params, context);
      default:
  ...
  _plans() {
    return {
      '1hour': { duration: 60 * 60 * 1000, price: 1.0, label: '1 Hour' },
      '1Day': { duration: 24 * 60 * 60 * 1000, price: 5.0, label: '1 Day' },
      '7Day': { duration: 7 * 24 * 60 * 60 * 1000, price: 25.0, label: '7 Days' },
      '30Day': { duration: 30 * 24 * 60 * 60 * 1000, price: 80.0, label: '30 Days' },
    };
  }
  ...
  _expiresAt(plan) {
    ...
    if (!planDef)
      throw new Error(`Unknown plan: ${plan}. Use: ${Object.keys(this._plans()).join(', ')}`);
  ...
  async revoke({ code }, context) {
    ...
    if (context.mikrotik) {
      try {
        await context.mikrotik.removeHotspotUser(code);
      } catch {}
    }
