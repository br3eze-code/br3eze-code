...
const onboard = {
  name: 'onboard',
  description:
    'Onboard fleet, generate Mission Control, and auto-create agents with skills/roles/duties/tools',
  parameters: {
    type: 'object',
    properties: {
      target: { type: 'string' },
      admin_password: { type: 'string' },
      hotspot_name: { type: 'string', default: 'AgentOS-Hotspot' },
      dns_servers: { type: 'string', default: '1.1.1.1,8.8.8.8' },
  ...
  run: async (
    ...
    if (target === 'all') targets = inventory;
      ...
      targets = inventory.filter(r => r.role === target.split(':')[1]);
    ...
    for (const router of targets) {
      const cfg = {
        host: router.host,
        user: router.user,
        password: admin_password || router.password,
        wan_interface: router.wan_interface || 'ether1',
        lan_ip: router.lan_ip || '10.5.50.1/24',
      };
        ...
        await create_agent.run(
          {
            ...
            duties:
              'monitor health,provision users,backup configs,manage hotspot,respond to alerts',
    ...
    const missionHtml = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Mission Control</title><script src="https://cdn.jsdelivr.net/npm/chart.js"></script><style>body{margin:0;background:#0f172a;color:#fff;font-family:system-ui}header{background:#1e293b;padding:16px}main{padding:20px;max-width:1200px;margin:auto}.card{background:#1e293b;padding:16px;border-radius:12px;margin:12px 0}</style></head><body><header><h1>🚀 AgentOS Mission Control</h1></header><main><div class="card"><h3>Fleet: ${ok} agents active</h3><p>Agents created: ${results
      .filter(r => r.agent)
      .map(r => r.agent)
      .join(
        ', '
      )}</p><p>Fleet-master ready</p></div><canvas id="c" height="80"></canvas></main><script>new Chart(document.getElementById('c'),{type:'line',data:{labels:[],datasets:[{label:'Fleet CPU',data:[]}]}})</script></body></html>`;
