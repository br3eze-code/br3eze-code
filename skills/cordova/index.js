...
class CordovaSkill extends BaseSkill {
  ...
  async _cordovaCmd(project, cmd, timeout = 600) {
    ...
    const { stdout, stderr } = await execAsync(`cordova ${cmd}`, {
      cwd,
    ...
    try {
      switch (toolName) {
        ...
        case 'cordova.platform.add':
          this.logger.warn(`CORDOVA PLATFORM ADD ${args.platform}`, {
            user: ctx.userId,
            reason: args.reason,
          });
        ...
        case 'cordova.plugin.add':
          this.logger.warn(`CORDOVA PLUGIN ADD ${args.plugin}`, {
            user: ctx.userId,
            reason: args.reason,
          });
          const vars = args.variables
            ? Object.entries(args.variables)
                ...
                .join(' ')
            : '';
          const { stdout: pout } = await this._cordovaCmd(
            args.project,
            `plugin add ${args.plugin} ${vars}`,
            300
          );
          return { project: args.project, plugin: args.plugin, log: pout };

        case 'cordova.build':
          this.logger.warn(`CORDOVA BUILD ${args.platform} release:${args.release}`, {
            user: ctx.userId,
            reason: args.reason,
          });
          const buildArgs = [
        ...
        case 'cordova.run':
          this.logger.warn(`CORDOVA RUN ${args.platform} ${args.target}`, {
            user: ctx.userId,
            reason: args.reason,
          });
          const runArgs = [
        ...
        case 'cordova.sign.android':
          ...
          const { stdout: sout } = await this._cordovaCmd(
            args.project,
        ...
        case 'cordova.ios.upload':
          ...
          const { stdout: uout } = await execAsync(
            `xcrun altool --upload-app -f "${ipa}" -t ios -u ${process.env.APPLE_ID} -p ${process.env.APPLE_APP_PWD} --output-format xml`,
            { timeout: 600000 }
          );
