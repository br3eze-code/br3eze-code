...
const hotspot_brand = {
  name: 'hotspot_brand',
  description:
    'Upload complete hotspot portal: login.html, status.html, logout.html, rlogin.html, style.css, script.js, images. Shards to fleet.',
  parameters: {
    type: 'object',
    properties: {
  ...
  run: async ({ target, bundle }, { logger }) => {
    ...
    results.forEach(r => {
      msg += r.success ? `✅ ${r.id}: ${r.files.join(', ')}\n` : `❌ ${r.id}: ${r.error}\n`;
    });
