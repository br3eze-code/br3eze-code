...
const skill_create = {
  ...
  run: async ({ name, purpose, trigger, code }, { gemini }) => {
    ...
    const validation = await gemini.generate({
      prompt: `Validate AgentOS skill. Must export { ${name} } with name,description,parameters,run.\nCode:\n${code}\nReply: VALID or INVALID`
    });
    ...
    return { success: true, skill: name, file: filePath, warning: 'Restart AgentOS to load' };
  }
};
