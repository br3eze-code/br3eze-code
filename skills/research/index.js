  ...
  static getTools() {
    return {
      ...
      'research.citavi.search': {
        ...
        parameters: {
          ...
          properties: {
            ...
            type: {
              type: 'string',
              enum: ['reference', 'knowledge', 'quotation', 'any'],
              default: 'any',
            },
          },
          required: ['query'],
      ...
      'research.endnote.search': {
        ...
        parameters: {
          ...
          properties: {
            ...
            field: {
              type: 'string',
              enum: ['any', 'author', 'title', 'year', 'keywords'],
              default: 'any',
            },
          },
          required: ['query'],
      ...
      'research.logseq.search': {
        ...
        parameters: {
          ...
          properties: {
            ...
            mode: {
              type: 'string',
              enum: ['block', 'page', 'property', 'datalog'],
              default: 'block',
            },
          },
      ...
      'research.obsidian.sync': {
        risk: 'medium',
        description:
          'Sync Obsidian vault: index.md files, build backlink graph. Requires approval.',
        parameters: {
          type: 'object',
          properties: {
      ...
      'research.obsidian.write': {
        risk: 'medium',
        description:
          'Write note to Obsidian vault with wikilinks + frontmatter. Requires approval.',
        parameters: {
          type: 'object',
          properties: {
            vault_path: { type: 'string' },
      ...
      'research.zotero.search': {
        ...
        parameters: {
          ...
          properties: {
            ...
            item_type: {
              type: 'string',
              enum: ['journalArticle', 'book', 'conferencePaper', 'any'],
              default: 'any',
            },
          },
    ...
    try {
      switch (toolName) {
        case 'research.papers.sync': {
          this.logger.warn(`RESEARCH PAPERS SYNC ${args.library}`, {
            user: ctx.userId,
            reason: args.reason,
          });
          ...
          const libRes = await fetch(
            `https://api.readcube.com/libraries/${args.library}/articles?limit=1000&expand=annotations,collections`,
            { headers }
          );
          ...
          await fs.writeFile(
            cachePath,
            JSON.stringify({ articles, synced_at: new Date().toISOString() }, null, 2)
          );
          ...
          return {
            ...
            annotations: articles.reduce((n, a) => n + (a.annotations?.length || 0), 0),
            cache: cachePath,
          };
        }
        case 'research.papers.search': {
          this.logger.info(`RESEARCH PAPERS SEARCH ${args.field}: ${args.query}`, {
            user: ctx.userId,
          });
          const cache = JSON.parse(
            await fs.readFile(path.join(this.outputDir, 'papers.json'), 'utf8')
          );
          const q = args.query.toLowerCase();

          const results = cache.articles
            .filter(a => {
              if (args.field === 'title') return a.title?.toLowerCase().includes(q);
              if (args.field === 'author')
                return a.authors?.some(au => au.name.toLowerCase().includes(q));
        ...
        case 'research.papers.annotate': {
          this.logger.warn(`RESEARCH PAPERS ANNOTATE ${args.paper_id}`, {
            user: ctx.userId,
            reason: args.reason,
          });
          ...
          const res = await fetch(
            `https://api.readcube.com/articles/${args.paper_id}/annotations`,
            {
              method: 'POST',
              headers: headers2,
        ...
        case 'research.citavi.import': {
          this.logger.warn(`RESEARCH CITAVI IMPORT ${args.path}`, {
            user: ctx.userId,
            reason: args.reason,
          });
          ...
          if (ext === '.ctv6') {
            ...
            const refsRows =
              await get(`SELECT r.*, GROUP_CONCAT(p.LastName||', '||p.FirstName, '; ') as authors
          ...
          await fs.writeFile(
            cachePath2,
            JSON.stringify(
              { refs, knowledge, quotes, imported_at: new Date().toISOString() },
              null,
              2
            )
          );
        ...
        case 'research.citavi.search': {
          this.logger.info(`RESEARCH CITAVI SEARCH ${args.type}: ${args.query}`, {
            user: ctx.userId,
          });
          const cache2 = JSON.parse(
            await fs.readFile(path.join(this.outputDir, 'citavi.json'), 'utf8')
          );
              ...
              ...cache2.refs
                .filter(
                  r =>
                    ...
                    r.authors?.toLowerCase().includes(q2) ||
                    r.abstract?.toLowerCase().includes(q2)
                )
          ...
          if (args.type === 'knowledge' || args.type === 'any') {
            results2.push(
              ...cache2.knowledge
                .filter(
                  k =>
                    k.core_statement?.toLowerCase().includes(q2) ||
                    k.text?.toLowerCase().includes(q2)
                )
          ...
          if (args.type === 'quotation' || args.type === 'any') {
            results2.push(
              ...cache2.quotes
                .filter(q => q.text?.toLowerCase().includes(q2))
                .map(q => ({ ...q, _type: 'quotation' }))
            );
          }
        ...
        case 'research.citavi.export': {
          ...
          if (args.format === 'bibtex') {
            ...
            // Word = simple RTF
            output = refs2
              .map(r => `${r.authors} (${r.year}). ${r.title}. ${r.periodical}.`)
              .join('\n\n');
          }

          const outPath = path.join(
            this.outputDir,
            `citavi_export_${Date.now()}.${args.format === 'bibtex' ? 'bib' : args.format === 'ris' ? 'ris' : 'rtf'}`
          );
        ...
        case 'research.mendeley.sync': {
          ...
          for (const doc of docs) {
            if (doc.file_attached) {
              ...
              if (files[0]) {
                ...
                const annRes = await fetch(`${base}/annotations?document_id=${doc.id}`, {
                  headers,
                });
          ...
          await fs.writeFile(
            cachePath,
            JSON.stringify({ docs, synced_at: new Date().toISOString() }, null, 2)
          );
          return {
            documents: docs.length,
            annotations: docs.reduce((n, d) => n + (d.annotations?.length || 0), 0),
            cache: cachePath,
          };
        }
        case 'research.mendeley.search': {
          this.logger.info(`RESEARCH MENDELEY SEARCH ${args.type}: ${args.query}`, {
            user: ctx.userId,
          });
          const cache = JSON.parse(
            await fs.readFile(path.join(this.outputDir, 'mendeley.json'), 'utf8')
          );
          const q = args.query.toLowerCase();

          const results = cache.docs
            .filter(d => {
              if (args.type === 'author')
                return d.authors?.some(a =>
        ...
        case 'research.mendeley.annotate': {
          this.logger.warn(`RESEARCH MENDELEY ANNOTATE doc ${args.document_id}`, {
            user: ctx.userId,
            reason: args.reason,
          });
          const headers2 = {
        ...
        case 'research.endnote.import': {
          this.logger.warn(`RESEARCH ENDNOTE IMPORT ${args.path}`, {
            user: ctx.userId,
            reason: args.reason,
          });
          const filePath = path.resolve(this.workspace, args.path);
          ...
          if (ext === '.ris' || ext === '.bib') {
            ...
            const get = promisify(db.all.bind(db));
            const rows =
              await get(`SELECT refs.*, GROUP_CONCAT(authors.last_name||', '||authors.first_name, '; ') as authors
          ...
          await fs.writeFile(
            cachePath2,
            JSON.stringify({ refs, imported_at: new Date().toISOString() }, null, 2)
          );
          return { path: args.path, references: refs.length, cache: cachePath2 };
        }
        case 'research.endnote.search': {
          this.logger.info(`RESEARCH ENDNOTE SEARCH ${args.field}: ${args.query}`, {
            user: ctx.userId,
          });
          const cache2 = JSON.parse(
            await fs.readFile(path.join(this.outputDir, 'endnote.json'), 'utf8')
          );
          const q2 = args.query.toLowerCase();

          const results2 = cache2.refs
            .filter(r => {
        ...
        case 'research.endnote.export': {
          ...
          if (args.format === 'bibtex') {
            ...
            output = `<?xml version="1.0"?><records>${refs2
              .map(
                r =>
          ...
          const outPath = path.join(
            this.outputDir,
            `export_${Date.now()}.${args.format === 'bibtex' ? 'bib' : args.format === 'ris' ? 'ris' : 'xml'}`
          );
          await fs.writeFile(outPath, output);
          return { format: args.format, count: refs2.length, path: outPath };
        }
        case 'research.roam.sync': {
          this.logger.warn(`RESEARCH ROAM SYNC ${args.graph}`, {
            user: ctx.userId,
            reason: args.reason,
          });
          const headers = {
        ...
        case 'research.roam.search': {
          this.logger.info(`RESEARCH ROAM SEARCH ${args.mode}: ${args.query}`, {
            user: ctx.userId,
          });
          const headers2 = {
            Authorization: `Bearer ${args.token}`,
          ...
          const res2 = await fetch(`${base2}/q`, {
            method: 'POST',
            headers: headers2,
        ...
        case 'research.roam.write': {
          this.logger.warn(`RESEARCH ROAM WRITE ${args.page}`, {
            user: ctx.userId,
            reason: args.reason,
          });
          const headers3 = {
          ...
          await fetch(`${base3}/write`, {
            method: 'POST',
            headers: headers3,
            body: JSON.stringify({ action: 'batch-actions', actions: create }),
          }).catch(() => {});

          // Write block
          const block = {
            action: 'create-block',
            location: args.parent_uid
              ? { 'parent-uid': args.parent_uid, order: 'last' }
              : { 'page-title': args.page, order: 'last' },
            block: { string: args.content },
          };
          const res3 = await fetch(`${base3}/write`, {
            method: 'POST',
            headers: headers3,
            body: JSON.stringify({ action: 'batch-actions', actions: [block] }),
          });
          const out = await res3.json();
          return { page: args.page, uid: out[0]?.uid, written: true };
        }
        case 'research.logseq.sync': {
          this.logger.warn(`RESEARCH LOGSEQ SYNC ${args.graph_path}`, {
            user: ctx.userId,
            reason: args.reason,
          });
            ...
            for (const e of entries) {
              ...
              if (e.isDirectory() && !e.name.startsWith('.')) {
                ...
                pages.push({
                  name: pageName,
                  path: rel,
                  blocks: blocks.filter(b => b.page === pageName).length,
                });
              }
            }
          };
          ...
          await fs.writeFile(
            idxPath,
            JSON.stringify(
              { pages, blocks, graphLinks, props, synced_at: new Date().toISOString() },
              null,
              2
            )
          );
        ...
        case 'research.logseq.search': {
          this.logger.info(`RESEARCH LOGSEQ SEARCH ${args.mode}: ${args.query}`, {
            user: ctx.userId,
          });
          const idx2 = JSON.parse(
            await fs.readFile(path.join(this.outputDir, 'logseq_index.json'), 'utf8')
          );
          ...
          if (args.mode === 'page') {
            ...
            results2 = idx2.blocks.filter(b =>
              idx2.props[b.uid]?.[key]?.toLowerCase().includes(val.toLowerCase())
            );
        ...
        case 'research.logseq.write': {
          this.logger.warn(`RESEARCH LOGSEQ WRITE ${args.page}`, {
            user: ctx.userId,
            reason: args.reason,
          });
          ...
          if (args.properties) {
            const propLines = Object.entries(args.properties)
              .map(([k, v]) => ` ${k}:: ${v}`)
              .join('\n');
            block += `\n${propLines}`;
          }
        ...
        case 'research.obsidian.sync': {
          this.logger.warn(`RESEARCH OBSIDIAN SYNC ${args.vault_path}`, {
            user: ctx.userId,
            reason: args.reason,
          });
          ...
          const walk = async dir => {
            ...
            for (const e of entries) {
              ...
              if (e.isDirectory() && !e.name.startsWith('.')) {
                ...
                if (fmMatch) {
                  try {
                    frontmatter = require('js-yaml').load(fmMatch[1]);
                  } catch {}
                }
          ...
          await fs.writeFile(
            indexPath,
            JSON.stringify(
              { notes, linkGraph, tagIndex, synced_at: new Date().toISOString() },
              null,
              2
            )
          );
        ...
        case 'research.obsidian.search': {
          this.logger.info(`RESEARCH OBSIDIAN SEARCH ${args.mode}: ${args.query}`, {
            user: ctx.userId,
          });
          const idx = JSON.parse(
            await fs.readFile(path.join(this.outputDir, 'obsidian_index.json'), 'utf8')
          );
            ...
            for (const n of idx.notes) {
              ...
              if (full.toLowerCase().includes(q)) {
                const excerpt = full.slice(
                  full.toLowerCase().indexOf(q) - 50,
                  full.toLowerCase().indexOf(q) + 150
                );
        ...
        case 'research.obsidian.write': {
          this.logger.warn(`RESEARCH OBSIDIAN WRITE ${args.path}`, {
            user: ctx.userId,
            reason: args.reason,
          });
        ...
        case 'research.readwise.search': {
          ...
          const cache = JSON.parse(
            await fs.readFile(path.join(this.outputDir, 'readwise.json'), 'utf8')
          );
          const q = args.query.toLowerCase();

          let books = cache.books;
          if (args.book)
            books = books.filter(b => b.title.toLowerCase().includes(args.book.toLowerCase()));
        ...
        case 'research.zotero.sync': {
          this.logger.warn(`RESEARCH ZOTERO SYNC ${args.collection || 'all'}`, {
            user: ctx.userId,
            reason: args.reason,
          });
          ...
          return {
            ...
            items: items.length,
            version: res.headers.get('Last-Modified-Version'),
          };
        }
        case 'research.zotero.search': {
          ...
          const res2 = await fetch(
            `${base2}/items?q=${q}${type}&format=json&include=bib&limit=25`,
            { headers: headers2 }
          );
        ...
        case 'research.zotero.add': {
          this.logger.warn(`RESEARCH ZOTERO ADD ${args.url}`, {
            user: ctx.userId,
            reason: args.reason,
          });
          const base3 = `https://api.zotero.org/users/${args.user_id}`;
          const headers3 = {
            'Zotero-API-Key': args.api_key,
        ...
        case 'research.notion.export': {
          this.logger.warn(`RESEARCH NOTION EXPORT ${args.title}`, {
            user: ctx.userId,
            reason: args.reason,
          });
            ...
            properties: {
              ...
              ...Object.entries(args.properties || {}).reduce((acc, [k, v]) => {
                acc[k] = Array.isArray(v)
                  ? { multi_select: v.map(n => ({ name: n })) }
                  : { rich_text: [{ text: { content: String(v) } }] };
                return acc;
              }, {}),
            },
