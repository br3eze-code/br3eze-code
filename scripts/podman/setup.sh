#!/usr/bin/env bash
# One-time host setup for rootless AgentOS in Podman. Uses the current
# non-root user throughout, builds or pulls the image into that user's Podman
# store, writes config under ~/.agentos by default, and uses the repo-local
# launch script at ./scripts/run-agent-podman.sh.
#
...
#   Or set AGENT_PODMAN_QUADLET=1 (or 0) to choose without a flag.
#
# After this, start the gateway manually:
#   ./scripts/run-agent-podman.sh launch
#   ./scripts/run-agent-podman.sh launch setup
# Or, if you used --quadlet:
#   systemctl --user start agentos.service
set -euo pipefail

REPO_PATH="${AGENT_REPO_PATH:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)}"
RUN_SCRIPT_SRC="$REPO_PATH/scripts/run-agent-podman.sh"
QUADLET_TEMPLATE="$REPO_PATH/scripts/podman/agentos.container.in"
AGENT_USER="$(id -un)"
AGENT_HOME="${HOME:-}"
AGENT_CONFIG_DIR="${AGENT_CONFIG_DIR:-}"
AGENT_WORKSPACE_DIR="${AGENT_WORKSPACE_DIR:-}"
AGENT_IMAGE="${AGENT_PODMAN_IMAGE:-${AGENT_IMAGE:-agentos:local}}"
AGENT_CONTAINER_NAME="${AGENT_PODMAN_CONTAINER:-agentos}"
PLATFORM_NAME="$(uname -s 2>/dev/null || echo unknown)"
HOST_GATEWAY_PORT="${AGENT_PODMAN_GATEWAY_HOST_PORT:-${AGENT_GATEWAY_PORT:-18789}}"
...
PY
  ...
  echo "Missing dependency: need openssl or python3 (or od) to generate AGENT_GATEWAY_TOKEN." >&2
  exit 1
}
...
if [[ -n "${AGENT_PODMAN_QUADLET:-}" ]]; then
  case "${AGENT_PODMAN_QUADLET,,}" in
...
if [[ "$AGENT_IMAGE" == "agentos:local" ]] && [[ ! -f "$REPO_PATH/Dockerfile" ]]; then
  echo "Dockerfile not found at $REPO_PATH. Set AGENT_REPO_PATH to the repo root." >&2
  exit 1
fi
...
if [[ -z "$AGENT_HOME" ]]; then
  echo "Unable to resolve HOME for user $AGENT_USER." >&2
  exit 1
fi
if [[ -z "$AGENT_CONFIG_DIR" ]]; then
  AGENT_CONFIG_DIR="$AGENT_HOME/.agentos"
fi
if [[ -z "$AGENT_WORKSPACE_DIR" ]]; then
  AGENT_WORKSPACE_DIR="$AGENT_CONFIG_DIR/workspace"
fi
validate_absolute_path "home directory" "$AGENT_HOME"
validate_mount_source_path "config directory" "$AGENT_CONFIG_DIR"
validate_mount_source_path "workspace directory" "$AGENT_WORKSPACE_DIR"
validate_container_name "$AGENT_CONTAINER_NAME"
validate_image_name "$AGENT_IMAGE"
validate_port "gateway host port" "$HOST_GATEWAY_PORT"
validate_port "seed gateway port" "$SEED_GATEWAY_PORT"

install -d -m 700 "$AGENT_CONFIG_DIR" "$AGENT_WORKSPACE_DIR"
ensure_private_existing_dir_owned_by_user "config directory" "$AGENT_CONFIG_DIR"
ensure_private_existing_dir_owned_by_user "workspace directory" "$AGENT_WORKSPACE_DIR"

BUILD_ARGS=()
if [[ -n "${AGENT_DOCKER_APT_PACKAGES:-}" ]]; then
  BUILD_ARGS+=(--build-arg "AGENT_DOCKER_APT_PACKAGES=${AGENT_DOCKER_APT_PACKAGES}")
fi
if [[ -n "${AGENT_EXTENSIONS:-}" ]]; then
  BUILD_ARGS+=(--build-arg "AGENT_EXTENSIONS=${AGENT_EXTENSIONS}")
fi

if [[ "$AGENT_IMAGE" == "agentos:local" ]]; then
  echo "Building image $AGENT_IMAGE ..."
  podman build -t "$AGENT_IMAGE" -f "$REPO_PATH/Dockerfile" "${BUILD_ARGS[@]+"${BUILD_ARGS[@]}"}" "$REPO_PATH"
else
  if podman image exists "$AGENT_IMAGE" >/dev/null 2>&1; then
    echo "Using existing image $AGENT_IMAGE"
  else
    echo "Pulling image $AGENT_IMAGE ..."
    podman pull "$AGENT_IMAGE"
  fi
fi

ENV_FILE="$AGENT_CONFIG_DIR/.env"
if [[ ! -f "$ENV_FILE" ]]; then
...
EOF
  ...
  echo "Generated AGENT_GATEWAY_TOKEN and wrote it to $ENV_FILE"
fi
upsert_env_var "$ENV_FILE" "AGENT_PODMAN_CONTAINER" "$AGENT_CONTAINER_NAME"
upsert_env_var "$ENV_FILE" "AGENT_PODMAN_IMAGE" "$AGENT_IMAGE"

CONFIG_JSON="$AGENT_CONFIG_DIR/agentos.json"
...
if [[ "$INSTALL_QUADLET" == true ]]; then
  QUADLET_DIR="$AGENT_HOME/.config/containers/systemd"
  QUADLET_DST="$QUADLET_DIR/agentos.container"
  echo "Installing Quadlet to $QUADLET_DST ..."
  mkdir -p "$QUADLET_DIR"
  ensure_safe_existing_dir "quadlet directory" "$QUADLET_DIR"
  AGENT_HOME_ESCAPED="$(escape_sed_replacement_pipe_delim "$AGENT_HOME")"
  AGENT_CONFIG_ESCAPED="$(escape_sed_replacement_pipe_delim "$AGENT_CONFIG_DIR")"
  AGENT_WORKSPACE_ESCAPED="$(escape_sed_replacement_pipe_delim "$AGENT_WORKSPACE_DIR")"
  AGENT_IMAGE_ESCAPED="$(escape_sed_replacement_pipe_delim "$AGENT_IMAGE")"
  AGENT_CONTAINER_ESCAPED="$(escape_sed_replacement_pipe_delim "$AGENT_CONTAINER_NAME")"
  sed \
    -e "s|{{AGENT_HOME}}|$AGENT_HOME_ESCAPED|g" \
    -e "s|{{AGENT_CONFIG_DIR}}|$AGENT_CONFIG_ESCAPED|g" \
    -e "s|{{AGENT_WORKSPACE_DIR}}|$AGENT_WORKSPACE_ESCAPED|g" \
    -e "s|{{IMAGE_NAME}}|$AGENT_IMAGE_ESCAPED|g" \
    -e "s|{{CONTAINER_NAME}}|$AGENT_CONTAINER_ESCAPED|g" \
    "$QUADLET_TEMPLATE" | write_file_atomically "$QUADLET_DST" 644

  if command -v systemctl >/dev/null 2>&1; then
    echo "Reloading and starting user service..."
    if systemctl --user daemon-reload && systemctl --user start agentos.service; then
      echo "Quadlet installed and service started."
    else
      echo "Quadlet installed, but automatic start failed." >&2
      echo "Try: systemctl --user daemon-reload && systemctl --user start agentos.service" >&2
