...
class QNAPProcessor {
  ...
  _textToEmbedding(text) {
    ...
    for (const [intent, words] of Object.entries(keywords)) {
      embedding[intent] =
        words.reduce((sum, word) => sum + (text.toLowerCase().includes(word) ? 1 : 0), 0) /
        words.length;
    }
  ...
  _getDeviceReputation(fingerprint) {
    ...
    const history = Array.from(this.memory.values()).filter(
      m => m.deviceFingerprint === fingerprint
    );
  ...
  _calculateVelocity(userId) {
    const now = Date.now();
    const recent = Array.from(this.memory.entries()).filter(
      ([_, v]) => v.timestamp > now - 3600000 && _.startsWith(userId)
    );
  ...
  _matchHistoricalPattern(tx) {
    ...
    const similar = Array.from(this.memory.values()).filter(
      m => Math.abs(m.telemetry.amount_norm - tx.amount / 10) < 0.1
    );
