...
const tools = {
  ...
  'user.add': async (conn, username, password, profile = 'default') => {
    ...
    if (existing.length > 0) {
      const id =
        typeof existing[0] === 'object' ? existing[0]['.id'] || existing[0].id : existing[0];
  ...
  'user.remove': async (conn, username) => {
    ...
    if (!id)
      throw new Error(
        `Could not resolve router ID for user: ${username}. Got: ${JSON.stringify(users[0])}`
      );
  ...
  'user.kick': async (conn, username) => {
    ...
    if (!id)
      throw new Error(
        `Could not resolve session ID for user: ${username}. Got: ${JSON.stringify(active[0])}`
      );
  ...
  'dhcp.leases': async conn => conn.menu('/ip/dhcp-server/lease').get(),

  'arp.table': async conn => conn.menu('/ip/arp').get(),

  interfaces: async conn => conn.menu('/interface').get(),

  // ── Firewall ───────────────────────────────────────────

  'firewall.list': async (conn, type = 'filter') => conn.menu(`/ip/firewall/${type}`).get(),
  ...
  'firewall.unblock': async (conn, address, list = 'blocked') => {
    ...
    if (!id)
      throw new Error(
        `Could not resolve entry ID for address: ${address}. Got: ${JSON.stringify(entries[0])}`
      );
