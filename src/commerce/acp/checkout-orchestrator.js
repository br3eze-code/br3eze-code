...
function conflictError() { const error = new Error('Idempotency key was already used with a different request.'); error.status = 409; return error; }
function pendingError() { const error = new Error('The checkout request is already being processed. Retry with the same idempotency key.'); error.status = 409; return error; }

export async function executeCheckout({ idempotencyKey, merchantId, buyerId = null, platform, channelId, address = {}, payMethod = 'cod', scope = {}, idempotencyStore = null, idempotencyOptions = {}, checkoutFn = defaultCheckout } = {}) {
