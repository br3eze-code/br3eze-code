...
class DahuaSkill extends BaseSkill {
      ...
      'dahua.device.info': {
        ...
        parameters: {
          ...
          properties: {
            device: {
              type: 'string',
              description: 'deviceId from workspace (optional, defaults to first)',
            },
          },
          required: [],
        },
        ...
        parameters: {
          ...
          properties: {
            ...
            codes: {
              type: 'array',
              items: { type: 'string' },
              default: ['VideoMotion', 'CrossLineDetection'],
            },
  ...
  _client(deviceId) {
    ...
    this.clients.set(targetDevice, {
      client,
      base: `http://${dev.host}:${dev.port || 80}/cgi-bin`,
  ...
  async _get(deviceId, path) {
    ...
    if (!res.ok)
      throw new Error(`Dahua API ${res.status} on device ${resolvedId}: ${await res.text()}`);
    return res;
  }
    ...
    try {
      switch (toolName) {
        ...
        case 'dahua.device.info':
          const targetInfoDevice =
            args.device || Object.keys(this.workspace.dahua_devices || {})[0];
        ...
        case 'dahua.ptz.move':
          const targetMoveDevice =
            args.device || Object.keys(this.workspace.dahua_devices || {})[0];
          this.logger.warn(`DAHUA PTZ ${args.action} on ${targetMoveDevice}`, {
            user: ctx.userId,
            reason: args.reason,
          });
          ...
          setTimeout(
            () =>
              this._get(targetMoveDevice, `ptz.cgi?action=stop&channel=${ch1}&code=${args.action}`),
            1000
          );
          return { device: targetMoveDevice, channel: ch1, action: args.action, status: 'moving' };

        case 'dahua.ptz.preset':
          const targetPresetDevice =
            args.device || Object.keys(this.workspace.dahua_devices || {})[0];
          this.logger.warn(`DAHUA PRESET ${args.preset} on ${targetPresetDevice}`, {
            user: ctx.userId,
            reason: args.reason,
          });
          ...
          await this._get(
            targetPresetDevice,
            `ptz.cgi?action=start&channel=${ch2}&code=GotoPreset&arg1=0&arg2=${args.preset}&arg3=0`
          );
        ...
        case 'dahua.events.subscribe':
          ...
          const events = await this._get(
            args.device,
            `log.cgi?action=find&startTime=${start}&endTime=${end}&types=${codes}`
          );
        ...
        case 'dahua.system.reboot':
          const targetRebootDevice =
            args.device || Object.keys(this.workspace.dahua_devices || {})[0];
          this.logger.warn(`DAHUA REBOOT ${targetRebootDevice}`, {
            user: ctx.userId,
            reason: args.reason,
          });
