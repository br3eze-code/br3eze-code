      ...
      switch (toolName) {
        ...
        case 'mac.process.list':
          ...
          const ps = await this._exec(
            args.host,
            `ps aux ${sortKey} | head -n ${(args.top || 10) + 1}`
          );
        ...
        case 'mac.process.kill':
          this.logger.warn(`MAC KILL PID ${args.pid} on ${args.host}`, {
            user: ctx.userId,
            reason: args.reason,
          });
        ...
        case 'mac.service.restart':
          this.logger.warn(`MAC SERVICE RESTART ${args.host}`, {
            user: ctx.userId,
            label: args.label,
            reason: args.reason,
          });
        ...
        case 'mac.log.query':
          ...
          if (!/^[a-zA-Z0-9_.\s=<>!'"-]+$/.test(args.predicate))
            throw new Error('Invalid predicate');
          return await this._exec(
            args.host,
            `log show --predicate '${args.predicate}' --last ${args.last || '1h'} --style compact`
          );
