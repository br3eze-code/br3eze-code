...
class GossipSkill extends BaseSkill {
  ...
  static description =
    'Distributed gossip: peer discovery, state sync, failure detection, rumor spreading';
        ...
        parameters: {
          ...
          properties: {
            ...
            seeds: {
              type: 'array',
              items: { type: 'string' },
              description: 'ws://host:port seeds',
            },
            reason: { type: 'string' },
          },
          required: ['reason'],
  ...
  _handleMessage(peerId, msg) {
    ...
    switch (msg.type) {
      case 'digest':
        ...
        this._send(peerId, {
          type: 'update',
          state: Object.fromEntries(delta),
  ...
  async execute(toolName, args, ctx) {
    try {
      switch (toolName) {
        case 'gossip.start':
          this.logger.warn(`GOSSIP START port ${args.port || this.port}`, {
            user: ctx.userId,
            reason: args.reason,
          });
          await this._load();
          ...
          this.server.on('connection', (ws, req) => {
            ...
            ws.on('message', data => {
              try {
                this._handleMessage(peerId, JSON.parse(data));
              } catch (e) {
                this.logger.error(`GOSSIP MSG ERR: ${e.message}`);
              }
            });
        ...
        case 'gossip.broadcast':
          this.logger.warn(`GOSSIP BROADCAST ${args.topic}`, {
            user: ctx.userId,
            reason: args.reason,
          });
          const msg = {
