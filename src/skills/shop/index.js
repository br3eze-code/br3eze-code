...
class ShopSkill extends BaseSkill {
  ...
  static getTools() {
    return {
      ...
      'shop.create_shipment': { risk: 'medium', description: 'Book a real shipment with a courier provider', parameters: { type: 'object', properties: { orderId: { type: 'string' }, provider: { type: 'string' } }, required: ['orderId', 'provider'] } },
      ...
      'shop.related_products': { risk: 'low', description: 'Get other active products in the same category', parameters: { type: 'object', properties: { productRef: { type: 'string' }, limit: { type: 'number', default: 3 } }, required: ['productRef'] } },
  ...
  async execute(toolName, args, ctx) {
    args = args || {};
    switch (toolName) {
      case 'shop.list_products': return (await shop.listProducts({ category: args.category, search: args.search, scope: scopeOf(ctx) })).map((p) => ({ ...p, url: shop.productUrl(p.id) }));
      case 'shop.list_payment_methods': return shop.getPaymentMethods({ country: args.country || ctx?.country || null, device: args.device || ctx?.device || 'mobile', uid: ctx?.userId || args.uid || null, config: ctx?.paymentConfig || {} });
      ...
      case 'shop.get_product': { const p = await shop.getProduct(args.productRef, scopeOf(ctx)); return p ? { ...p, url: shop.productUrl(p.id) } : p; }
      ...
      case 'shop.related_products': { const p = await shop.getProduct(args.productRef, scopeOf(ctx)); if (!p) throw new Error(`Product "${args.productRef}" not found.`); return (await shop.relatedProducts(p, args.limit || 3, scopeOf(ctx))).map((r) => ({ ...r, url: shop.productUrl(r.id) })); }
      case 'shop.recommend_products': {
        const scope = scopeOf(ctx); const seedProduct = await shop.getProduct(args.productRef, scope); if (!seedProduct) throw new Error(`Product "${args.productRef}" not found.`);
