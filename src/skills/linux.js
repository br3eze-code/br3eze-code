      ...
      switch (toolName) {
        ...
        case 'lin.service.status':
          return await this._exec(
            args.host,
            `systemctl show ${args.name} --no-page -p LoadState,ActiveState,SubState | tr '\n' ' '`
          );

        case 'lin.service.restart':
          this.logger.warn(`LINUX SERVICE RESTART ${args.host}`, {
            user: ctx.userId,
            service: args.name,
            reason: args.reason,
          });
        ...
        case 'lin.process.list':
          ...
          const ps = await this._exec(
            args.host,
            `ps -eo pid,user,%cpu,%mem,comm ${sort} | head -n ${(args.top || 10) + 1}`
          );
          return ps
            .split('\n')
            .slice(1)
        ...
        case 'lin.process.kill':
          this.logger.warn(`LINUX KILL PID ${args.pid} on ${args.host}`, {
            user: ctx.userId,
            reason: args.reason,
          });
        ...
        case 'lin.logs.journal':
          ...
          return await this._exec(
            args.host,
            `journalctl ${unit} -p ${prio} --since="${args.since || '1h'}" --no-pager -n 50`
          );

        case 'lin.pkg.outdated':
          // Auto-detect package manager
          const pm = await this._exec(
            args.host,
            `command -v apt && echo apt || command -v dnf && echo dnf || command -v yum && echo yum`
          );
          if (pm === 'apt')
            return await this._exec(args.host, `apt list --upgradable 2>/dev/null | tail -n +2`);
