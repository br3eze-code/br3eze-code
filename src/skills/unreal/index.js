...
class UnrealSkill extends BaseSkill {
      ...
      'ue.multiplayer.deploy': {
        ...
        parameters: {
          ...
          properties: {
            ...
            region: {
              type: 'string',
              enum: ['us-west-2', 'eu-central-1', 'ap-northeast-1'],
              default: 'us-west-2',
            },
      ...
      'ue.insights.query': {
        ...
        parameters: {
          ...
          properties: {
            ...
            query: {
              type: 'string',
              enum: ['summary', 'cpu', 'gpu', 'memory', 'loadtime', 'bookmarks'],
              default: 'summary',
            },
      ...
      'ue.horde.job': {
        ...
        parameters: {
          ...
          properties: {
            ...
            template: {
              type: 'string',
              enum: ['Editor', 'Cook', 'Tests', 'Package', 'Gauntlet'],
              default: 'Cook',
            },
      ...
      'ue.cook.run': {
        ...
        parameters: {
          ...
          properties: {
            ...
            platform: {
              type: 'string',
              enum: ['Windows', 'Linux', 'PS5', 'XSX', 'IOS', 'Android'],
              default: 'Linux',
            },
            maps: { type: 'array', items: { type: 'string' } },
  ...
  async _runUAT(args, timeout = 1800) {
    ...
    if (stderr.includes('Error:') || stderr.includes('Failed'))
      throw new Error(stderr.slice(-2000));
  ...
  async _remotePython(project, script, timeout = 60) {
    try {
      const res = await axios.put(
        `${this.remotePy}/remote/object/call`,
        {
      ...
      throw new Error(
        `Remote Python failed: ${e.message}. Ensure Editor running with -RemotePython`
      );
    }
  }
    ...
    try {
      switch (toolName) {
        case 'ue.android.test': {
          this.logger.warn(`UE ANDROID TEST ${args.device_id}`, {
            user: ctx.userId,
            test: args.test,
            reason: args.reason,
          });
          ...
          if (!adbOut.includes(args.device_id))
            throw new Error(`Device ${args.device_id} not found in adb devices`);
          ...
          if (!passed) {
            ...
            return {
              ...
              test: args.test,
              passed,
              logcat: logcat.slice(-6000),
              log: andOut.slice(-2000),
            };
          }
          ...
          return {
            ...
            test: args.test,
            passed,
            log: andOut.slice(-4000),
          };
        }

        case 'ue.android.playstore': {
          this.logger.warn(`UE PLAYSTORE UPLOAD ${args.aab_path}`, {
            user: ctx.userId,
            track: args.track,
            reason: args.reason,
          });
          ...
          if (!process.env.GOOGLE_PLAY_SERVICE_ACCOUNT)
            throw new Error('GOOGLE_PLAY_SERVICE_ACCOUNT json not set');
...
`;
          const { stdout } = await execAsync(`python3 -c "${py.replace(/"/g, '\\"')}"`, {
            timeout: 600000,
          });
          return {
            project: args.project,
        ...
        case 'ue.switch.deploy': {
          this.logger.warn(`UE SWITCH DEPLOY ${args.devkit_ip}`, {
            user: ctx.userId,
            nsp: args.nsp_path,
            reason: args.reason,
          });
          ...
          const devmenu = path.join(
            process.env.NINTENDO_SDK_ROOT,
            'Tools/CommandLineTools/DevMenu/DevMenu.exe'
          );
        ...
        case 'ue.switch.lotcheck':
          this.logger.warn(`UE SWITCH LOTCHECK ${args.nsp_path}`, {
            user: ctx.userId,
            reason: args.reason,
          });
          const nsp2 = path.resolve(this.projectRoot, args.nsp_path);
          const lotcheck = path.join(
            process.env.NINTENDO_SDK_ROOT,
            'Tools/CommandLineTools/LotCheck/LotCheck.exe'
          );

          const { stdout: lotOut, stderr: lotErr } = await execAsync(
            `"${lotcheck}" -p "${nsp2}" -o "${this.outputDir}/lotcheck_${Date.now()}.html"`,
            { timeout: 600000 }
          );
          ...
          return {
            ...
            passed,
            report: reportPath,
            log: lotOut.slice(-4000),
          };
        case 'ue.multiplayer.deploy':
          this.logger.warn(`UE AGONES DEPLOY ${args.project} ${args.image}`, {
            user: ctx.userId,
            region: args.region,
            reason: args.reason,
          });
...
`;
          ...
          const apply = await this.agent.registry.execute(
            'k8s.apply',
            {
              yaml: fleetYaml,
          ...
          for (let i = 0; i < 30; i++) {
            ...
            const gs = await this.agent.registry.execute(
              'k8s.get',
              {
                kind: 'GameServer',
          ...
          setTimeout(async () => {
            await this.agent.registry.execute(
              'k8s.delete',
              { kind: 'Fleet', name: fleetName, namespace: 'agones-system' },
              'system'
            );
        ...
        case 'ue.multiplayer.stop':
          ...
          const result = await this.agent.registry.execute(
            'k8s.delete',
            {
              kind: 'Fleet',
        ...
        case 'ue.loc.gather': {
          ...
          return {
            project: args.project,
            config: args.config,
            words_gathered: parseInt(words),
            files_updated: parseInt(files),
        ...
        case 'ue.loc.sync':
          this.logger.warn(`UE LOC SYNC ${args.service} ${args.project}`, {
            user: ctx.userId,
            reason: args.reason,
          });
          ...
          if (args.service === 'onesky') {
            ...
            if (args.push_source) {
              ...
              await axios.post(
                `https://api.oneskyapp.com/1/projects/${ONESKY_PROJECT}/files`,
                form,
                {
            ...
            if (args.pull_translations) {
              const langs = await axios.get(
                `https://api.oneskyapp.com/1/projects/${ONESKY_PROJECT}/languages`,
                {
                  auth: { username: ONESKY_KEY, password: ONESKY_SECRET },
                }
              );
              for (const lang of langs.data.data) {
                ...
                const res = await axios.get(
                  `https://api.oneskyapp.com/1/projects/${ONESKY_PROJECT}/translations`,
                  {
              ...
              // Recompile loc
              await this._runUAT(
                ['Localize', `-project="${proj14}"`, `-targetname=${args.config}`, '-compile'],
                120
              );
            }
            ...
            return {
              project: args.project,
              service: 'onesky',
              pushed: args.push_source,
              pulled: args.pull_translations,
            };
        ...
        case 'ue.uht.run': {
          ...
          const uhtDll = path.join(
            path.dirname(this.uatPath),
            '..',
            'DotNET',
            'UnrealHeaderTool',
            'UnrealHeaderTool.dll'
          );
          ...
          const errors =
            (stdout.match(/Error:/g) || []).length + (stdout.match(/error C\d+/g) || []).length;
        ...
        case 'ue.p4.submit':
          this.logger.warn(`UE P4 SUBMIT ${args.project}`, {
            user: ctx.userId,
...
`;
        ...
        case 'ue.horde.job':
          this.logger.warn(`UE HORDE JOB ${args.template} ${args.stream}`, {
            user: ctx.userId,
            change: args.change,
            reason: args.reason,
          });
        ...
        case 'ue.python.exec': {
          this.logger.warn(`UE PYTHON EXEC ${args.project}`, {
            user: ctx.userId,
            reason: args.reason,
          });
        ...
        case 'ue.lighting.build': {
          this.logger.warn(`UE LIGHTING BUILD ${args.project}`, {
            user: ctx.userId,
            maps: args.maps,
        ...
        case 'ue.cook.run': {
          this.logger.warn(`UE COOK ${args.project} ${args.platform}`, {
            user: ctx.userId,
            reason: args.reason,
          });
        ...
        case 'ue.test.automation': {
          this.logger.warn(`UE TESTS ${args.project}`, {
            user: ctx.userId,
            filter: args.tests,
            reason: args.reason,
          });
        ...
        case 'ue.render.movie': {
          this.logger.warn(`UE MRQ RENDER ${args.project}`, {
            user: ctx.userId,
            seq: args.level_sequence,
            reason: args.reason,
          });
...
`;
          ...
          return {
            project: args.project,
            sequence: args.level_sequence,
            s3_output: args.s3_output,
            log: renOut.slice(-4000),
          };
        }

        case 'ue.asset.import': {
          this.logger.warn(`UE IMPORT ${args.source} -> ${args.destination}`, {
            user: ctx.userId,
            reason: args.reason,
          });
