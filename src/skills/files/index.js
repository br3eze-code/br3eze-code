...
class FileStorageSkill extends BaseSkill {
      ...
      'files.gdrive': {
        ...
        parameters: {
          ...
          properties: {
            action: {
              type: 'string',
              enum: ['upload', 'download', 'list', 'share', 'delete', 'search'],
              default: 'list',
            },
            file: { type: 'string', description: 'local path or file ID' },
  ...
  async _onedrive(args, ctx) {
    ...
    if (!token)
      throw new Error('OneDrive token required. Run OAuth flow or set onedrive_token in config.');
    ...
    if (args.action === 'share') {
      const res = await axios.post(
        `${base}/items/${args.file}/createLink`,
        {
          type: args.link_type,
          scope: 'anonymous',
        },
  ...
  async _gdrive(args, ctx) {
    if (!this.gdrive)
      throw new Error('Google Drive not configured. Set gdrive_credentials in config.');
    ...
    if (args.action === 'upload') {
      ...
      const fileMetadata = {
        name: args.name || path.basename(args.file),
        parents: args.folder_id ? [args.folder_id] : [],
      };
  ...
  async _sync(args, ctx) {
    ...
    if (args.service === 'onedrive' || args.service === 'both') {
      ...
      const matches = files.filter(
        f => args.pattern === '*' || f.match(args.pattern.replace('*', '.*'))
      );
  ...
  async _bucket(args, ctx) {
    ...
    if (args.action === 'list') {
      const res = await s3.send(
        new ListObjectsV2Command({ Bucket: args.bucket, Prefix: args.key })
      );
      return {
        action: 'list',
        bucket: args.bucket,
