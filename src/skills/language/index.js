...
class LanguageSkill extends BaseSkill {
  ...
  static description =
    'Translation, grammar check, style rewrite, language detect, vocabulary, readability';
  ...
  static getTools() {
    return {
      'language.discourse': {
        risk: 'low',
        description:
          'Discourse analysis: speech acts, coherence, cohesion, pragmatics, implicature',
        parameters: {
          type: 'object',
          properties: {
            text: { type: 'string' },
            ...
            mode: {
              type: 'string',
              enum: ['speech_acts', 'coherence', 'cohesion', 'pragmatics', 'all'],
              default: 'all',
            },
          },
          required: ['text'],
      ...
      'language.typology': {
        ...
        parameters: {
          ...
          properties: {
            ...
            feature: {
              type: 'string',
              enum: ['word_order', 'alignment', 'morphology', 'phonology', 'all'],
              default: 'all',
            },
          },
          required: ['lang'],
      ...
      'language.compare_typology': {
        ...
        parameters: {
          ...
          properties: {
            ...
            feature: {
              type: 'string',
              description: 'WALS feature or general: word_order, case, tense',
            },
          },
          required: ['langs'],
        },
      ...
      'language.dialect': {
        ...
        parameters: {
          ...
          properties: {
            ...
            mode: {
              type: 'string',
              enum: ['identify', 'features', 'convert', 'map'],
              default: 'identify',
            },
            target_dialect: {
              type: 'string',
              description: 'for convert mode: en-US, en-GB, en-AU, es-MX, es-ES, etc',
            },
          },
          required: ['text'],
        },
      ...
      'language.syntax': {
        ...
        parameters: {
          ...
          properties: {
            ...
            format: {
              type: 'string',
              enum: ['dependencies', 'constituency', 'both'],
      ...
      'language.prosody': {
        ...
        parameters: {
          ...
          properties: {
            ...
            mode: {
              type: 'string',
              enum: ['stress', 'rhythm', 'intonation', 'meter', 'all'],
              default: 'all',
            },
          },
          required: ['text'],
      ...
      'language.morphology': {
        ...
        parameters: {
          ...
          properties: {
            ...
            type: {
              type: 'string',
              enum: ['breakdown', 'inflection', 'derivation', 'compound'],
              default: 'breakdown',
            },
          },
          required: ['word'],
        ...
        parameters: {
          ...
          properties: {
            ...
            lang: {
              type: 'string',
              default: 'en-us',
              description: 'en-us, en-gb, es, fr, de, etc',
            },
      ...
      'language.corpus': {
        ...
        parameters: {
          ...
          properties: {
            ...
            corpus: {
              type: 'string',
              enum: ['google_books', 'coca', 'bnc', 'general'],
              default: 'general',
            },
            metric: {
              type: 'string',
              enum: ['frequency', 'collocations', 'ngrams', 'trends', 'all'],
              default: 'all',
            },
      ...
      'language.rhymes': {
        ...
        parameters: {
          ...
          properties: {
            ...
            type: {
              type: 'string',
              enum: ['perfect', 'near', 'alliteration', 'consonant'],
              default: 'perfect',
            },
      ...
      'language.rewrite': {
        ...
        parameters: {
          ...
          properties: {
            ...
            style: {
              type: 'string',
              enum: ['formal', 'casual', 'academic', 'simple', 'concise', 'persuasive'],
              default: 'concise',
            },
      ...
      switch (toolName) {
        case 'language.discourse': {
          ...
          if (!this.agent.registry.skills.llm)
            throw new Error('Discourse analysis requires llm skill');
...
Feature focus: ${args.feature}.`;
          ...
          try {
            ...
            return { lang: args.lang, typology: res.text };
          }
        }
        case 'language.compare_typology': {
          this.logger.info(`LANGUAGE COMPARE TYPOLOGY ${args.langs.join(',')}`, {
            user: ctx.userId,
          });
          if (!this.agent.registry.skills.llm)
            throw new Error('Typology comparison requires llm skill');
...
JSON: {"feature":"","comparison":[{"lang":"","value":"","note":""}],"pattern":"","outliers":[]}`;
          ...
          try {
            ...
            return { langs: args.langs, comparison: res.text };
          }
        }
...
Text:\n${args.text}`;
          ...
          try {
            ...
            return { text: args.text, pragmatics: res.text };
          }
        }
        ...
        case 'language.dialect': {
          ...
          if (!this.agent.registry.skills.llm)
            throw new Error('Dialect analysis requires llm skill');
...
Text: ${args.text}`;
              ...
              try {
                result.dependencies = JSON.parse(res.text);
              } catch {
                result.dependency_note = res.text;
              }
            }
          }
...
Text: ${args.text}`;
              const res = await this.agent.registry.execute('llm.chat', { prompt }, ctx.userId);
              try {
                result.constituency = JSON.parse(res.text);
              } catch {
                result.constituency_note = res.text;
              }
            }
          }
        ...
        case 'language.register': {
          ...
          if (!this.agent.registry.skills.llm)
            throw new Error('Register analysis requires llm skill');
...
Text:\n${args.text}`;
          ...
          try {
            ...
            return { text: args.text, analysis: res.text };
          }
        }
        case 'language.variation': {
          this.logger.info(`LANGUAGE VARIATION ${args.lang}`, { user: ctx.userId });
          if (!this.agent.registry.skills.llm)
            throw new Error('Variation analysis requires llm skill');
...
Text:\n${args.text}`;
        ...
        case 'language.prosody': {
          this.logger.info(`LANGUAGE PROSODY ${args.mode}: ${args.text.slice(0, 40)}`, {
            user: ctx.userId,
          });
          const nlp = require('compromise');
          const doc = nlp(args.text);
          ...
          if (args.mode === 'stress' || args.mode === 'all') {
            ...
            try {
              ...
              const ipa = execSync(
                `espeak-ng -v ${args.lang} -q -x --ipa "${args.text.replace(/"/g, '\\"')}"`,
                { encoding: 'utf8' }
              ).trim();
              const stress_marks = [...ipa.matchAll(/[ˈˌ]/g)].map((m, i) => ({
                position: m.index,
                type: m[0] === 'ˈ' ? 'primary' : 'secondary',
              }));
              result.stress = {
                ipa,
                marks: stress_marks,
                pattern: ipa.replace(/[^ˈˌ]/g, '').split('').join('-'),
              };
            } catch {
              result.stress = {
                note: 'Install espeak-ng for stress marks. Syllables estimated.',
                syllables: terms.map(t => t.syllables || t.text.length / 3),
              };
            }
          }
          ...
          if (args.mode === 'rhythm' || args.mode === 'all') {
            ...
            const syllables = words.map(
              w =>
                w
                  .toLowerCase()
                  .replace(/[^a-z]/g, '')
                  .replace(/[aeiouy]+/g, 'a').length || 1
            );
            result.rhythm = {
              words: words.length,
              syllables: syllables.reduce((a, b) => a + b, 0),
              avg_syllables_per_word: (syllables.reduce((a, b) => a + b, 0) / words.length).toFixed(
                2
              ),
          ...
          if (args.mode === 'intonation' || args.mode === 'all') {
            ...
            if (this.agent.registry.skills.llm) {
              ...
              const res = await this.agent.registry.execute('llm.chat', { prompt }, ctx.userId);
              try {
                result.intonation = JSON.parse(res.text);
              } catch {
                result.intonation = { note: res.text };
              }
            }
          }
          ...
          if (args.mode === 'meter' || args.mode === 'all') {
            ...
            if (result.stress?.pattern) {
              ...
              if (/^([ˌ]?ˈ)+$/.test(p.replace(/-/g, '')))
                result.meter = { type: 'iambic', pattern: p };
              else if (/^(ˈ[ˌ]?)+$/.test(p.replace(/-/g, '')))
                result.meter = { type: 'trochaic', pattern: p };
        ...
        case 'language.morphology': {
          ...
          const res = await this.agent.registry.execute(
            'llm.chat',
            { prompt: prompts[args.type], model: 'gpt-4' },
            ctx.userId
          );
          try {
...
Text:\n${args.text}`;
          ...
          try {
            ...
            return { text: args.text, scansion: res.text };
          }
        }
...
JSON: {
  ...
  "semantic_shift":[{"period":"","meaning":""}]
          ...
          try {
            ...
            return { word: args.word, etymology: res.text };
          }
        }
        case 'language.ipa': {
          this.logger.info(`LANGUAGE IPA ${args.lang}: ${args.text.slice(0, 30)}`, {
            user: ctx.userId,
          });
          ...
          try {
            ...
            if (args.format === 'both') {
              const arpa = execSync(`espeak-ng -v ${voice} -q -x --phonout "${escaped}"`, {
                encoding: 'utf8',
              }).trim();
            ...
            // LLM fallback
            if (!this.agent.registry.skills.llm)
              throw new Error('IPA requires espeak-ng or llm skill');
        ...
        case 'language.phonetics': {
          ...
          try {
            const { execSync } = require('child_process');
            const ipa = execSync(`espeak-ng -v en -q -x --ipa "${args.word}"`, {
              encoding: 'utf8',
            }).trim();
        ...
        case 'language.corpus': {
          ...
          try {
            ...
            if (args.metric === 'frequency' || args.metric === 'all') {
              ...
              results.frequency_note = results.frequency
                ? `f:${results.frequency} = occurs ~${Math.pow(10, parseFloat(results.frequency)).toFixed(0)} per million words`
                : 'unknown';
            }
            ...
            if (args.metric === 'trends' || args.metric === 'all') {
              ...
              if (this.agent.registry.skills.llm) {
                ...
                const res = await this.agent.registry.execute('llm.chat', { prompt }, ctx.userId);
                try {
                  results.trends = JSON.parse(res.text);
                } catch {
                  results.trends_note = res.text;
                }
              }
            }
...
JSON: {
  ...
  "when_to_use_each":""
          ...
          try {
            ...
            return { word1: args.word1, word2: args.word2, analysis: res.text };
          }
        }
        case 'language.thesaurus': {
          this.logger.info(`LANGUAGE THESAURUS ${args.word} ${args.relation}`, {
            user: ctx.userId,
          });
          ...
          try {
            ...
            if (!this.agent.registry.skills.llm)
              throw new Error('Thesaurus requires network or llm skill');
...
Hypernym = broader category. Hyponym = specific type. Meronym = part of. Holonym = whole of.`;
            ...
            try {
              return { word: args.word, lang: args.lang, ...JSON.parse(res.text) };
            } catch {
              return { word: args.word, note: res.text };
            }
          }
        }
...
Be concise but scholarly.`;
              const llmRes = await this.agent.registry.execute(
                'llm.chat',
                { prompt, model: 'gpt-4' },
                ctx.userId
              );
              try {
                return JSON.parse(llmRes.text);
              } catch {
                return { word: args.word, etymology: llmRes.text };
              }
            }
        ...
        case 'language.rhymes': {
          ...
          try {
            ...
            if (!this.agent.registry.skills.llm)
              throw new Error('Rhymes requires network or llm skill');
            const prompt = `List ${args.limit} ${args.type} rhymes for "${args.word}". JSON: {"rhymes":[]}`;
            const res = await this.agent.registry.execute('llm.chat', { prompt }, ctx.userId);
            try {
              return { word: args.word, type: args.type, ...JSON.parse(res.text) };
            } catch {
              return { word: args.word, note: res.text };
            }
          }
        ...
        case 'language.grammar': {
          ...
          if (doc.match('#Verb #Verb').found)
            issues.push({ type: 'double_verb', text: doc.match('#Verb #Verb').text() });
          if (doc.match('a #Vowel').found)
            issues.push({
              type: 'a_an',
              text: doc.match('a #Vowel').text(),
              fix: doc.match('a #Vowel').text().replace('a ', 'an '),
            });
          ...
          if (this.agent.registry.skills.llm) {
            ...
            try {
              suggestions = JSON.parse(res.text);
            } catch {
              suggestions = [{ note: res.text }];
            }
          }
        ...
        case 'language.readability': {
          ...
          const syllables = args.text
            .toLowerCase()
            .replace(/[^a-z]/g, '')
            .replace(/[aeiouy]+/g, 'a').length;
        ...
        case 'language.vocab': {
          ...
          if (args.action === 'extract') {
            ...
            const res2 = await this.agent.registry.execute(
              'llm.chat',
              { prompt: prompt2 },
              ctx.userId
            );
            try {
              return { action: 'define', definitions: JSON.parse(res2.text) };
            } catch {
              return { action: 'define', text: res2.text };
            }
          } else {
            ...
            const res3 = await this.agent.registry.execute(
              'llm.chat',
              { prompt: prompt3 },
              ctx.userId
            );
