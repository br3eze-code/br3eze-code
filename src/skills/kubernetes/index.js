...
class KubernetesSkill extends BaseSkill {
  ...
  _checkCluster(clusterId, ctx) {
    ...
    if (!cluster || cluster.driver !== 'kubernetes')
      throw new Error(`K8s cluster ${clusterId} not found`);
  ...
  async execute(toolName, args, ctx) {
    ...
    try {
      switch (toolName) {
        ...
        case 'k8s.pods.logs':
          const logStream = await this.log(
            ns,
            args.pod,
            args.container,
            undefined,
        ...
        case 'k8s.pods.delete':
          this.logger.warn(`K8S POD DELETE ${ns}/${args.pod}`, {
            user: ctx.userId,
            reason: args.reason,
          });
        ...
        case 'k8s.deployments.restart':
          this.logger.warn(`K8S RESTART ${ns}/${args.name}`, {
            user: ctx.userId,
            reason: args.reason,
          });
          const patch = {
          ...
          await this.appsV1.patchNamespacedDeployment(args.name, ns, patch, undefined, {
            headers: { 'Content-Type': 'application/strategic-merge-patch+json' },
          });
          return { restarted: args.name, namespace: ns };

        case 'k8s.deployments.scale':
          this.logger.warn(`K8S SCALE ${ns}/${args.name} -> ${args.replicas}`, {
            user: ctx.userId,
            reason: args.reason,
          });
          const scale = { spec: { replicas: args.replicas } };
          await this.appsV1.patchNamespacedDeploymentScale(args.name, ns, scale, undefined, {
