...
const TerminalAnimator = {
  ...
  async showSpinner(message, durationMs = 1000) {
    ...
    while (Date.now() < end) {
      process.stdout.write(
        `\r  ${ANSICodes.CYBER_PURPLE}${frames[i % frames.length]}${ANSICodes.RESET} ${message}`
      );
      await sleep(80);
      i++;
    }
  ...
  async decode(text, speed = 40) {
    ...
    for (let i = 0; i < text.length; i++) {
      for (let j = 0; j < 5; j++) {
        ...
        process.stdout.write(
          `\r  ${ANSICodes.BOLD}${current}${ANSICodes.NEON_CYAN}${rand}${ANSICodes.RESET}`
        );
        await sleep(speed / 2);
      }
      current += text[i];
  ...
  progressBar(label, progress, total = 100, width = 30) {
    ...
    process.stdout.write(
      `\r  ${ANSICodes.DIM}${label.padEnd(15)}${ANSICodes.RESET} [${ANSICodes.PRIMARY}${bar}${ANSICodes.RESET}] ${ANSICodes.BOLD}${pct}%${ANSICodes.RESET}`
    );
    if (p >= 1) console.log();
  },
  ...
  printHeader(title) {
    ...
    console.log(
      `  ${ANSICodes.DIM}║${ANSICodes.RESET} ${this.gradient(center, [0, 229, 255], [181, 102, 255])} ${ANSICodes.DIM}║${ANSICodes.RESET}`
    );
