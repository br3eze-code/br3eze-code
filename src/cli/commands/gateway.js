    ...
    .action(async options => {
      ...
      try {
        ...
        try {
          ...
          logger.warn(
            `LLMCoordinator not initialized: ${err.message}. AskEngine running rule-only.`
          );
        }
        ...
        try {
          spinner.stop(chalk.red(`Failed to start: ${error.message}`));
        } catch (_) {}
          ...
          if (tryKill && !isCancel(tryKill)) {
            ...
            try {
              ...
              if (pids.length > 0) {
                ...
                s.stop(
                  chalk.yellow('No processes found on port, but port is blocked. Check firewall?')
                );
              }
            } catch (killErr) {
