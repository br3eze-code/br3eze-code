      ...
      res => {
        ...
        res.on('end', () => {
          ...
          try {
            ...
            reject(new Error(`Bad JSON from gateway: ${e.message}`));
          }
        });
      }
    );
...
function startRepl(dispatch, { json }) {
  console.log(
    'AgentOS interactive ask — type a message and press Enter. Ctrl+C or "exit" to quit.\n'
  );
...
module.exports = program => {
  program
    .command('ask [prompt...]')
    .description(
      'Ask AgentOS a question or give it a command. Omit the prompt to start an interactive session.'
    )
    ...
    .action(async (promptParts, options) => {
      ...
      if (!prompt.trim()) {
        if (!process.stdin.isTTY) {
          console.error(
            'Usage: agentos ask "<your question or command>" (or run with no args in an interactive terminal)'
          );
          process.exitCode = 1;
          return;
        }
      ...
      try {
        if (usingGateway) {
          if (options.stream) {
            console.error(
              '--stream is only supported in standalone mode (no gateway running); printing full response instead.'
            );
          }
          const result = await dispatch(prompt);
          if (options.json) {
