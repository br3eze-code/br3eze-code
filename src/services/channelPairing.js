...
class ChannelPairingService extends EventEmitter {
  ...
  async generateCode(context) {
    ...
    if (!this.channels.has(channel) && channel !== 'api') {
      throw new AgentOSError(ErrorCodes.VALIDATION_ERROR, `Channel ${channel} not registered`, {
  ...
  async completePairing(pairingData, routerInfo) {
    ...
    if (!session) {
      throw new AgentOSError(ErrorCodes.AUTH_FAILED, 'Invalid or expired pairing code', { code });
    }

    // Check expiry
    if (Date.now() > session.expiresAt) {
      this.pending.delete(code);
      throw new AgentOSError(ErrorCodes.AUTH_FAILED, 'Pairing code expired', {
        code,
...
:if ([:len $response] > 0) do={
  ...
  getStatsByChannel() {
    ...
    for (const [type] of this.channels) {
      stats[type] = {
        pending: Array.from(this.pending.values()).filter(s => s.channel === type).length,
        paired: Array.from(this.routers.values()).filter(r => r.pairedBy.channel === type).length,
      };
    }
    return stats;
  }
