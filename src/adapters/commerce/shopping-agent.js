...
import { buildExecutionContext } from '../../core/execution-context.js';
...
function requestIdFor(message = {}) {
  return message.requestId || message.messageId || message.eventId || message.id || message.key?.id || message.message?.id || null;
}

function channelScope(context) {
  return context.scope || { tenantId: context.tenantId || null, siteId: context.siteId || null, domain: context.domain || null };
}

function navigationButtons(extra = []) {
  return [...extra, { label: '🛒 Cart', action: 'shop:cart', data: { action: 'cart' } }, { label: '✖ Cancel', action: 'nav:cancel', data: { action: 'cancel' } }].slice(0, 3);
}

function productButtons(product) {
  const buttons = [];
  if (Array.isArray(product.sizes) && product.sizes.length === 1) {
    buttons.push({ label: `Add ${product.sizes[0]}`, action: `shop:add:${product.id}:${product.sizes[0]}`, data: { action: 'add', productRef: product.id, size: product.sizes[0] } });
  } else if (!Array.isArray(product.sizes) || product.sizes.length === 0) {
    buttons.push({ label: 'Add to cart', action: `shop:add:${product.id}`, data: { action: 'add', productRef: product.id } });
  }
...
export async function handleShop(channel, jid, msg = {}, args = []) {
  ...
  const context = buildExecutionContext({ ...msg, message: msg, userId: msg._uid || msg.userId || jid, platformId: jid, channel: platform, userDoc: msg.userDoc });
  ...
  try {
    if (action === 'list' || action === 'catalog') {
      ...
      const buttons = shown.slice(0, 2).map((p) => ({ label: p.name.slice(0, 30), action: `shop:product:${p.id}`, data: { action: 'product', productRef: p.id } }));
    ...
    if (action === 'add' || action === 'buy') {
      ...
      if (Array.isArray(product.sizes) && product.sizes.length > 1 && !size) return send(channel, jid, `📏 Choose a size for *${product.name}*: ${product.sizes.join(', ')}\nUse \`/shop add ${product.id} <size>\`.`, navigationButtons());
      if (size && Array.isArray(product.sizes) && !product.sizes.includes(size)) return send(channel, jid, `❌ Invalid size. Choose: ${product.sizes.join(', ')}`, navigationButtons());
      const result = await shop.addToCart(platform, context.channelId || jid, product.id, { size, scope });
      return send(channel, jid, `✅ Added *${result.product.name}*${size ? ` (${size})` : ''} to your cart.`, navigationButtons([{ label: 'Checkout', action: 'shop:checkout', data: { action: 'checkout' } }, { label: 'Continue shopping', action: 'shop:list', data: { action: 'list' } }]));
    }
    ...
    if (action === 'cart') {
      ...
      if (!items.length) return send(channel, jid, '🛒 Your cart is empty.', navigationButtons([{ label: 'Browse catalog', action: 'shop:list', data: { action: 'list' } }]));
      const total = shop.subtotal(items);
      const lines = items.map((i) => `• ${i.name}${i.size ? ` (${i.size})` : ''} × ${i.qty} — ${money(i.price * i.qty)}`);
      return send(channel, jid, `🛒 *Your cart*\n\n${lines.join('\n')}\n\nSubtotal: ${money(total)}\nShipping: ${money(shop.SHIPPING_FLAT)}\nTotal: ${money(total + shop.SHIPPING_FLAT)}`, navigationButtons([{ label: 'Checkout', action: 'shop:checkout', data: { action: 'checkout' } }]));
    }
    if (action === 'remove') {
      ...
      return send(channel, jid, '🗑️ Removed from cart.', navigationButtons([{ label: 'View cart', action: 'shop:cart', data: { action: 'cart' } }]));
    }
    if (action === 'checkout') {
      const payMethod = args[2] || 'cod';
      const methods = shop.getPaymentMethods({ uid: context.userId, country: context.country, device: context.device === 'unknown' ? 'mobile' : context.device, config: context.paymentConfig });
      if (!methods.some((method) => method.id === payMethod)) return send(channel, jid, `❌ Payment method ${payMethod} is unavailable. Use /shop methods to see configured options.`, navigationButtons());
      const requestId = requestIdFor(msg);
      if (!requestId) return send(channel, jid, '❌ Checkout request id is missing. Please retry the checkout action.', navigationButtons());
      const channelId = context.channelId || jid;
      const result = await executeCheckout({ idempotencyKey: `channel-${platform}-${String(requestId)}`, merchantId: context.tenantId || context.userDoc?.tenantId || 'default', buyerId: context.userId, platform, channelId, address: context.address || {}, payMethod, scope });
      return send(channel, jid, `✅ *Order placed*\nInvoice: ${result.invoiceNumber}\nTotal: ${money(result.total)}\nPayment: ${result.payMethod}${result.replayed ? '\n↩️ Already processed — this was a retry.' : ''}`, navigationButtons([{ label: 'Shop again', action: 'shop:list', data: { action: 'list' } }]));
    }
    if (action === 'track') {
