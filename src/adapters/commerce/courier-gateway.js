...
function sanitizeLocationContext(context = {}) {
    const permissionGranted = context.locationPermission === true || ['true', 'granted', 'allowed', 'precise', 'approximate'].includes(String(context.locationPermission || '').toLowerCase());
    if (!permissionGranted) return { permissionGranted: false };
    const source = context.locationContext || context.location || {};
    return { permissionGranted: true, countryCode: source.countryCode || source.country || context.country || null, region: source.region || source.state || null, siteId: context.siteId || context.scope?.siteId || null };
}

function auditContext(context = {}) {
    const scope = context.scope || {};
    return { userId: context.userId || null, tenantId: context.tenantId || scope.tenantId || null, domain: context.domain || scope.domain || null, siteId: context.siteId || scope.siteId || null, role: context.role || (Array.isArray(context.roles) ? context.roles[0] : null), location: sanitizeLocationContext(context) };
}

class CourierGateway {
    constructor(config = {}) {
        this.config = { dhlApiKey: config.dhlApiKey || process.env.DHL_API_KEY, dhlBaseUrl: config.dhlBaseUrl || process.env.DHL_BASE_URL || 'https://api-eu.dhl.com', pargoApiKey: config.pargoApiKey || process.env.PARGO_API_KEY, pargoBaseUrl: config.pargoBaseUrl || process.env.PARGO_BASE_URL || 'https://api.pargo.co.za/v3.1', aftershipApiKey: config.aftershipApiKey || process.env.AFTERSHIP_API_KEY, aftershipBaseUrl: config.aftershipBaseUrl || process.env.AFTERSHIP_BASE_URL || 'https://api.aftership.com', ...config };
    ...
    getAvailableProviders(context = {}) {
        ...
        return [...this.providers.entries()].map(([id, p]) => ({ id, name: p.name, configured: p.isConfigured(), verified: p.verified, supportsCreate: typeof p.createShipment === 'function' && p.supportsCreate !== false }));
    }
    _get(providerId) {
...
let instance = null;
function getCourierGateway(config) { if (!instance) instance = new CourierGateway(config); return instance; }
