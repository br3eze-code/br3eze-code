...
class CircuitBreaker {
  ...
  _onFailure(error) {
    ...
    const isSystemic = systemicErrors.some(keyword =>
    ...
    if (this.failures >= this.threshold) {
      ...
      logger.warn(
        `CircuitBreaker OPEN — tripped by: ${error?.message || 'Unknown error'}. Will retry after ${this.timeout}ms`
      );
    } else {
      logger.debug(
        `CircuitBreaker failure ${this.failures}/${this.threshold}: ${error?.message || 'Unknown error'}`
      );
    }
  }
}
...
class MikroTikManager extends EventEmitter {
  constructor(options = {}) {
    ...
    const mikrotikConfig =
      globalConfig.tools?.mikrotik?.connection ||
      globalConfig.adapters?.mikrotik ||
      globalConfig.mikrotik ||
      {};

    this.config = {
      host:
        options.host || options.ip || mikrotikConfig.host || mikrotikConfig.ip || '192.168.88.1',
      user: options.user || mikrotikConfig.username || mikrotikConfig.user || 'admin',
      password:
        options.password || options.pass || mikrotikConfig.password || mikrotikConfig.pass || '',
  ...
  _getId(obj) {
    ...
    if (obj.name || obj.user) {
      logger.debug(
        `[MikroTik] _getId: no ID for '${obj.name || obj.user}'. Keys: [${Object.keys(obj).join(', ')}]`
      );
    }
    return null;
  }
  ...
  disconnect() {
    ...
    if (this.state.client) {
      ...
      try {
        ...
        logger.warn('Close error:', e.message);
      }
      this.state.client = null;
    }
  ...
  async _writeRaw(parts) {
    ...
    if (!rawApi || typeof rawApi.write !== 'function') {
      ...
      throw new ConnectionError(
        'MikroTik write interface unavailable (connection may be transitioning)'
      );
    }
  ...
  _scheduleReconnect() {
    ...
    const delay = Math.min(
      RECONNECT_INTERVAL * Math.pow(2, this.state.reconnectAttempts - 1),
      60_000
    );
    logger.info(
      `Scheduling reconnect ${this.state.reconnectAttempts}/${MAX_RECONNECT_ATTEMPTS} in ${delay}ms`
    );
  ...
  _ensureConnected() {
    if (!this.state.isConnected || !this.state.client || !this.state.conn) {
      const reason = !this.state.isConnected
        ? 'not marked as connected'
        : !this.state.client
          ? 'client is null'
          : 'connection handler is null';
      throw new ConnectionError(
        `MikroTik not connected (${reason}). Connection may have dropped or rebooted.`
      );
    }
    ...
    if (typeof this.state.conn.menu !== 'function') {
      logger.error(
        '[MikroTik] Invalid connection state: this.state.conn is not a RosApiMenu handler',
        {
  ...
  async executeTool(toolName, ...args) {
    ...
    return this.circuitBreaker.execute(async () => {
      ...
      try {
        ...
        const result =
          typeof params === 'object' && !Array.isArray(params) && tool.length === 1
            ? await tool(params)
            : await tool(...args);
  ...
  async addHotspotUser(usernameOrObj, passwordArg, profileArg = 'default') {
    ...
    const existing = await this.state.conn.menu('/ip/hotspot/user').where('name', username).get();
    ...
    if (existing.length > 0) {
      ...
      try {
        ...
        if (
          (lowerMsg.includes('value of profile') ||
            lowerMsg.includes('does not match any value of profile')) &&
          profile !== 'default'
        ) {
          logger.warn(
            `Profile '${profile}' not found on router during update. Falling back to 'default' for user ${username}`
          );
      ...
      try {
        ...
        if (e.message.includes('match any value of profile') || e.message.includes('not found')) {
          logger.warn(
            `Profile '${profile}' not found on router during creation. Falling back to 'default' for user ${username}`
          );
  ...
  async getHotspotProfiles() {
    ...
    try {
      ...
      throw new ToolExecutionError(
        'hotspot.profiles',
        `Failed to fetch hotspot profiles: ${err.message}`
      );
    }
  }
  ...
  async updateHotspotProfile(params = {}) {
    ...
    try {
      ...
      throw new ToolExecutionError(
        'hotspot.profile.update',
        `Failed to update profile '${name}': ${err.message}`
      );
    }
  }
  ...
  async getSystemResource() {
    ...
    try {
      ...
      throw new ToolExecutionError(
        'system.resource',
        `Failed to fetch system resources: ${err.message}`
      );
    }
  }
  ...
  async removeHotspotUser(usernameOrObj) {
    const username =
      typeof usernameOrObj === 'object'
        ? usernameOrObj.username || usernameOrObj.target || usernameOrObj.id
        : usernameOrObj;

    this._ensureConnected();
    if (!username) throw new ToolExecutionError('user.remove', 'Username required');

    const users = await this.state.conn.menu('/ip/hotspot/user').where('name', username).get();

    if (users.length === 0) {
      logger.warn(
        `MikroTikManager: User '${username}' not found on router during removal attempt.`
      );
    ...
    if (!id) {
      logger.error(
        `MikroTikManager: Found user '${username}' but missing ID. User object: ${JSON.stringify(user)}`
      );
  ...
  async editHotspotUser(usernameOrObj, paramsArg) {
    const username =
      typeof usernameOrObj === 'object'
        ? usernameOrObj.username || usernameOrObj.target
        : usernameOrObj;
    const params = typeof usernameOrObj === 'object' ? usernameOrObj : paramsArg;
  ...
  async disableHotspotUser(usernameOrObj) {
    ...
    try {
      ...
      if (!id) {
        logger.error(
          `[MikroTik] disableHotspotUser: No ID found for user '${username}'. Keys: ${Object.keys(user).join(', ')}`
        );
      ...
      logger.error(
        `[MikroTik] disableHotspotUser failed for '${username}': ${err.message || 'unknown error'}`,
        err
      );
      throw err;
    }
  ...
  async enableHotspotUser(usernameOrObj) {
    ...
    try {
      ...
      if (!id) {
        logger.error(
          `[MikroTik] enableHotspotUser: No ID found for user '${username}'. Keys: ${Object.keys(user).join(', ')}`
        );
        return { action: 'failed', reason: 'missing_id', username };
      }
      ...
      logger.error(
        `[MikroTik] enableHotspotUser failed for '${username}': ${err.message || 'unknown error'}`,
        err
      );
      throw err;
    }
  ...
  async addHotspotProfile(params) {
    const { name, sharedUsers, rateLimit, transparentProxy, macCookieTimeout } = params;
    this._ensureConnected();
  ...
  async editHotspotProfile(params) {
    const { id, name, ...updates } = params;
    this._ensureConnected();

    if (!id && !name)
      throw new ToolExecutionError('profile.edit', 'Profile ID or name is required');

    let targetId = id;
    if (!targetId && name) {
      const profiles = await this.state.conn
        .menu('/ip/hotspot/user/profile')
        .where('name', name)
        .get();
      if (profiles.length === 0)
        throw new ToolExecutionError('profile.edit', `Profile '${name}' not found`);
    ...
    if (updates.transparentProxy !== undefined)
      rosUpdates['transparent-proxy'] = updates.transparentProxy ? 'yes' : 'no';
  ...
  async removeHotspotProfile(params) {
    ...
    if (!targetId && name) {
      const profiles = await this.state.conn
        ...
        .get();
      if (profiles.length === 0)
        throw new ToolExecutionError('profile.remove', `Profile '${name}' not found`);
      targetId = this._getId(profiles[0]);
    }
  ...
  async getUserReport() {
    ...
    try {
      ...
      throw new ToolExecutionError(
        'users.report',
        `Failed to generate user report: ${err.message}`
      );
    }
  }
  ...
  async getUserStatus(usernameOrObj) {
    const target =
      typeof usernameOrObj === 'object'
        ? usernameOrObj.username || usernameOrObj.name || usernameOrObj.target || usernameOrObj.id
        : usernameOrObj;
    ...
    const [active, users] = await Promise.all([
      this.state.conn
        ...
        .where(isId ? '.id' : 'user', target)
        .get(),
      this.state.conn
        .menu('/ip/hotspot/user')
        .where(isId ? '.id' : 'name', target)
        .get(),
    ]);
  ...
  async kickUser(usernameOrObj) {
    ...
    const active = await this.state.conn.menu('/ip/hotspot/active').where('user', username).get();
  ...
  async getSystemStats(force = false) {
    ...
    try {
      ...
      const normalized = {
        ...
        'architecture-name': getVal(
          ['architecture-name', 'architectureName', 'architecture'],
          'unknown'
        ),
  ...
  async getSystemHealth() {
    ...
    const score = Math.max(
      0,
      100 - (cpu > 80 ? 30 : 0) - (cpu > 95 ? 40 : 0) - (memPct > 90 ? 30 : 0)
    );
    return {
      score,
  ...
  async speedtest(iface = 'ether1') {
    ...
    try {
      ...
      return {
        status: 'started',
        interface: iface,
        message: 'Speed test running. Check router logs for results.',
      };
    } catch (err) {
  ...
  async getFirewallRules(type = 'filter') {
    ...
    if (!validTypes.includes(type)) {
      throw new ToolExecutionError(
        'firewall.list',
        `Invalid type '${type}'. Valid: ${validTypes.join(', ')}`
      );
    }
  ...
  async flood(params = {}) {
    ...
    this._ensureConnected();
    return await this._writeRaw(['/tool/flood-ping', `=address=${target}`, `=count=${count}`]);
  }
