...
class AIOrchestrator extends EventEmitter {
  ...
  async initialize() {
    if (VertexAI && this.config.project) {
      try {
        ...
        console.info(
          `[AIOrchestrator] Vertex AI ready: ${this.config.project}/${this.config.model}`
        );
      } catch (err) {
        console.warn(
          '[AIOrchestrator] Vertex AI init failed, falling back to developer API:',
          err.message
        );
      }
    }
  ...
  async _restGenerate(parts, history, tools) {
    ...
    if (this.config.apiKey) {
      ...
      endpoint =
        `https://${this.config.location}-aiplatform.googleapis.com/v1/projects/` +
  ...
  async _dispatchFunctionCalls(calls, baseResult, sessionId) {
    ...
    for (const call of calls) {
      const route = this.config.agentRoutes.find(
        r => r.intent === call.name || r.capability === call.name
      );
      if (route) {
        try {
