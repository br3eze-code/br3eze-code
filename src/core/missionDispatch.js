...
async function submitMission(input) {
  ...
  const readyCheck = await isMissionReady({
    diagnostics: {
      rpcOk: true,
      stateWritable: true,
  ...
  logger.info(
    `Mission dispatched [${dispatchId}] task=${task.taskId.slice(0, 8)} thinking=${submission.thinking} turns=${maxTurns}`
  );

  _runMissionAsync(task.taskId, dispatchId, submission.mission, {
    maxTurns,
    permissionMode:
      submission.thinking === ThinkingLevel.HIGH ? PermissionMode.AUTO : PermissionMode.PROMPT,
  }).catch(err => {
...
async function _runMissionAsync(taskId, dispatchId, mission, opts) {
  ...
  try {
    ...
    const finalStatus =
      !last || last.stopReason === 'completed' ? TaskStatus.COMPLETED : TaskStatus.FAILED;
...
async function abortMission(taskId) {
  ...
  if (task.status !== TaskStatus.RUNNING) {
    throw Object.assign(new Error(`Task ${taskId} is not running (status: ${task.status})`), {
      status: 409,
    });
  }
