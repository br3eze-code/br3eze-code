...
class WebSocketCLI {
  ...
  async cmdQR([code]) {
    if (!code) {
      this._out({ type: 'error', message: 'Usage: qr <code>' });
      return;
    }
    try {
      const db = await getDatabase();
      ...
      const qrText = await QRCode.toString(JSON.stringify({ code, plan: v.plan, url }), {
        type: 'terminal',
        small: true,
      });
      this._out({ type: 'code', language: 'text', content: qrText });
    } catch (err) {
      this._out({ type: 'error', message: `QR generation failed: ${err.message}` });
    }
  }
