...
class EmailChannel extends BaseChannel {
    ...
    return async (emailAddress, msg, match) => {
      try {
        if (!this.isAuthorized(emailAddress)) {
          return this.send(emailAddress, {
            subject: 'Unauthorized',
            text: 'Your email address is not in the allowed list.',
          });
        }
  ...
  async _handleStart(emailAddress) {
    await this.send(emailAddress, {
      subject: 'Welcome to AgentOS',
      text: 'Welcome to AgentOS via Email! Reply with "menu" to see available commands.',
    });
  }
