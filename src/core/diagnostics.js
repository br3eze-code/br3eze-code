...
function isSystemReady(snapshot) {
  return (
    snapshot.diagnostics.stateWritable &&
...
function isMissionReady(snapshot) {
  return isSystemReady(snapshot) && snapshot.diagnostics.routerHealth === DiagnosticHealth.HEALTHY;
}
...
async function buildRouterDiagnostics() {
  ...
  if (state.isConnected) {
    try {
      routerStats = await mikrotik.executeTool('system.stats');
    } catch (_) {}
  }
  ...
  if (state.reconnectAttempts > 5)
    issues.push(`High reconnect attempt count: ${state.reconnectAttempts}`);
...
async function buildGatewayDiagnostics() {
  ...
  const routerDiag = await buildRouterDiagnostics().catch(err => ({
    health: DiagnosticHealth.OFFLINE,
    isConnected: false,
    issues: [err.message],
  }));
  ...
  if (!config.gateway?.token)
    securityWarnings.push('Gateway token is not set — API is unauthenticated');
  if (!config.telegram?.allowedChats?.length)
    securityWarnings.push('Telegram allowedChats is empty — bot accepts any sender');
...
async function handleSnapshot(req, res) {
  try {
    ...
    res
      .status(500)
      .json({ error: err.message, mode: 'fallback', generatedAt: new Date().toISOString() });
  }
}
