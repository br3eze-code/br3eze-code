 ...
 * executes a capability after scope, authorization, approval and lifecycle
 * checks have been satisfied. Vendor/domain adapters stay outside this layer.
 */
...
export function describeCapability({ id, domain = 'general', skill = null, tool = null, phase = 'plan', risk = 'low', requiresApproval = false, permissions = [] } = {}) {
  ...
  return Object.freeze({
    ...
    phase,
    risk,
    requiresApproval: Boolean(requiresApproval || ['execute'].includes(phase) && risk !== 'low'),
...
export function checkCapabilityBoundary(capability, context = {}, { phase = 'plan', approved = false } = {}) {
  ...
  if (capability && !PHASES.includes(phase)) errors.push('invalid_phase');
  ...
  if (capability?.permissions?.length && !granted.includes('*')) {
    for (const permission of capability.permissions) {
      if (!granted.includes(permission)) errors.push(`missing_permission:${permission}`);
    }
  }
...
export function createExecutionIntent({ taskId = null, wbsId = null, capability, context = {}, input = {}, phase = 'plan' } = {}) {
  ...
  return Object.freeze({
    intentId: `intent_${cryptoRandomUUID()}`,
    taskId,
    wbsId,
    capability: descriptor,
    phase,
    input,
    scope: { tenantId: context.tenantId || null, domainId: context.domainId || null, siteId: context.siteId || null, userId: context.userId || context.actorId || null },
    status: boundary.allowed ? (phase === 'execute' ? 'ready' : 'draft') : 'blocked',
    boundary
  });
}

function cryptoRandomUUID() {
  return globalThis.crypto?.randomUUID?.() || `fallback_${Date.now()}_${Math.random().toString(36).slice(2)}`;
}
