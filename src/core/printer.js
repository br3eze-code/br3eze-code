...
const {
  listSerialPortsFromRegistry: _listSerialPortsFromRegistry,
  comPortToInterface: _comPortToInterface,
  listCUPSPrinters: _listCUPSPrinters,
  listDevPrinters: _listDevPrinters,
} = require('../drivers/printer/discovery');
...
async function _printVoucherDirect(voucherData, printerId = 'PRINTER_MAIN', _triedInterfaces = []) {
  ...
  const isPrinterEnabled = printerEnabledStr
    ? printerEnabledStr !== 'false'
  ...
  const isWindowsRaw =
    process.platform === 'win32' && (iface.startsWith('printer:') || iface.startsWith('serial:'));
  ...
  logger.info(
    `[Printer] Initializing ${printerModel} (${printerWidth}mm). Interface: ${iface}, Type: ${printerTypeStr}`
  );
  ...
  try {
    ...
    if (voucherData.price)
      printer.leftRight(
        'Price:',
        `${voucherData.currency || '$'}${Number(voucherData.price).toFixed(2)}`
      );
    printer.drawLine();
    ...
    logger.error(
      `[Printer] ❌ Error printing for ${voucherData?.username} via ${iface}: ${errorMsg}`
    );
...
`;
  try {
    ...
    let output;
    try {
      output = execSync(`powershell -ExecutionPolicy Bypass -File "${scriptFile}"`, {
        windowsHide: true,
        stdio: 'pipe',
      });
    } catch (execError) {
...
async function testPrinterConnection(printerConfig = {}) {
  ...
  if (iface.startsWith('cups:')) {
    ...
    try {
      ...
      return {
        ...
        portSource,
        portDiscovered,
        message: 'CUPS queue unavailable',
      };
    }
  }
  ...
  if (iface.startsWith('dev:')) {
    ...
    try {
      ...
      return {
        ...
        portSource,
        portDiscovered,
        message: `${devPath} not accessible`,
      };
    }
  }
  ...
  const config = {
    type: printerType,
    interface: iface,
    characterSet: printerConfig.characterSet || 'PC437_USA',
  ...
  try {
    ...
    return {
      ...
      portSource,
      portDiscovered,
      message: portDiscovered ? `Port found but printer not responding` : 'Disconnected',
    };
  } catch (e) {
    ...
    if (msg.includes('Access denied') || msg.includes('EBUSY')) {
      return {
        ...
        portSource,
        portDiscovered,
        message: 'Active (Port Busy / Access Denied)',
      };
    }
...
async function printRaw(buffer, printerConfig = {}) {
  ...
  if (iface.startsWith('cups:')) {
    _spoolToCUPS(buffer, iface.replace('cups:', ''));
    return { success: true };
  }
  if (iface.startsWith('dev:')) {
    _writeToDevNode(buffer, iface.replace('dev:', ''));
    return { success: true };
  }
