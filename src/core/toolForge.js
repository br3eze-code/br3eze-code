...
class ToolProposal {
  ...
  activate() {
    ...
    this.activeAt = new Date().toISOString();
  }
  addReceipt(r) {
...
class ToolForge extends EventEmitter {
  ...
  revoke(toolId, { by = 'admin' } = {}) {
    ...
    if (this.skillRegistry) {
      ['skills', 'manifests', 'implementations'].forEach(m =>
        this.skillRegistry[m]?.delete(toolId)
      );
    }
    this._saveToDisk();
  ...
  async execute(toolId, args = {}, context = {}) {
    ...
    try {
      const { result, durationMs } = await VMSandbox.execute(spec, args, {
        db: this.db,
  ...
  async amend(toolId, { code, description, proposedBy, reason } = {}) {
    ...
    return this.propose({
      ...
      author: proposedBy || old.author,
      proposedBy,
      reason: reason || `Amending ${toolId}`,
    });
  }
  ...
  listActive() {
    return [...this._active.entries()].map(([toolId, { spec, proposal }]) => ({
      ...
      author: spec.author,
      activeAt: proposal.activeAt,
      receipts: proposal.receipts.length,
    }));
  }
  ...
  async _shadowRun(proposal) {
    ...
    let sr;
    try {
      const { result, durationMs } = await VMSandbox.execute(
        proposal.spec,
        {},
        { db: this.db, mikrotik: this.mikrotik }
      );
  ...
  _activate(proposal) {
    ...
    if (this.skillRegistry) {
      const manifest = {
        ...
        description: spec.description,
        tools: {
          [spec.toolName]: { description: spec.description, parameters: spec.paramsSchema },
        },
      };
      const forge = this;
    ...
    logger.info(
      `[ToolForge] ✅ ACTIVE '${spec.toolName}' (${spec.toolId}) toolRef=${spec.toolRef}`
    );
  ...
  _saveToDisk() {
    try {
      const state = {
        proposals: [...this._proposals.values()].map(p => ({
          id: p.id,
          status: p.status,
      ...
      for (const e of proposals) {
        ...
        try {
          ...
          const p = Object.assign(new ToolProposal(spec, { proposedBy: e.proposedBy }), {
            ...
            activeAt: e.activeAt,
            shadowRuns: e.shadowRuns || [],
          });
          this._proposals.set(e.id, p);
          this._activate(p);
          n++;
        } catch (ex) {
          logger.warn(`[ToolForge] Skip corrupt entry ${e.id}: ${ex.message}`);
        }
      }
      if (n) logger.info(`[ToolForge] Reactivated ${n} persisted tool(s)`);
    } catch (err) {
      logger.warn('[ToolForge] Load failed:', err.message);
    }
  }
}
