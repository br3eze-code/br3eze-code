  ...
  async _provisionRouter() {
    ...
    for (const p of profiles) {
      try {
        ...
        const existing = await this.mikrotik.executeRawAPI([
    ...
    const sysInterval = setInterval(async () => {
      ...
      try {
        ...
        if (health) {
          ...
          if (!isNaN(voltage) && voltage < 11.5) {
            // Low voltage alert (assuming 12V supply)
            this.bot?.alertOnce?.(
              'low-voltage',
              `🔌 *Low Voltage Warning:* ${voltage}V\nSystem may be unstable.`
            );
          }
          if (!isNaN(temp) && temp > 65) {
