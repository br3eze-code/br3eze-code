...
export function createHandoff({
  from, to, workPackageId, requestedAction, tenantId, userId, evidence = [],
  acceptanceCriteria = [], payload = {}, riskLevel = 'medium', requiresApproval = false,
  createdAt = new Date().toISOString(),
} = {}) {
  return Object.freeze({
    handoffId: `handoff_${randomUUID()}`,
    from: required(from, 'from'), to: required(to, 'to'), workPackageId: required(workPackageId, 'workPackageId'),
    requestedAction: required(requestedAction, 'requestedAction'), tenantId: required(tenantId, 'tenantId'), userId: required(userId, 'userId'),
    ...
    payload: Object.freeze({ ...payload }), riskLevel, requiresApproval: Boolean(requiresApproval), status: 'requested', createdAt,
  });
}
