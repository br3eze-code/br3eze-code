...
function createRouter(deps) {
  ...
  const { mikrotik, database, askEngine, financial, nodeRegistry, agentMemory, config, brand } =
    deps;
  ...
  router.get('/api/vouchers', authMiddleware, async (req, res) => {
    try {
      ...
      const used =
        req.query.used === 'true' ? true : req.query.used === 'false' ? false : undefined;
  ...
  const redeemSchema = Joi.object({
    code: Joi.string()
      .pattern(/^STAR-[A-Z0-9]{4}-[A-Z0-9]{4}$/)
      .required(),
