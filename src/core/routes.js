...
function createRouter(deps) {
    ...
    // Rate limit: 1 STK push per phone per 60s
    const _mpesaRateLimit = new Map();

    const paySchema = Joi.object({
        phone:    Joi.string().pattern(/^\+?[1-9]\d{8,14}$/).required(),
        plan:     Joi.string().min(1).max(40).required(),
        amount:   Joi.number().positive().optional(),
        provider: Joi.string().valid('safaricom', 'vodacom', 'mtn', 'airtel').optional()
    });

    router.post('/api/payment/initiate', authMiddleware, async (req, res) => {
        const { error, value } = paySchema.validate(req.body);
        if (error) return res.status(400).json({ error: error.details[0].message });

        const { phone, plan, amount, provider } = value;
        const now = Date.now();
        const lastSent = _mpesaRateLimit.get(phone) || 0;
        if (now - lastSent < 60_000) {
            return res.status(429).json({ error: 'Please wait 60 seconds before sending another payment request to this number.' });
        }
        _mpesaRateLimit.set(phone, now);

        try {
            const result = await financial.initiatePayment({ provider: provider || 'safaricom', phone, amount, plan });
            await database.logAuditTrail('api', 'payment.initiate', { phone, plan, provider });
            res.json({ success: true, ...result });
        } catch (err) {
            metrics.errors++;
    ...
    router.get('/api/payment/status/:id', authMiddleware, async (req, res) => {
        const { id } = req.params;
        const provider = req.query.provider || process.env.MPESA_PROVIDER || 'safaricom';
        try {
            const result = await financial.verifyPayment(id);
            res.json({ checkoutRequestId: id, provider, ...result });
        } catch (err) {
            res.status(500).json({ error: err.message });
        }
    });

    router.post('/api/webhook/mpesa/:provider', async (req, res) => {
        const { provider } = req.params;
        try {
            const { MobileMoney } = require('../services/mobile-money');
            const mm = new MobileMoney();
            const parsed = mm.webhook(provider, req.body, process.env.MPESA_WEBHOOK_SECRET);

            if (!parsed.valid) {
                logger.warn(`[Webhook] Invalid ${provider} IPN payload`);
                return res.status(400).json({ error: 'Invalid payload' });
            }

            logger.info(`[Webhook] ${provider} IPN: paid=${parsed.paid}, ref=${parsed.checkoutRequestId}`);
            await database.logAuditTrail('webhook', `payment.ipn.${provider}`, {
                paid:    parsed.paid,
                ref:     parsed.checkoutRequestId,
                phone:   parsed.phone,
                amount:  parsed.amount
            });

            // Forward to billing for voucher issuance if paid
            if (parsed.paid && financial.emit) {
                financial.emit('billing:webhook:paid', { provider, ...parsed });
            }

            res.json({ ResultCode: 0, ResultDesc: 'Accepted' });
        } catch (err) {
            logger.error(`[Webhook] ${req.params.provider} IPN error:`, err.message);
            res.status(500).json({ error: err.message });
        }
    });

    // ── LLM Health (Phase 3 stub) ─────────────────────────────────────────────
    router.get('/api/llm/health', authMiddleware, (_req, res) => {
        res.json({ status: 'ok', note: 'Multi-LLM health routing added in Phase 3' });
    });
