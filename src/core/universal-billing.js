...
class UniversalBilling {
  constructor(config = {}) {
    ...
    const provider = config.paymentProvider || process.env.PAYMENT_PROVIDER || 'none';
  ...
  async createPaymentLink({ plan, label, amount, currency }) {
    ...
    switch (this.provider) {
      case 'stripe':
        return this._stripeLink(plan, amt, cur, lbl, cfg);
      case 'pesapay':
        return this._pesapayLink(plan, amt, cur, lbl, cfg);
      case 'mpesa':
        return this._mpesaLink(plan, amt, cur, lbl, cfg);
      case 'mastercard':
      case 'peach':
        return this._peachLink(plan, amt, cur, lbl, cfg);
      case 'mastercard-a2a':
        return this._mastercardA2ALink(plan, amt, cur, lbl, cfg);
      case 'a2a':
        return this._mastercardA2ALink(plan, amt, cur, lbl, cfg);
      case 'webhook':
        return this._webhookLink(plan, amt, cur, lbl, cfg);
      default:
        throw new Error(
  ...
  async verifyPayment(reference) {
    ...
    switch (this.provider) {
      case 'stripe':
        return this._stripeVerify(reference, cfg);
      case 'pesapay':
        return this._pesapayVerify(reference, cfg);
      case 'mpesa':
        return this._mpesaVerify(reference, cfg);
      case 'mastercard':
      case 'peach':
        return this._peachVerify(reference, cfg);
      case 'mastercard-a2a':
      case 'a2a':
        return this._mastercardA2AVerify(reference, cfg);
      case 'webhook':
        return { paid: true, reference }; // trust webhook IPN
      default:
        return { paid: false, reason: 'no_provider' };
    }
  }
  ...
  async _pesapayLink(plan, amount, currency, label, cfg) {
    ...
    const baseUrl =
      cfg?.credentials?.baseUrl || process.env.PESAPAY_BASE_URL || 'https://www.pesapay.co.za';
    ...
    const response = await axios.post(`${baseUrl}/api/paynow/v2/payment/initiate`, payload, {
  ...
  async _pesapayVerify(reference, cfg) {
    ...
    const resp = await axios.get(`${baseUrl}/api/paynow/v2/payment/query/${reference}`, {
  ...
  async _peachLink(plan, amount, currency, label, cfg) {
    ...
    const resp = await axios.post(`${baseUrl}/v1/checkouts`, params.toString(), {
  ...
  async _peachVerify(checkoutId, cfg) {
    ...
    try {
      const resp = await axios.get(`${baseUrl}/v1/checkouts/${checkoutId}/payment`, {
        params: { entityId },
        headers: { Authorization: `Bearer ${apiKey}` },
      });
  ...
  async _mastercardA2ALink(plan, amount, currency, label, cfg) {
    ...
    const recipientAccount =
      cfg?.credentials?.recipientAccount ||
      process.env.MASTERCARD_RECIPIENT_ACCOUNT ||
      'ZW1234567890';
    const recipientBankCode =
      cfg?.credentials?.recipientBankCode || process.env.MASTERCARD_RECIPIENT_BANK || 'BR3EZE';
  ...
  async createVoucher(params = {}) {
    const { type = 'access', value, resourceId, metadata = {} } = params;
    ...
    if (metadata.generateQR) {
      ...
      voucher.qrCode = await QRCode.toDataURL(
        JSON.stringify({ code, type: this.resourceType, resource: resourceId })
      );
    }
  ...
  calculateExpiry(planOrType, legacyValue) {
    ...
    if (planOrType && typeof planOrType === 'object') {
      ...
      switch (durationUnit) {
        case 'hours':
          now.setHours(now.getHours() + v);
          break;
        case 'days':
          now.setDate(now.getDate() + v);
          break;
        case 'weeks':
          now.setDate(now.getDate() + v * 7);
          break;
        case 'months':
    ...
    switch (type) {
      case 'time':
      case '1h':
        now.setHours(now.getHours() + (Number(value) || 1));
        break;
      case 'day':
      case '1d':
        now.setDate(now.getDate() + (Number(value) || 1));
        break;
      case 'week':
      case '1w':
        now.setDate(now.getDate() + 7 * (Number(value) || 1));
        break;
      case 'month':
      case '1m':
        now.setMonth(now.getMonth() + (Number(value) || 1));
        break;
      default:
        now.setDate(now.getDate() + 1);
    }
  ...
  async guardHotspot() {
    ...
    if (mikrotik.isCircuitOpen) {
      logger.warn(
        '[Billing] Guard: MikroTik circuit breaker is OPEN. Skipping enforcement run to avoid spam.'
      );
      return;
    }
    ...
    try {
        ...
        for (const user of userReport) {
          try {
            ...
            if (status.expired) {
              ...
              if (dbUser && (voucher?.plan || dbUser.lastPlanId)) {
                try {
                  ...
                  if (wallet && wallet.balance >= cost) {
                    logger.info(
                      `[Billing] Auto-renewing plan ${planId} for user ${user.username} (Balance: ${wallet.balance})`
                    );
                  ...
                  logger.warn(
                    `[Billing] Auto-renewal failed for ${user.username}: ${renewErr.message}`
                  );
                }
              }
              // ─────────────────────────────────────────────────────────
              ...
              logger.debug(
                `[Guard] Unidentified active session: ${user.username} (Uptime: ${user.uptime || 'unknown'}, IP: ${user.address || 'unknown'})`
              );
            }
          } catch (userError) {
            logger.error(`[Guard] Failed to process user ${user.username}`, {
      ...
      if (Array.isArray(activeVouchers)) {
        for (const v of activeVouchers.slice(0, 100)) {
          // Audit 100 per run
          try {
  ...
  async auditGhostSessions() {
    ...
    try {
      ...
      if (Array.isArray(userReport)) {
        ...
        logger.info(
          `[Audit] Fetched ${userReport.length} users from MikroTik. Reconciling with database...`
        );
      ...
      logger.info(
        `[Audit] Ghost sessions audit completed. Found ${report.ghosts.length} ghosts out of ${report.totalChecked} users.`
      );
      return report;
    } catch (error) {
  ...
  async checkVoucherStatus(voucher, usage = {}, user = null) {
    ...
    if (this.resourceType === 'compute' || this.resourceType === 'api') {
      ...
      if (user && this.db) {
        ...
        credits = Number(
          voucher?.redemption?.remainingValue ?? voucher?.value ?? user?.credits ?? 0
        );
      }
  ...
  _getPlanAmount(planId, cfg) {
    ...
    const plans = Array.isArray(cfg?.plans) ? cfg.plans : Object.values(cfg?.plans || {}); // tolerate old object-map shape
    const found = plans.find(
      p => p.name === planId || p.mikrotikProfile === planId || p.id === planId
    );
