...
class Database {
    ...
    try {
      ...
      this.sqlite.transaction(() => {
        for (const [id, user] of this._users) {
          this.sqlite
            .prepare(
              'INSERT OR REPLACE INTO users (uid, username, fullname, email, phoneNumber, address, platform, deviceModel, lastIP, role, credits, subscriptions, pendingNotification, channels, createdAt, lastSeen) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
            )
            .run(
              user.uid,
              user.username,
              ...
              user.role,
              user.credits,
              SQLiteDB.toDB(user.subscriptions),
              SQLiteDB.toDB(user.pendingNotification),
              SQLiteDB.toDB(user.channels),
              user.createdAt,
              user.lastSeen
            );
        }
        for (const [code, v] of this._vouchers) {
          this.sqlite
            .prepare(
              'INSERT OR REPLACE INTO vouchers (code, status, used, usedAt, usedBy, redeemedByUsername, plan, planName, durationUnit, durationValue, deviceLimit, value, currency, loginUrl, expiresAt, createdBy, redemption, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
            )
            .run(
              v.code,
              v.status,
  ...
  _loadLocal() {
    const loadMap = (filePath, map) => {
      if (fs.existsSync(filePath)) {
        try {
          ...
          logger.warn(`Load error ${filePath}: ${e.message}`);
        }
      }
    };
    ...
    logger.info(
      `Local DB: ${this._vouchers.size} vouchers, ${this._users.size} users, ${this._plans.size} plans`
    );
  }
  ...
  _saveLocal(key) {
    ...
    try {
      ...
      logger.error(`Save error (${key}): ${e.message}`);
    }
  }
  ...
  hashPlanId(name) {
    ...
    logger.debug(
      `[PlanHash] name="${name}" -> full=${fullHash.substring(0, 8)}... short=${shortHash}`
    );
    return shortHash;
  }
  ...
  async createPlan(planId, data) {
    ...
    if (this.db) await this.db.collection('plans').doc(planId).set(doc);
      ...
      this.sqlite
        .prepare(
          'INSERT OR REPLACE INTO plans (id, name, description, value, currency, durationUnit, durationValue, deviceLimit, active, speedLimit, dataLimit, features, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
        )
        .run(
          planId,
          doc.name,
  ...
  async updatePlan(planId, updates) {
    if (this.db) {
      await this.db.collection('plans').doc(planId).update(updates);
    } else {
      ...
      this.sqlite
        ...
        .run(
          ...
          updated.speedLimit,
          updated.dataLimit,
          SQLiteDB.toDB(updated.features),
          updated.createdAt
        );
    }
  }
  ...
  async createVoucher(code, data = {}) {
    ...
    const doc = {
      ...
      // ── Financials ───────────────────────────────────────────────────
      value:
        data.value != null ? Number(data.value) : data.amount != null ? Number(data.amount) : 0,
    ...
    if (this.db) {
      ...
      this.sqlite
        ...
        .run(
          ...
          updated.expiresAt,
          updated.createdBy,
          SQLiteDB.toDB(updated.redemption),
          updated.createdAt
        );
    }
  }
  ...
  async redeemVoucher(code, userData = {}) {
    ...
    if (this.db) {
      ...
      this.sqlite.transaction(() => {
        ...
        this.sqlite
          .prepare(
            'UPDATE vouchers SET used = ?, usedAt = ?, usedBy = ?, status = ?, redeemedByUsername = ?, redemption = ? WHERE code = ?'
          )
          .run(
            vUpdate.used,
            vUpdate.usedAt,
        ...
        this.sqlite
          .prepare(
            'INSERT OR REPLACE INTO wallets (id, balance, currency, lastUpdated) VALUES (?, ?, ?, ?)'
          )
        ...
        this.sqlite
          .prepare(
            'UPDATE users SET credits = ?, lastSeen = ?, vouchersUsed = ?, platform = COALESCE(?, platform), deviceModel = COALESCE(?, deviceModel), lastIP = COALESCE(?, lastIP) WHERE uid = ?'
          )
          .run(
            newBalance,
            this._ts(),
            SQLiteDB.toDB(vouchersUsed),
        ...
        this.sqlite
          .prepare(
            'INSERT INTO transactions (id, type, userId, amount, currency, status, description, metadata, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)'
          )
          .run(
            txId,
            'voucher_redeem',
            ...
            'completed',
            `Voucher ${code} redeemed for ${val} credits`,
            SQLiteDB.toDB({ voucherCode: code }),
            this._ts()
          );
  ...
  async revokeVoucher(code, reason = '') {
    ...
    if (this.db) await this.db.collection('vouchers').doc(code).update(update);
      ...
      if (v) {
        this._vouchers.set(code, { ...v, ...update });
  ...
  async resolveUser(identifier, context = null) {
    ...
    const isMac =
      /^([0-9A-Fa-f]{2}[:-]){5}([0-9A-Fa-f]{2})$/.test(String(identifier)) ||
      identifier.startsWith('T-');

    if (isVoucher || isMac) {
      logger.debug(
        `[DB] resolveUser: skipping non-user identifier "${identifier}" (${isVoucher ? 'voucher' : 'MAC'})`
      );
      return null;
    }
    ...
    if (user) {
      ...
      if (isEmail || isLongNumeric) {
        logger.warn(`[DB] resolveUser: failed to find record for potential user "${identifier}"`, {
          context,
          identifier,
        });
      } else {
        logger.debug(`[DB] resolveUser: no record for generic identifier "${identifier}"`, {
          context,
          identifier,
        });
      }
  ...
  async upsertUser(userId, data) {
    ...
    if (data.email) {
      ...
      if (emailUser) {
        logger.debug(
          `[DB] upsertUser: found existing user by email "${email}" (ID: ${emailUser.id})`
        );
      ...
      if (this.db) {
        ...
        if (authUser) {
          logger.debug(
            `[DB] upsertUser: bridged channel to Firebase Auth user "${email}" (UID: ${authUser.uid})`
          );
  ...
  async setPendingNotification(userId, notification = {}) {
    ...
    if (this.db) {
      await this.db.collection('users').doc(id).set({ pendingNotification }, { merge: true });
    } else {
      const u = this._users.get(id) || {};
  ...
  async recordUserVoucher(userId, voucherCode) {
    ...
    if (this.db) {
      await this.db
        ...
        .set(
          {
            vouchersUsed: admin.firestore.FieldValue.arrayUnion(voucherCode),
            lastSeen: this._ts(),
          },
          { merge: true }
        );
  ...
  async createTransaction(data = {}) {
    ...
    if (this.db) await this.db.collection('transactions').doc(id).set(doc);
      ...
      this._transactions.push(doc);
      this._saveLocal('transactions');
    }
    return doc;
  }
  ...
  async getTransactions(limit = 50, filters = {}) {
    if (this.db) {
      ...
      return results
        .sort(
          (a, b) =>
            this._toDate(b.createdAt || b.timestamp || 0) -
            this._toDate(a.createdAt || a.timestamp || 0)
        )
        .slice(0, limit);
    }
  ...
  async getRevenue(period = 'daily') {
    ...
    const txs = this._transactions.filter(
      t =>
        t.status === 'completed' &&
    ...
    if (this.db) {
      ...
      await this.db.runTransaction(async t => {
        ...
        if (fromBalance < totalDebit) {
          throw new Error(
            `Insufficient balance. Required: ${totalDebit} (Amount: ${amount} + Fee: ${fee}), Balance: ${fromBalance}`
          );
        }
        ...
        t.set(
          fromRef,
          { balance: fromBalance - totalDebit, lastUpdated: this._ts() },
          { merge: true }
        );
