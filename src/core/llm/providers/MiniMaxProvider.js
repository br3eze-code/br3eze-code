...
class MiniMaxProvider extends BaseProvider {
  ...
  constructor(config = {}) {
    super(config);
    this.model = config.model || process.env.MINIMAX_MODEL || 'abab6.5-chat';
    this.apiKey = config.apiKey || process.env.MINIMAX_API_KEY;
    this.base = config.baseURL || 'https://api.minimax.chat/v1';
  }
  ...
  async generate(messages, tools = []) {
    const response = await fetch(`${this.base}/text_completion_v2`, {
      method: 'POST',
      headers: {
        ...
        messages: messages.map(m => {
          ...
          if (Array.isArray(m.blocks)) {
            ...
            if (toolResults.length > 0) {
              ...
              mapped.content =
                typeof first.output === 'string' ? first.output : JSON.stringify(first.output);
            }
          }
    ...
    return {
      ...
      usage: {
        promptTokenCount: data.usage?.total_tokens || 0,
  ...
  async embed(input) {
    ...
    const data = await response.json();
    if (!response.ok) {
      throw new Error(data.error?.message || 'MiniMax embedding error');
    }

    return {
      embeddings: data.vectors,
      usage: {
        promptTokenCount: data.total_tokens,
      },
    };
  }
}
