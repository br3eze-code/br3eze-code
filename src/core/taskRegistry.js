...
class TaskRegistry extends EventEmitter {
  ...
  list(statusFilter = null, scope = {}) {
    ...
    return values.filter((task) => (!statusFilter || task.status === statusFilter) && ['tenantId', 'domainId', 'siteId', 'userId'].every((key) => !scope[key] || task.scope?.[key] === scope[key]));
  }
  ...
  updateWbs(taskId, stepId, patch = {}) {
    ...
    task.wbs = updateActionWbs(task.wbs, stepId, patch); task.wbsSummary = summarizeActionWbs(task.wbs); task.nextActionProposal = createNextActionProposal({ task, context: task.planningContext || task.scope, now: this.clock() }); task.updatedAt = this.clock(); this._persist(task); this.emit('task:wbs-updated', task); return task;
  }
  ...
  setStatus(taskId, status, reason = null) {
    ...
    const task = this.tasks.get(taskId); if (!task) return null;
    task.status = status; task.updatedAt = this.clock();
  ...
  setPhase(taskId, phase, { approvalId = null } = {}) {
    ...
    if (phase === TaskPhase.EXECUTE) {
      ...
      task.execution.executionStartedAt = this.clock();
  ...
  markPlanReady(taskId, plan = {}) { const task = this.update(taskId, { plan }); if (!task) return null; task.execution.planReady = true; if (task.execution.phase === TaskPhase.PLAN) task.execution.phase = TaskPhase.DRAFT; task.nextActionProposal = createNextActionProposal({ task, context: task.planningContext, now: this.clock() }); this._persist(task); this.emit('task:planned', task); return task; }
  markDraftReady(taskId, draft = {}) { const task = this.update(taskId, { draft }); if (!task) return null; if (!task.execution.planReady) throw new Error('Draft requires a completed plan'); task.execution.draftReady = true; task.execution.phase = task.execution.approvalRequired ? TaskPhase.APPROVE : TaskPhase.DRAFT; this._persist(task); this.emit('task:drafted', task); return task; }
  ...
  beginExecution(taskId) { const task = this.tasks.get(taskId); if (!task) return null; return this.setPhase(taskId, TaskPhase.EXECUTE); }
  ...
  findIncomplete({ scope = {}, staleAfterMs = this.staleAfterMs } = {}) {
    const now = this.clock();
    return this.list(null, scope).filter((task) => !TERMINAL_STATUSES.has(task.status)).map((task) => ({ ...task, stale: now - task.updatedAt > staleAfterMs }));
  }
