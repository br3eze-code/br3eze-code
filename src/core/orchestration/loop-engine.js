...
export const LOOP_STATES = Object.freeze({ RECEIVED: 'RECEIVED', PLANNING: 'PLANNING', EXECUTING: 'EXECUTING', OBSERVING: 'OBSERVING', EVALUATING: 'EVALUATING', COMPLETE: 'COMPLETE', RETRY: 'RETRY', HANDOFF: 'HANDOFF', ESCALATE: 'ESCALATE', FAILED: 'FAILED' });
export const DEFAULT_LOOP_LIMITS = Object.freeze({ maxIterations: 8, maxToolCalls: 25, timeoutMs: 120000, maxHandoffs: 5, maxRetries: 2 });
const TRANSITIONS = Object.freeze({ RECEIVED: ['PLANNING'], PLANNING: ['EXECUTING', 'HANDOFF', 'ESCALATE', 'FAILED'], EXECUTING: ['OBSERVING', 'FAILED'], OBSERVING: ['EVALUATING', 'FAILED'], EVALUATING: ['COMPLETE', 'RETRY', 'HANDOFF', 'ESCALATE', 'FAILED'], RETRY: ['PLANNING', 'ESCALATE', 'FAILED'], HANDOFF: ['PLANNING', 'ESCALATE', 'FAILED'], ESCALATE: ['FAILED', 'HANDOFF'], COMPLETE: [], FAILED: [] });
...
export class LoopEngine {
  ...
  createState({ workId, goal, specialist = null, acceptanceCriteria = [], tenantId, projectId = null, taskId = null }) {
    ...
    const state = { loopId: this.idFactory(), work, acceptanceCriteria: work.acceptanceCriteria, specialist, state: LOOP_STATES.RECEIVED, status: 'running', iteration: 0, toolCalls: 0, retries: 0, handoffCount: 0, observations: [], actions: [], decisions: [], blockers: [], evidence: [], trace: [], startedAt: this.clock() };
  ...
  async run({ state, plan, execute, verify = this.verifier }) {
    ...
    try {
      while (![LOOP_STATES.COMPLETE, LOOP_STATES.FAILED].includes(loop.state)) {
        if (this.clock() - started >= this.limits.timeoutMs || loop.iteration >= this.limits.maxIterations || loop.toolCalls >= this.limits.maxToolCalls) return this.#finish(loop, 'LOOP_LIMIT', 'loop limit reached');
        ...
        if (loop.state === LOOP_STATES.PLANNING) {
          ...
          if (!action?.tool && action?.type !== 'handoff') return this.#finish(loop, 'UNKNOWN_ERROR', 'plan did not produce an action');
