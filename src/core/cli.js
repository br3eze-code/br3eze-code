...
class AgentOSCLI {
  ...
  _printBanner() {
    ...
    console.log(
      pc.cyan('  ║') +
        pc.bold(pc.white(`        AgentOS Platform  ${pc.yellow(`v${v}`)}`.padEnd(56))) +
        pc.cyan('║')
    );
    console.log(
      pc.cyan('  ║') +
        pc.dim('        Modular AI Agent OS · MikroTik Intelligence'.padEnd(58)) +
        pc.cyan('║')
    );
    console.log(pc.cyan('  ╚══════════════════════════════════════════════════════════╝'));
    console.log('');
  ...
  async start() {
    ...
    // Main input loop
    // eslint-disable-next-line no-constant-condition -- intentional REPL loop, exits via break/return below
    while (true) {
  ...
  async cmdStatus() {
    ...
    try {
      ...
      console.log(
        `  ${pc.dim('Free RAM:')}  ${pc.green(fmtBytes(parseInt(st['free-memory']) || 0))}`
      );
  ...
  async cmdRawApi(args) {
    const cmd = args.join(' ');
    if (!cmd) {
      this._warn('Usage: api </path/command>');
      return;
    }
  ...
  async cmdActive() {
    ...
    users.forEach(u =>
      console.log(
        `  ${pc.green('●')}  ${pc.bold(u.user.padEnd(16))} ${pc.dim(u.address.padEnd(16))} ${pc.yellow(u.uptime)}`
      )
    );
    console.log('');
  }
  ...
  async cmdAddUser([username, password, profile = 'default']) {
    if (!username || !password) {
      this._warn('Usage: adduser <name> <pass> [profile]');
      return;
    }
  ...
  async cmdKick([username]) {
    ...
    s.stop(
      res.kicked ? pc.green(`✔ ${pc.bold(username)} kicked`) : pc.yellow(`⚠ ${username} not active`)
    );
  }

  async cmdVoucher([plan]) {
    if (!plan) {
      this._warn('Usage: voucher <plan>');
      return;
    }
    ...
    const code =
      (this.config?.VOUCHER_PREFIX || 'STAR-') +
    ...
    if (expiresAt)
      console.log(
        pc.cyan('  │') +
          pc.dim(`  Expires: ${new Date(expiresAt).toLocaleDateString().padEnd(28)}`) +
          pc.cyan('│')
      );
    console.log(pc.cyan('  └─────────────────────────────────────┘'));
    console.log('');
  ...
  async cmdRedeem([code, username]) {
    if (!code || !username) {
      this._warn('Usage: redeem <code> <username>');
      return;
    }
    ...
    if (v.used) {
      this._warn('Voucher already used');
      return;
    }
    const ok = await this._nav(
      `Redeem ${pc.bold(code)} for user ${pc.bold(username)}  (plan: ${pc.cyan(v.plan)})?`
    );
    if (!ok) {
      this._info('Cancelled.');
      return;
  ...
  async cmdPing([host, count = '4']) {
    if (!host) {
      this._warn('Usage: ping <host> [count]');
      return;
    }
    const s = clack.spinner();
  ...
  async cmdLogs([lines = '20']) {
    ...
    logs.forEach(l =>
      console.log(
        `  ${pc.dim(l.time || '')}  ${pc.cyan((l.topics || '').padEnd(16))}  ${pc.white(l.message || '')}`
      )
    );
    console.log('');
  }
  ...
  async cmdDhcp() {
    ...
    leases
      ...
      .forEach(l =>
        console.log(
          `  ${pc.cyan(l.address.padEnd(16))}  ${pc.white((l.hostname || '').padEnd(20))}  ${pc.dim(l.status || 'bound')}`
        )
      );
    console.log('');
  }
  ...
  async cmdArp() {
    ...
    arp
      ...
      .forEach(e =>
        console.log(
          `  ${pc.cyan('◆')}  ${pc.white(e.address.padEnd(16))}  ${pc.dim(e['mac-address'] || 'N/A')}`
        )
      );
    console.log('');
  }
  ...
  async cmdFirewall() {
    ...
    rules
      ...
      .forEach(r =>
        console.log(
          `  ${pc.yellow(r.chain.padEnd(10))}  ${pc.cyan(r.action.padEnd(10))}  ${pc.dim(r.comment || '')}`
        )
      );
    console.log('');
  }
  ...
  async cmdBlock([target]) {
    if (!target) {
      this._warn('Usage: block <ip-or-mac>');
      return;
    }
  ...
  async cmdUnblock([target]) {
    if (!target) {
      this._warn('Usage: unblock <ip-or-mac>');
      return;
    }
