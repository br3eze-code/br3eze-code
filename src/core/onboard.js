...
function templateRsc(content, extra = {}) {
  const vars = {
    ...
    WHATSAPP_ENABLED:
      extra.whatsappEnabled !== undefined
        ? String(extra.whatsappEnabled)
...
function generateSetupScript(config = {}) {
  const AGENTOS_NODE_URL =
    config.AGENTOS_NODE_URL || process.env.AGENTOS_NODE_URL || 'http://localhost:3000';
  const TELEGRAM_BOT_TOKEN =
    config.TELEGRAM_BOT_TOKEN || process.env.TELEGRAM_BOT_TOKEN || process.env.TELEGRAM_TOKEN || '';
  const TELEGRAM_CHAT_ID =
    config.TELEGRAM_CHAT_ID ||
    process.env.TELEGRAM_CHAT_ID ||
    (process.env.ALLOWED_CHAT_IDS ? process.env.ALLOWED_CHAT_IDS.split(',')[0] : '');
  ...
  try {
    ...
    for (const scriptInfo of scriptsToRun) {
      ...
      if (options.dryRun) {
        ...
        try {
          ...
          await manager.state.conn.write(['/system/script/run', `=number=${tempName}`]);
          ...
          logger.warn(
            `Connection dropped or script ${scriptInfo.name} caused a reboot (expected).`
          );
          ...
          try {
            ...
            if (reconnected && manager.isConnected) {
              ...
              try {
                ...
                const created = scripts.filter(s => s.name === tempName);
                if (created.length > 0) {
                  await manager.state.conn
                    .menu('/system/script')
                    .remove(manager._getId(created[0]));
                }
              } catch (cleanupErr) {
                logger.debug(
                  `Post-reboot cleanup of ${tempName} failed (expected if ephemeral): ${cleanupErr.message}`
                );
              }
            }
          } catch (reconnectError) {
            logger.error(
              `Failed to reconnect after stage ${scriptInfo.name}: ${reconnectError.message}`
            );
          }
        }
      }
    ...
    if (!options.dryRun) {
      try {
        ...
        const identity = options.name
          ? `AgentOS-${options.name}`
          : `AgentOS-${options.id || 'Node'}`;
        logger.info(`Setting system identity to ${identity}...`);
        await manager.state.conn.write(['/system/identity/set', `=name=${identity}`]);

        logger.info('Saving post-onboard backup...');
        await manager.state.conn.write(['/system/backup/save', '=name=agentos-post-onboard']);
      } catch (e) {
    ...
    const result = await create_agent.run(
      {
        ...
        duties:
          'fleet onboarding,agent spawning,health aggregation,policy enforcement,skill distribution',
        ...
        <div class="grid">
            ...
            <div class="card">
                ...
                <div style="display: flex; flex-wrap: wrap; gap: 8px; margin-top: 16px;">
                    ${results
                      .filter(r => r.success)
                      .map(r => `<span class="badge">router-${r.id}</span>`)
                      .join('')}
                ...
                <div style="display: flex; flex-wrap: wrap; gap: 8px; margin-top: 16px;">
                    ${
                      [
                        ...
                        .map(
                          c =>
                            `<span class="badge" style="background: rgba(16, 185, 129, 0.2); color: #10b981;">${c}</span>`
                        )
                        .join('') ||
                      '<span class="badge" style="background: rgba(239, 68, 68, 0.2); color: #ef4444;">None</span>'
                    }
                </div>
...
async function runWizard() {
  ...
  const S = {
    ...
    whatsappAuthDir: './auth_info_baileys',
    whatsappDebug: false,
    slackEnabled: false,
    slackBotToken: '',
    slackChannel: '',
    ...
    econetClientId: '',
    econetClientSecret: '',
    econetFromName: 'AgentOS',
    econetBaseUrl: 'https://api.econet.co.zw',
    ussdEnabled: false,
    atApiKey: '',
    atUsername: '',
    ussdServiceCode: '',
  ...
  const STEPS = [
    ...
    async () => {
      const h = await ask(() =>
        text({
          message: `${pc.cyan('1/7')}  MikroTik IP / hostname`,
          placeholder: '192.168.88.1',
          initialValue: S.host,
          validate: v => (v.trim() ? undefined : 'Required'),
        })
      );
      if (h === CANCEL) return CANCEL;
      S.host = h;
      const u = await ask(() =>
        text({
          message: `${pc.cyan('1/7')}  API username`,
          placeholder: 'agentos-api',
          initialValue: S.user,
        })
      );
      ...
      const p = await ask(() =>
        clackPwd({
          message: `${pc.cyan('1/7')}  API password`,
          validate: v => (v.length >= 4 ? undefined : 'Min 4 chars'),
        })
      );
      if (p === CANCEL) return CANCEL;
      S.pass = p;
      const pt = await ask(() =>
        text({ message: `${pc.cyan('1/7')}  API port`, placeholder: '8728', initialValue: S.port })
      );
      if (pt === CANCEL) return CANCEL;
      S.port = pt || '8728';
      return true;
    },
    ...
    async i => {
      ...
      const u = await ask(() =>
        text({
          message: `${pc.cyan('2/7')}  AgentOS Node URL`,
          placeholder: 'http://your-server:3000',
          initialValue: S.nodeUrl,
          validate: v => {
            try {
              new URL(v.trim());
              return undefined;
    ...
    async i => {
      ...
      const en = await ask(() =>
        confirm({
          message: `${pc.cyan('3/7')}  Enable Telegram bot?`,
          initialValue: S.telegramEnabled,
        })
      );
      if (en === CANCEL) return CANCEL;
      S.telegramEnabled = en;
      if (en) {
        const tok = await ask(() =>
          text({
            message: `${pc.cyan('3/7')}  Bot Token`,
            placeholder: '123456:ABC-xxx',
            initialValue: S.telegramToken,
            validate: v => (v.trim() ? undefined : 'Required'),
          })
        );
        if (tok === CANCEL) return CANCEL;
        S.telegramToken = tok.trim();
        const cid = await ask(() =>
          text({
            message: `${pc.cyan('3/7')}  Allowed IDs (Telegram numeric or WhatsApp @jid, comma-separated)`,
            placeholder: '7733493073,number@s.whatsapp.net',
            initialValue: S.allowedIds,
          })
        );
        if (cid === CANCEL) return CANCEL;
        S.allowedIds = cid.trim();
      }
      return true;
    },
    ...
    async i => {
      ...
      const enWa = await ask(() =>
        confirm({
          message: `${pc.cyan('4/7')}  Enable WhatsApp? (needs @whiskeysockets/baileys)`,
          initialValue: S.whatsappEnabled,
        })
      );
      if (enWa === CANCEL) return CANCEL;
      S.whatsappEnabled = enWa;
      if (enWa) {
        const dir = await ask(() =>
          text({
            message: `${pc.cyan('4/7')}  Baileys auth directory`,
            placeholder: './auth_info_baileys',
            initialValue: S.whatsappAuthDir,
          })
        );
        if (dir === CANCEL) return CANCEL;
        S.whatsappAuthDir = dir.trim() || './auth_info_baileys';
        const dbg = await ask(() =>
          confirm({
            message: `${pc.cyan('4/7')}  Enable WhatsApp verbose debug?`,
            initialValue: S.whatsappDebug,
          })
        );
        if (dbg === CANCEL) return CANCEL;
        S.whatsappDebug = dbg;
        ...
        if (!S.telegramEnabled) {
          const cid = await ask(() =>
            text({
              message: `${pc.cyan(
                '4/7'
              )}  Allowed IDs (WhatsApp @jid or Telegram numeric, comma-separated)`,
              placeholder: 'number@s.whatsapp.net,7733493073',
              initialValue: S.allowedIds,
            })
          );
          if (cid === CANCEL) return CANCEL;
        ...
        note(
          'WhatsApp shows a QR code on first start.\nScan with WhatsApp mobile to link the session.',
          pc.yellow('WhatsApp Pairing')
        );
      }

      const enSl = await ask(() =>
        confirm({
          message: `${pc.cyan('4/7')}  Enable Slack integration?`,
          initialValue: S.slackEnabled,
        })
      );
      if (enSl === CANCEL) return CANCEL;
      S.slackEnabled = enSl;
      if (enSl) {
        const tok = await ask(() =>
          text({
            message: `${pc.cyan('4/7')}  Slack Bot Token (xoxb-…)`,
            placeholder: 'xoxb-xxx',
            initialValue: S.slackBotToken,
            validate: v => (v.trim().startsWith('xoxb-') ? undefined : 'Must start with xoxb-'),
          })
        );
        if (tok === CANCEL) return CANCEL;
        S.slackBotToken = tok.trim();
        const ch = await ask(() =>
          text({
            message: `${pc.cyan('4/7')}  Default Slack channel ID`,
            placeholder: 'C0123456789',
            initialValue: S.slackChannel,
          })
        );
        if (ch === CANCEL) return CANCEL;
        S.slackChannel = ch.trim();
      }
      return true;
    },
    ...
    async i => {
      ...
      const en = await ask(() =>
        confirm({
          message: `${pc.cyan('5/8')}  Enable Discord integration?`,
          initialValue: S.discordEnabled,
        })
      );
      if (en === CANCEL) return CANCEL;
      S.discordEnabled = en;
      if (en) {
        const tok = await ask(() =>
          text({
            message: `${pc.cyan('5/8')}  Discord Bot Token`,
            placeholder: 'MTxxx.xxx.xxx',
            initialValue: S.discordBotToken,
            validate: v => (v.trim() ? undefined : 'Required'),
          })
        );
        if (tok === CANCEL) return CANCEL;
        S.discordBotToken = tok.trim();
        const chId = await ask(() =>
          text({
            message: `${pc.cyan('5/8')}  Discord Channel ID`,
            placeholder: '123456789012345678',
            initialValue: S.discordChannelId,
          })
        );
        if (chId === CANCEL) return CANCEL;
        S.discordChannelId = chId.trim();
      }
      return true;
    },
    ...
    async i => {
      ...
      // ── SMS ────────────────────────────────────────────────────────
      const enSms = await ask(() =>
        confirm({ message: `${pc.cyan('6/8')}  Enable SMS channel?`, initialValue: S.smsEnabled })
      );
      if (enSms === CANCEL) return CANCEL;
      S.smsEnabled = enSms;
      ...
      if (enSms) {
        ...
        if (prov === CANCEL) return CANCEL;
        S.smsProvider = prov;

        if (prov === 'twilio') {
          const sid = await ask(() =>
            text({
              message: `${pc.cyan('6/8')}  Twilio Account SID`,
              placeholder: 'ACxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',
              initialValue: S.twilioSid,
              validate: v => (v.trim() ? undefined : 'Required'),
            })
          );
          if (sid === CANCEL) return CANCEL;
          S.twilioSid = sid.trim();
          const tok = await ask(() =>
            clackPwd({
              message: `${pc.cyan('6/8')}  Twilio Auth Token`,
          ...
          const frm = await ask(() =>
            text({
              message: `${pc.cyan('6/8')}  Twilio From Number (E.164)`,
              placeholder: '+12025551234',
              initialValue: S.twilioFrom,
              validate: v => (v.trim().startsWith('+') ? undefined : 'Must be E.164 (+…)'),
            })
          );
          if (frm === CANCEL) return CANCEL;
          S.twilioFrom = frm.trim();
        }

        if (prov === 'econet') {
          const url = await ask(() =>
            text({
              message: `${pc.cyan('6/8')}  Econet Base URL`,
              placeholder: 'https://api.econet.co.zw',
              initialValue: S.econetBaseUrl,
            })
          );
          if (url === CANCEL) return CANCEL;
          S.econetBaseUrl = url.trim() || 'https://api.econet.co.zw';
          const cid = await ask(() =>
            text({
              message: `${pc.cyan('6/8')}  Econet Client ID`,
              placeholder: 'your-client-id',
              initialValue: S.econetClientId,
              validate: v => (v.trim() ? undefined : 'Required'),
            })
          );
          if (cid === CANCEL) return CANCEL;
          S.econetClientId = cid.trim();
          const csec = await ask(() =>
            clackPwd({
              message: `${pc.cyan('6/8')}  Econet Client Secret`,
              validate: v => (v.length >= 4 ? undefined : 'Too short'),
            })
          );
          if (csec === CANCEL) return CANCEL;
          S.econetClientSecret = csec;
          const fn = await ask(() =>
            text({
              message: `${pc.cyan('6/8')}  Econet From Name (sender ID)`,
              placeholder: 'AgentOS',
              initialValue: S.econetFromName,
            })
          );
          if (fn === CANCEL) return CANCEL;
          S.econetFromName = fn.trim() || 'AgentOS';
        }
      }

      // ── USSD (AfricasTalking) ────────────────────────────────────
      const enUssd = await ask(() =>
        confirm({
          message: `${pc.cyan('6/8')}  Enable USSD channel? (AfricasTalking)`,
          initialValue: S.ussdEnabled,
        })
      );
      if (enUssd === CANCEL) return CANCEL;
      S.ussdEnabled = enUssd;

      if (enUssd) {
        const atUser = await ask(() =>
          text({
            message: `${pc.cyan('6/8')}  AfricasTalking Username`,
            placeholder: 'sandbox',
            initialValue: S.atUsername,
            validate: v => (v.trim() ? undefined : 'Required'),
          })
        );
        if (atUser === CANCEL) return CANCEL;
        S.atUsername = atUser.trim();
        const atKey = await ask(() =>
          clackPwd({
            message: `${pc.cyan('6/8')}  AfricasTalking API Key`,
            validate: v => (v.length >= 4 ? undefined : 'Too short'),
          })
        );
        if (atKey === CANCEL) return CANCEL;
        S.atApiKey = atKey;
        const sc = await ask(() =>
          text({
            message: `${pc.cyan('6/8')}  USSD Service Code`,
            placeholder: '*123#',
            initialValue: S.ussdServiceCode,
          })
        );
        if (sc === CANCEL) return CANCEL;
        S.ussdServiceCode = sc.trim();
      }

      // ── Email (SMTP) ─────────────────────────────────────────────
      const enEmail = await ask(() =>
        confirm({
          message: `${pc.cyan('6/8')}  Enable Email channel? (SMTP)`,
          initialValue: S.emailEnabled,
        })
      );
      if (enEmail === CANCEL) return CANCEL;
      S.emailEnabled = enEmail;

      if (enEmail) {
        const host = await ask(() =>
          text({
            message: `${pc.cyan('6/8')}  SMTP Host`,
            placeholder: 'smtp.gmail.com',
            initialValue: S.smtpHost,
            validate: v => (v.trim() ? undefined : 'Required'),
          })
        );
        if (host === CANCEL) return CANCEL;
        S.smtpHost = host.trim();
        const port = await ask(() =>
          text({
            message: `${pc.cyan('6/8')}  SMTP Port`,
            placeholder: '587',
            initialValue: S.smtpPort,
          })
        );
        if (port === CANCEL) return CANCEL;
        S.smtpPort = port.trim() || '587';
        const user = await ask(() =>
          text({
            message: `${pc.cyan('6/8')}  SMTP Username`,
            placeholder: 'you@br3eze.africa',
            initialValue: S.smtpUser,
            validate: v => (v.trim() ? undefined : 'Required'),
          })
        );
        if (user === CANCEL) return CANCEL;
        S.smtpUser = user.trim();
        const pass = await ask(() =>
          clackPwd({
            message: `${pc.cyan('6/8')}  SMTP Password`,
            validate: v => (v.length >= 1 ? undefined : 'Required'),
          })
        );
        ...
        const from = await ask(() =>
          text({
            message: `${pc.cyan('6/8')}  From Address`,
            placeholder: 'agentos@br3eze.africa',
            initialValue: S.smtpFrom,
          })
        );
        if (from === CANCEL) return CANCEL;
        S.smtpFrom = from.trim();
      }
    ...
    async i => {
      ...
      if (lvl === CANCEL) return CANCEL;
      S.logLevel = lvl;
      const dbg = await ask(() =>
        confirm({
          message: `${pc.cyan('7/8')}  Enable skills debug? (DEBUG=agentos:*)`,
          initialValue: S.skillsDebug,
        })
      );
      if (dbg === CANCEL) return CANCEL;
      S.skillsDebug = dbg;
      const channels = [
        ...
        S.whatsappEnabled && 'whatsapp',
        S.slackEnabled && 'slack',
        S.discordEnabled && 'discord',
        S.smsEnabled && 'sms',
        S.ussdEnabled && 'ussd',
    ...
    async i => {
      ...
      const dry = await ask(() =>
        confirm({
          message: `${pc.cyan('8/8')}  Dry run? (scripts uploaded, not executed)`,
          initialValue: S.dryRun,
        })
      );
      if (dry === CANCEL) return CANCEL;
      S.dryRun = dry;
      note(
        [
  ...
  const envLines = [
    ...
    (S.telegramEnabled || S.whatsappEnabled) && S.allowedIds
      ? `ALLOWED_CHAT_IDS=${S.allowedIds}`
      : '',
    ...
    // Twilio
    S.smsEnabled && S.smsProvider === 'twilio' && S.twilioSid
      ? `TWILIO_ACCOUNT_SID=${S.twilioSid}`
      : '',
    S.smsEnabled && S.smsProvider === 'twilio' && S.twilioToken
      ? `TWILIO_AUTH_TOKEN=${S.twilioToken}`
      : '',
    S.smsEnabled && S.smsProvider === 'twilio' && S.twilioFrom
      ? `TWILIO_FROM_NUMBER=${S.twilioFrom}`
      : '',
    // Econet A2A
    S.smsEnabled && S.smsProvider === 'econet' && S.econetClientId
      ? `ECONET_BASE_URL=${S.econetBaseUrl}`
      : '',
    S.smsEnabled && S.smsProvider === 'econet' && S.econetClientId
      ? `ECONET_CLIENT_ID=${S.econetClientId}`
      : '',
    S.smsEnabled && S.smsProvider === 'econet' && S.econetClientSecret
      ? `ECONET_CLIENT_SECRET=${S.econetClientSecret}`
      : '',
  ...
  try {
    ...
    note(
      `Saved to ${pc.cyan('.env.agentos')}\nMerge with:\n${pc.dim('cat .env.agentos >> .env')}`,
      'Environment written'
    );
  } catch (e) {
  ...
  try {
    ...
    s.stop(
      result.success ? pc.green('✔  Onboarding complete!') : pc.red(`✘  Failed: ${result.error}`)
    );
