    ...
    steps: [
      {
        ...
        description: 'Verifying RouterOS API connectivity and credentials.',
      },
      {
        id: 'interfaces',
        label: 'Configuring interfaces',
        description: 'Setting up WAN, LAN, and bridge interfaces.',
      },
      {
        id: 'dhcp',
        label: 'Setting up DHCP',
        description: 'Provisioning DHCP server and IP pools for hotspot clients.',
      },
      {
        id: 'hotspot',
        label: 'Enabling hotspot',
        description: 'Activating hotspot server with captive portal.',
      },
      {
        id: 'profiles',
        label: 'Creating billing profiles',
        description: `Provisioning ${opts.plans ?? 3} bandwidth/time plans.`,
      },
      {
        id: 'firewall',
        label: 'Applying firewall rules',
        description: 'Writing NAT masquerade and forward chain rules.',
      },
      {
        id: 'dns',
        label: 'Configuring DNS',
        description: 'Setting upstream DNS servers and local overrides.',
      },
      {
        id: 'scheduler',
        label: 'Installing scheduler',
        description: 'Setting up RouterOS scheduler for voucher expiry checks.',
      },
      {
        id: 'telegram',
        label: 'Wiring Telegram bot',
        description: 'Configuring RouterOS → AgentOS webhook bindings.',
      },
      {
        id: 'verify',
        label: 'Verifying deployment',
        description: 'Running smoke tests on all configured subsystems.',
      },
    ],
  };
}
...
function buildVoucherBatchTemplate(count) {
  return {
    ...
    steps: [
      {
        id: 'generate',
        label: 'Generating codes',
        description: `Creating ${count} cryptographically random voucher codes.`,
      },
      {
        id: 'register',
        label: 'Registering to DB',
        description: 'Persisting voucher records to Firebase / local store.',
      },
      {
        id: 'mikrotik',
        label: 'Adding to RouterOS',
        description: 'Provisioning hotspot user accounts on the router.',
      },
      {
        id: 'output',
        label: 'Preparing output',
        description: 'Generating QR codes and formatted voucher list.',
      },
    ],
  };
}
...
class OperationProgressTracker extends EventEmitter {
  ...
  get isFinished() {
    return this._snapshot.steps.every(
      s => s.status === StepStatus.DONE || s.status === StepStatus.ERROR
    );
  }
