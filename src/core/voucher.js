...
class VoucherAgent {
  ...
  generate(plan = 'default') {
    ...
    const codePart = format.replace(/X+/g, m => genPart(m.length));
  ...
  async createVoucher(plan = 'default') {
    ...
    if (global.mikrotik && global.mikrotik.isConnected) {
      ...
      console.warn(
        '[Voucher] MikroTik not connected — voucher generated but not provisioned to router.'
      );
    }
