...
class AgentOSBot {
  ...
  checkAuth(msg) {
    ...
    if (!this.rateLimiter.check(chatId)) {
      this.bot
        .sendMessage(chatId, '⏳ *Rate limit:* Too many commands.', { parse_mode: 'Markdown' })
        .catch(() => {});
      return false;
    }
    ...
    if (!this.config.allowedChats?.length) {
      this.bot.sendMessage(
        chatId,
        '⚠️ *Setup Mode*\nNo admins configured. Use `/claim` to become admin.',
        { parse_mode: 'Markdown' }
      );
      return false;
    }
  ...
  async handleStart(msg) {
    ...
    const keyboard = {
      inline_keyboard: [
        ...
        [
          { text: '🤖 Run Agent', callback_data: 'action:run' },
  ...
  async handleDashboard(msg) {
    ...
    try {
      ...
      await this.bot.sendMessage(msg.chat.id, text, {
        parse_mode: 'Markdown',
        reply_markup: {
          inline_keyboard: [[{ text: '🔄 Refresh', callback_data: 'action:dashboard' }]],
        },
      });
    } catch (error) {
  ...
  async handleUsers(msg) {
    ...
    const keyboard = {
      inline_keyboard: [
        ...
        [
          { text: '🔕 Disable User', callback_data: 'users:disable' },
          { text: '🗑 Remove User', callback_data: 'users:remove' },
        ],
  ...
  async handleWallet(msg) {
    ...
    try {
      ...
      this.bot.sendMessage(msg.chat.id, `❌ Wallet error: ${e.message}`);
    }
  }
  ...
  async handleTasks(msg) {
    ...
    if (recent.length === 0) {
      ...
      for (const t of recent) {
        ...
        lines.push(
          `${emoji} \`${t.taskId.slice(0, 8)}\` — ${t.description || t.prompt.slice(0, 40)}`
        );
      }
    }
  ...
  async handleSession(msg) {
    ...
    if (!engine) {
      return this.bot.sendMessage(msg.chat.id, '_No active session. Use /run to start one._', {
        parse_mode: 'Markdown',
      });
    }
    await this.bot.sendMessage(
  ...
  async handleMode(msg, match) {
    ...
    await this.bot.sendMessage(msg.chat.id, `${emoji} Permission mode set to \`${requested}\``, {
      parse_mode: 'Markdown',
    });
  }
  ...
  async handleCallback(query) {
    ...
    try {
      ...
      switch (category) {
        case 'action':
          await this.handleActionButton(chatId, action, query);
          break;
        case 'voucher':
          await this.handleVoucherButton(chatId, action);
          break;
        case 'wallet':
          await this.handleWalletButton(chatId, action);
          break;
        case 'tool':
        ...
        case 'confirm':
          ...
          break;
        default:
          logger.warn(`Unknown callback category: ${category}`, { data, chatId });
      }
    } catch (error) {
  ...
  async handleNetworkButton(chatId, action) {
    if (action === 'reboot') {
      ...
      await this.bot.sendMessage(
        chatId,
        `🚧 ${action} — use /run ${action} for natural language dispatch`
      );
    }
  }
  ...
  async handleUsersButton(chatId, action) {
    try {
      ...
      if (action === 'active') {
        ...
        await this.bot.sendMessage(
          chatId,
          `🔕 Enter username to *disable* (blocks login + kicks session):`,
          {
            reply_markup: { force_reply: true },
            parse_mode: 'Markdown',
          }
        ...
        await this.bot.sendMessage(
          chatId,
          `➕ Enter: \`username password profile\` (e.g. \`john pass1 1Day\`):`,
          {
            reply_markup: { force_reply: true },
            parse_mode: 'Markdown',
          }
        ...
        await this.bot.sendMessage(
          chatId,
          `Use /run list ${action} users for natural language dispatch`
        );
      }
    } catch (error) {
      await this.bot.sendMessage(chatId, `❌ Error: ${error.message}`);
  ...
  async handleConfirmation(chatId, action, query) {
    ...
    if (action === 'reboot') {
      ...
      try {
        await this.bot.editMessageText(`🗑 Removing *${username}*...`, {
          ...editBase,
          parse_mode: 'Markdown',
        });
  ...
  async _handlePendingInput(chatId, input, pending) {
    ...
    switch (action) {
      case 'setup_step1':
        ...
        await this.bot.sendMessage(
          chatId,
          `🌐 IP set to \`${input}\`\n\nEnter RouterOS username (default: admin):`,
          { reply_markup: { force_reply: true }, parse_mode: 'Markdown' }
        );
        break;
      case 'setup_step2':
  ...
  async _finalizeSetup(chatId, ip, user, pass) {
    await this.bot.sendMessage(chatId, `⏳ Testing connection to \`${ip}\`...`, {
      parse_mode: 'Markdown',
    });
    try {
      const mikrotik = getMikroTikClient();
    ...
    try {
      ...
      switch (action) {
        ...
        case 'kick': {
          ...
          this.bot.sendMessage(
            chatId,
            res.kicked ? `🚫 *${input}* kicked.` : `⚠️ *${input}* not active.`,
            { parse_mode: 'Markdown' }
          );
          break;
        }
  ...
  async handleVerify(msg, match) {
    ...
    if (!identifier)
      return this.bot.sendMessage(msg.chat.id, '❌ Usage: /verify <email|phone|username|uid>');
          ...
          if (admin && admin.auth) {
            try {
              ...
              if (
                user.email &&
                authRec.email &&
                user.email.toLowerCase() !== authRec.email.toLowerCase()
              ) {
                warnings.push(
                  `⚠️ *Email Mismatch:* Firestore has \`${user.email}\` but Auth has \`${authRec.email}\``
                );
              }
              ...
              if (e.code === 'auth/user-not-found') {
                warnings.push(
                  `⚠️ *Auth Missing:* User has UID \`${user.uid}\` in Firestore but is missing from Firebase Auth.`
                );
              } else {
