...
const TerminalAnimator = {
  ...
  box(title, content, color = A.PRIMARY) {
    ...
    lines.forEach(line => {
      ...
      console.log(
        `${color}│${A.RESET}  ${line}${' '.repeat(Math.max(0, width - l - 2))}${color}│${A.RESET}`
      );
    });
    console.log(bottom);
  },
  ...
  async animateProgressBar(label, durationMs = 2000, width = 30) {
    ...
    while (Date.now() - start < durationMs) {
      ...
      process.stdout.write(
        `\r  ${A.BOLD}${label}${A.RESET} [${bar}] ${A.CYAN}${percent}%${A.RESET}`
      );
      await sleep(50);
    }
    process.stdout.write(
      `\r  ${A.BOLD}${label}${A.RESET} [${A.SUCCESS}${'█'.repeat(width)}${A.RESET}] ${A.CYAN}100%${A.RESET}\n`
    );
    process.stdout.write('\x1b[?25h'); // Show cursor
  },
