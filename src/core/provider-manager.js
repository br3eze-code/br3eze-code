...
class ProviderManager {
  ...
  normalizeResponse(raw) {
    ...
    if (raw.candidates) return { content: raw.candidates[0]?.content?.parts?.[0]?.text || '', toolCalls: this.extractGeminiToolCalls(raw), provider: 'gemini' };
    if (raw.content) return { content: Array.isArray(raw.content) ? (raw.content[0]?.text || '') : raw.content, toolCalls: this.extractClaudeToolCalls(raw), provider: 'claude' };
    if (raw.choices) {
      const message = raw.choices[0]?.message;
      return { content: message?.content || '', toolCalls: (message?.tool_calls || []).map((toolCall) => ({ id: toolCall.id, name: toolCall.function?.name, arguments: this.parseArguments(toolCall.function?.arguments) })), provider: 'openai' };
    }
    return raw;
  }
  ...
  extractGeminiToolCalls(raw) {
    return (raw.candidates?.[0]?.content?.parts || []).filter((part) => part.functionCall).map((part) => ({ id: `gemini-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`, name: part.functionCall.name, arguments: part.functionCall.args || {} }));
  }

  extractClaudeToolCalls(raw) {
    return (Array.isArray(raw.content) ? raw.content : []).filter((content) => content.type === 'tool_use').map((toolUse) => ({ id: toolUse.id, name: toolUse.name, arguments: toolUse.input || {} }));
  }
