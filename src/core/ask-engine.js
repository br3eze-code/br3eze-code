...
const FUNCTION_DECLARATIONS = [
  ...
  },
  {
    ...
    description:
      'Perform write operations on a MikroTik hotspot user: disable, enable, or permanently remove them.',
    parameters: {
      type: 'object',
      properties: {
        action: {
          ...
          description:
            'disable — blocks login immediately (sets disabled=yes and kicks session); enable — restores access; remove — permanently deletes the user from the router',
        },
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        action: {
          type: 'string',
          enum: ['get_balance', 'find_user', 'list_hotspot_profiles', 'list_active'],
        },
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        target: {
          type: 'string',
          description: 'Payment ID, reference, or recipient username for transfer',
        },
...
class AskEngine {
  ...
  _matchRule(input) {
    ...
    const enableMatch = lower.match(
      /(?:enable|unblock|restore|reactivate)\s+(?:user\s+)?([\w@.-]+)/
    );
  ...
  async _dispatchFunctionCall({ name, args }) {
    ...
    if (name === 'manage_vouchers' && this.database) {
      if (action === 'create')
        return this.database.createVoucher(voucherCode(), { value: amount || 0 });
    ...
    if (name === 'manage_users' && this.database) {
      ...
      if (action === 'list_hotspot_profiles' && this.database)
        return this.database.getHotspotUsers();
    ...
    if (name === 'manage_finance') {
      if (action === 'revenue_report' && this.database)
        return this.database.getRevenue(args.period || 'daily');
      ...
      if (action === 'payment_link' && this.billing)
        return this.billing.createPaymentLink({ plan, amount });
      if (action === 'transfer' && this.database)
        return this.database.p2pTransfer(args.from || 'system', target, args.amount);
    }
