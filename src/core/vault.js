...
class EncryptionVault {
  constructor(masterKey) {
    ...
    const key = (masterKey || process.env.VAULT_MASTER_KEY || 'changeme-agentos-omni-2026').padEnd(
      16,
      '!'
    );
...
class OAuthVault {
  ...
  async getAccessToken(provider, userId) {
    ...
    const tokens = {
      ...
      refreshToken: record.encrypted.refreshToken
        ? this.vault.decrypt(record.encrypted.refreshToken)
        : null,
