...
function checkPolicyForTool(policy, toolName) {
  ...
  if (policy.networkAccess === NetworkAccess.RESTRICTED) {
    ...
    if (networkDestructive.has(toolName)) {
      return {
        allowed: false,
        reason: `networkAccess=restricted: tool '${toolName}' requires enabled network access`,
      };
    }
  }
  ...
  if (policy.installScope === InstallScope.NONE) {
    ...
    if (mutations.has(toolName)) {
      return {
        allowed: false,
        reason: `installScope=none: tool '${toolName}' would mutate router config`,
      };
    }
  }
  ...
  if (policy.fileAccess === FileAccess.WORKSPACE_ONLY) {
    ...
    if (extendedTools.has(toolName)) {
      return {
        allowed: false,
        reason: `fileAccess=workspace-only: tool '${toolName}' requires extended router access`,
      };
    }
  }
...
function resolveHeartbeat(preset, heartbeatConfig = null) {
  ...
  return {
    enabled:
      typeof heartbeatConfig.enabled === 'boolean' ? heartbeatConfig.enabled : defaults.enabled,
