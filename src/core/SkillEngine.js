...
class SkillEngine {
  ...
  register(nameOrManifest, skill) {
    ...
    if (missing.length) {
      throw new TypeError(
        `SkillEngine.register('${manifest.name}'): manifest missing: ${missing.join(', ')}`
      );
    }
    if (typeof (executor.execute || executor) !== 'function') {
      throw new TypeError(
        `SkillEngine.register('${manifest.name}'): executor must be a function or have execute()`
      );
    }
  ...
  list() {
    return Array.from(this.skills.values()).map(s => s.manifest || { name: s.name });
  }
