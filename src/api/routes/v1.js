...
router.post('/users/sync', async (req, res) => {
  try {
    ...
    if (!user || !planId) return res.status(400).json({ ok: false, error: 'user and planId required' });
  ...
  try {
    ...
    if (global.database) {
      await global.database.createVoucher(code, {
        planId: plan, planName: planObj.name || plan,
        durationUnit: planObj.durationUnit || null, durationValue: planObj.durationValue || null,
        deviceLimit: planObj.deviceLimit || 1, expiresAt, loginUrl, amount, method,
        status: 'paid', createdBy: 'api-pay-v1',
      });
      if (global.mikrotik?.state?.isConnected) {
...
router.patch('/hotspot/profiles/:name', async (req, res) => {
  try {
    const mt = getManager();
    const result = await mt.executeTool('hotspot.profile.update', { name: req.params.name, ...req.body });
    ok(res, result);
  } catch (e) { err(res, e); }
});
