...
/** API Routes — Shop. Mounted at /api/v1/shop by gateway-engine.js. */
const router = express.Router();
const ok = (res, data) => res.json({ ok: true, data });
const fail = (res, e, status = 400) => res.status(status).json({ ok: false, error: e.message });
const requestOrderScope = (req) => { const user = req.firebaseUser || {}; const claims = user.customClaims || {}; return { tenantId: user.tenantId || claims.tenantId || null, siteId: user.siteId || claims.siteId || null, domain: user.domain || user.domainId || claims.domain || claims.domainId || null }; };
const canViewOrder = (req, order) => { const isOwner = req.firebaseUser?.uid && order.userId === req.firebaseUser.uid; const role = req.firebaseUser?.role || req.firebaseUser?.customClaims?.role; return isOwner || role === 'admin'; };
const requestProductScope = (req) => { const user = req.firebaseUser || {}; const claims = user.customClaims || {}; return { userId: user.uid || null, tenantId: user.tenantId || claims.tenantId || null, siteId: user.siteId || claims.siteId || null, domain: user.domain || user.domainId || claims.domain || claims.domainId || null, role: user.role || claims.role || 'user', tier: user.influenceTier || claims.influenceTier || 'standard' }; };
...
router.post('/checkout', async (req, res) => {
  try {
    const context = resolveCommerceContext(req); const { address, payMethod } = req.body || {}; const idempotencyKey = req.get('Idempotency-Key') || req.body?.idempotencyKey; requireIdempotencyKey(idempotencyKey);
    ...
    logger.info(`[Shop API] checkout ${context.platform}:${context.channelId} uid=${context.uid} order=${result.orderId}${result.replayed ? ' replay=true' : ''}`); ok(res, result);
