...
class AgentRuntime extends EventEmitter {
  ...
  routePrompt(prompt, limit = 5) {
    ...
    const scored = TOOL_MANIFEST.map(entry => ({
      name: entry.name,
      score: scorePrompt(tokens, entry),
    }))
      .filter(m => m.score > 0)
  ...
  async bootstrapSession(prompt, { sessionId = null, permissionMode = null } = {}) {
    const engine = sessionId
      ...
      : AgentEngine.create({
          ...this.defaultConfig,
          permissionMode: permissionMode || this.defaultConfig.permissionMode,
        });
    ...
    logger.info(
      `AgentRuntime bootstrap — tools: [${matchedTools.join(', ')}] denials: ${denials.length}`
    );
  ...
  async _executeTask(taskId, prompt, opts) {
    ...
    registry.setStatus(
      taskId,
      last?.stopReason === 'completed' ? TaskStatus.COMPLETED : TaskStatus.FAILED
    );
  }
  ...
  findTools(query) {
    ...
    return TOOL_MANIFEST.filter(
      t => t.name.includes(needle) || t.keywords.some(k => k.includes(needle))
    ).map(t => t.name);
  }
  async destroy() {
