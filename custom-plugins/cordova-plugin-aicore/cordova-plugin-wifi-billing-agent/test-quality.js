...
function testLogDirectory() {
    ...
    const content = fs.readFileSync(path.join(__dirname, 'www/agents/hotspotAgent.js'), 'utf8');
    assert.ok(content.includes("path.join(__dirname, 'logs')"), 'Logs directory resolution is NOT absolute!');
...
function testAIFormatting() {
    ...
    const content = fs.readFileSync(path.join(__dirname, 'www/agents/hotspotAgent.js'), 'utf8');
    assert.ok(content.includes("const strOpts = String(s);"), 'AI formatting lacks text type casting safe guards');
    assert.ok(content.includes("const { action, plan, target } = args || {};"), 'AI function dispatch lacks empty arg object fallbacks');
...
function testTelegramCallbackAuth() {
    ...
    const content = fs.readFileSync(path.join(__dirname, 'www/agents/hotspotAgent.js'), 'utf8');
    assert.ok(content.includes("if (!this._checkAuth(fakeMsg)) return;"), 'Callback queries lack strict inline authorization guards');
