...
import { normalizePaymentRequest, createIdempotencyKey, sanitizeMetadata } from '../src/payments/payment-guards.js';
...
describe('payment guards', () => {
  test('normalizes and validates server-facing payment input', () => {
    ...
    expect(request.amountMinor).toBe(1050); expect(request.currency).toBe('USD');
    expect(request.metadata).toEqual({ apiKey: '[redacted]', plan: 'basic' });
    expect(() => normalizePaymentRequest({ amount: 0, reference: 'x' })).toThrow();
  });

  test('deduplicates provider create requests by reference at the platform boundary', async () => {
    const testState = fs.mkdtempSync(path.join(os.tmpdir(), 'agentos-payment-test-'));
    const platform = createPaymentPlatform({ merchantCountry: 'ZW', idempotencyOptions: { dbPath: path.join(testState, 'payment-ledger.sqlite') } });
    ...
    expect(first).toEqual(second); expect(provider.createPayment).toHaveBeenCalledTimes(1);
  });

  test('requires webhook verification and a refund reason at the canonical boundary', async () => {
    ...
    provider.verifyWebhook = jest.fn().mockResolvedValue(false); provider.processWebhook = jest.fn(); provider.refundPayment = jest.fn();
    platform.registry.register(provider);
    await expect(platform.webhook('test', {}, {})).rejects.toThrow(/verification failed/);
    await expect(platform.refund('test', 'tx-1', undefined, '')).rejects.toThrow();
  });
...
describe('identity linking', () => {
  test('prevents cross-account identity takeover and expires tokens', async () => {
    let now = 1000; const service = new IdentityLinkingService({ now: () => now, ttlMs: 100 });
    ...
    const issued = await service.issueLinkToken({ userId: 'u1', identity: { provider: 'github', subject: 'octo' } }); now = 1200;
