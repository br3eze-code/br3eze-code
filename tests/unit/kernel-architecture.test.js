...
test('kernel dispatches through an injected session store', async () => {
  ...
  kernel.registerDomain('alpha', { name: 'alpha', getCapabilities: () => ['alpha'], execute: async () => ({ ok: true }) });
  kernel.init();
...
test('ambiguous dispatch fails with a stable error code', async () => {
  ...
  kernel.init();
  await assert.rejects(kernel.dispatch({}, { intent: { text: 'unknown intent' } }), (error) => error.code === 'DOMAIN_RESOLUTION_AMBIGUOUS');
});
