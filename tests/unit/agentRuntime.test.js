...
jest.mock('../../src/core/agentEngine', () => ({
  AgentEngine: class {
    static create() {
      return {
        sessionId: 'mock',
        submitMessage: jest.fn().mockResolvedValue({ stopReason: 'completed', output: 'ok' }),
        persistSession: jest.fn().mockReturnValue('/tmp/session'),
        renderSummary: jest.fn().mockReturnValue('summary'),
        enforcer: { check: jest.fn().mockReturnValue({ allowed: true }) },
      };
    }
    static fromSession(id) {
      return this.create();
    }
  },
}));
