...
jest.unstable_mockModule('../../../src/payments/providers/pesapay-provider.js', () => {
  const MockPesaPalProvider = jest.fn().mockImplementation(function (config) {
    this.config = config;
    this.getRegisteredIPNs = jest.fn();
    this.registerIPN = jest.fn();
    this.createPayment = jest.fn();
    this.verifyPayment = jest.fn();
    this.refund = jest.fn();
  });
  return { default: MockPesaPalProvider };
});

const { default: PesaPalProvider } = await import('../../../src/payments/providers/pesapay-provider.js');
const { default: PesaPalIntegration } = await import('../../../src/payments/pesapay-integration.js');
