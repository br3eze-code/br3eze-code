...
describe('MikroTikManager Logic', () => {
  ...
  describe('addHotspotUser', () => {
    ...
    test('falls back to default profile if requested profile is missing', async () => {
      ...
      expect(mockMenu.add).toHaveBeenNthCalledWith(
        2,
  ...
  describe('executeTool', () => {
    ...
    test('throws error for unknown tool', async () => {
      await expect(manager.executeTool('nonexistent.tool')).rejects.toThrow(ToolExecutionError);
    });
    ...
    test('circuit breaker opens after failures', async () => {
      ...
      await expect(manager.executeTool('system.stats')).rejects.toThrow('Circuit breaker is OPEN');
    });
  });
});
