...
jestTest('Phase 9 terminal boundary module loads', () => {
    expect(typeof renderMarkdown).toBe('function');
});

nodeTest('Phase 9 markdown renderer handles headings, emphasis, code and lists', () => {
...
nodeTest('Phase 9 code renderer preserves source exactly', () => {
    assert.equal(renderCode('const x = 1;\n', { language: 'js', color: false }), 'const x = 1;\n');
});

nodeTest('Phase 9 diff renderer preserves diff content', () => {
    assert.equal(renderDiff('+added\n-removed', { color: false }), '+added\n-removed');
});

nodeTest('Phase 9 navigation stays in the UI boundary', () => {
...
nodeTest('Phase 9 renderer writes agent results without orchestration', () => {
