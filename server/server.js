...
const CONFIG = {
  ...
  VOUCHER_PREFIX: 'STAR-',
  VOUCHER_PLANS: {
    '1hour': { duration: 60 * 60 * 1000, price: 1.0 },
    '1Day': { duration: 24 * 60 * 60 * 1000, price: 5.0 },
    '7Day': { duration: 7 * 24 * 60 * 60 * 1000, price: 25.0 },
    '30Day': { duration: 30 * 24 * 60 * 60 * 1000, price: 80.0 },
  },
  RATE_LIMIT: {
    WINDOW: 15 * 60 * 1000,
    MAX: 100,
...
class DatabaseService {
  ...
  createVoucher(code, plan, createdBy = 'system') {
    const expiresAt = new Date(
      Date.now() + CONFIG.VOUCHER_PLANS[plan]?.duration || 0
    ).toISOString();
  ...
  getVoucherStats() {
    ...
    const used = this.db
      .prepare('SELECT COUNT(*) as count FROM vouchers WHERE used = 1')
      .get().count;
    const active = this.db
      .prepare(
        'SELECT COUNT(*) as count FROM vouchers WHERE used = 0 AND expires_at > datetime("now")'
      )
      .get().count;
...
const TOOL_METADATA = {
  ...
  'voucher.list': {
    ...
    category: 'voucher',
    parameters: [{ name: 'limit', type: 'number', required: false, default: 50 }],
  ...
  'router.discover': {
    ...
    category: 'discovery',
    parameters: [{ name: 'subnet', type: 'string', required: false, default: '192.168.88.0/24' }],
  ...
  'service.list': {
    ...
    category: 'discovery',
    parameters: [{ name: 'category', type: 'string', required: false }],
...
class NetworkDiscoveryService {
  ...
  httpCheck(ip, port, timeout = 2000) {
    return new Promise(resolve => {
      ...
      const req = httpModule.request(options, res => {
        ...
        res.on('end', () => {
          const isMikrotik =
            data.includes('mikrotik') ||
            res.headers['server']?.toLowerCase().includes('mikrotik') ||
            data.includes('RouterOS');

          const hostname = res.headers['x-hostname'] || (isMikrotik ? ip : null);
  ...
  extractModel(html) {
    const match =
      html.match(/board_name["\s:=]+([^"<\s]+)/i) || html.match(/model["\s:=]+([^"<\s]+)/i);
    return match ? match[1] : null;
  }

  extractVersion(html) {
    const match = html.match(/version["\s:=]+([^"<\s]+)/i) || html.match(/ROS["\s]+([\d.]+)/i);
    return match ? match[1] : null;
  }
    ...
    return new Promise(resolve => {
      ...
      socket.on('connect', () => {
        ...
        resolve({
          ...
          alive: true,
          responseTime,
          ports: [80, 443, 22, 3389].filter(port => this.checkPort(ip, port, timeout)),
          foundAt: Date.now(),
        });
      });
...
const TOOLS = {
  ...
  'voucher.create': async (plan, createdBy = 'api') => {
    if (!CONFIG.VOUCHER_PLANS[plan]) {
      throw new Error(
        `Invalid plan: ${plan}. Available: ${Object.keys(CONFIG.VOUCHER_PLANS).join(', ')}`
      );
    }
  ...
  'router.active': async () => {
    if (!CONFIG.MIKROTIK_PASS) return { error: 'Router not configured' };
    return [{ user: 'user1', address: '192.168.88.100', uptime: '1h30m' }];
  },
...
class AgentOSGateway {
  ...
  _handleNodeRegister(clientId, ws, payload) {
    ...
    logger.info(
      `Node registered: ${payload.nodeId} (${payload.platform}) from ${ws.remoteAddress}`
    );
  ...
  _handleCommandInvoke(clientId, ws, msg) {
    ...
    global.commandHandler
      .execute(command, params || [], client.nodeId || clientId)
      .then(result => {
        this._send(ws, {
          type: 'command.result',
          id: msg.id,
