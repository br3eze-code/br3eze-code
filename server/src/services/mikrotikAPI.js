...
class MikroTikAPI {
  ...
  async createOrUpdateUser(username, password, options = {}) {
    return retryWithBackoff(async () => {
      ...
      if (existing) {
        ...
        const response = await this.client.put(`${this.baseURL}/ip/hotspot/user`, userData);
  ...
  async getUser(username) {
    try {
      const response = await this.client.get(`${this.baseURL}/ip/hotspot/user?name=${username}`);
  ...
  async deleteUser(username) {
    return retryWithBackoff(async () => {
      ...
      await this.client.delete(`${this.baseURL}/ip/hotspot/user/${user['.id']}`);
  ...
  async getActiveSessions() {
    return retryWithBackoff(async () => {
      const response = await this.client.get(`${this.baseURL}/ip/hotspot/active`);
      return response.data || [];
    });
  }
  ...
  async getUserSession(username) {
    try {
      const response = await this.client.get(`${this.baseURL}/ip/hotspot/active?user=${username}`);
      return response.data[0] || null;
    } catch (error) {
      return null;
    }
  ...
  async disconnectUser(username) {
    return retryWithBackoff(async () => {
      ...
      await this.client.delete(`${this.baseURL}/ip/hotspot/active/${session['.id']}`);
  ...
  async bindToMac(username, mac) {
    return retryWithBackoff(async () => {
      ...
      const response = await this.client.patch(`${this.baseURL}/ip/hotspot/user/${user['.id']}`, {
        'mac-address': mac,
      });
  ...
  async getWalledGarden() {
    return retryWithBackoff(async () => {
      const response = await this.client.get(`${this.baseURL}/ip/hotspot/walled-garden`);
      return response.data || [];
    });
  }
  ...
  async addWalledGardenEntry(entry) {
    return retryWithBackoff(async () => {
      const response = await this.client.put(`${this.baseURL}/ip/hotspot/walled-garden`, entry);
      return response.data;
    });
  }
  ...
  async removeWalledGardenEntry(id) {
    return retryWithBackoff(async () => {
      await this.client.delete(`${this.baseURL}/ip/hotspot/walled-garden/${id}`);
    });
  }
  ...
  async findUserByMac(mac) {
    try {
      const response = await this.client.get(`${this.baseURL}/ip/hotspot/user?mac-address=${mac}`);
      return response.data[0] || null;
    } catch (error) {
      return null;
    }
